/* ../netlib/strsen.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int strsen_(char *job, char *compq, logical *select, integer *n, real *t, integer *ldt, real *q, integer *ldq, real *wr, real *wi, integer *m, real *s, real *sep, real *work, integer *lwork, integer * iwork, integer *liwork, integer *info)
{
    ...
    *info = 0;
            ...
            ++k)
    {
        ...
        else
        {
            ...
            if (swap)
            {
                ...
                if (k != ks)
                {
                    strexc_(compq, n, &t[t_offset], ldt, &q[q_offset], ldq, & kk, &ks, &work[1], &ierr);
                }
                if (ierr == 1 || ierr == 2)
                {
    ...
    if (wants)
    {
        ...
        strsyl_("N", "N", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 1 + (n1 + 1) * t_dim1], ldt, &work[1], &n1, &scale, &ierr);
...
L30:
        ...
        if (kase != 0)
        {
            ...
            else
            {
                ...
                strsyl_("T", "T", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 1 + (n1 + 1) * t_dim1], ldt, &work[1], &n1, &scale, & ierr);
            }
            goto L30;
        }
        *sep = scale / est;
...
L40: /* Store the output eigenvalues in WR and WI. */
            ...
            ++k)
    {
        if (t[k + 1 + k * t_dim1] != 0.f)
        {
            wi[k] = sqrt((r__1 = t[k + (k + 1) * t_dim1], abs(r__1))) * sqrt(( r__2 = t[k + 1 + k * t_dim1], abs(r__2)));
            wi[k + 1] = -wi[k];
        }
        /* L60: */
    }
