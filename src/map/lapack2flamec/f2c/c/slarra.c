/* ../netlib/v3.9.0/slarra.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *nsplit = 1;
 ...
 ++i__) {
 eabs = (r__1 = e[i__], abs(r__1));
 if (eabs <= *spltol * sqrt((r__1 = d__[i__], abs(r__1))) * sqrt(( r__2 = d__[i__ + 1], abs(r__2)))) {
 e[i__] = 0.f;
 e2[i__] = 0.f;
 isplit[*nsplit] = i__;
 ++(*nsplit);
