/* ../netlib/v3.9.0/csytrf_aa.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
;
 ...
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/csytrf_ aa.f"> */
 /* > [TGZ]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/csytrf_ aa.f"> */
 /* > [ZIP]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/csytrf_ aa.f"> */
 /* > [TXT]</a> */
 /* > \endhtmlonly */
 /* Definition: */
 /* =========== */
 ...
 */
 ...
 char buffer[256]; 
#if FLA_ENABLE_ILP64 
 snprintf(buffer, 256,"csytrf_aa inputs: uplo %c, n %lld, lda %lld, lwork %lld",*uplo, *n, *lda, *lwork);
#else 
 snprintf(buffer, 256,"csytrf_aa inputs: uplo %c, n %d, lda %d, lwork %d",*uplo, *n, *lda, *lwork);
#endif
 ...
 */
 ...
 i__1 = 2 - k1;
 i__2 = *n - j;
 clasyf_aa_(uplo, &i__1, &i__2, &jb, &a[max(1,j) + (j + 1) * a_dim1], lda, &ipiv[j + 1], &work[1], n, &work[*n * nb + 1]) ;
 ...
 i__1 = j + (j + 1) * a_dim1;
 alpha.r = a[i__1].r; alpha.i = a[i__1].i; // , expr subst  
 i__1 = j + (j + 1) * a_dim1;
 ...
 --mj) {
 i__3 = jb + 1;
 cgemv_("No transpose", &mj, &i__3, &c_b19, &work[j3 - j1 + 1 + k1 * *n], n, &a[j1 - k2 + j3 * a_dim1], &c__1, &c_b15, &a[j3 + j3 * a_dim1], lda);
 ++j3;
 }
 /* Update off-diagonal block of J2-th block row with CGEMM */
 i__3 = *n - j3 + 1;
 i__4 = jb + 1;
 cgemm_("Transpose", "Transpose", &nj, &i__3, &i__4, & c_b19, &a[j1 - k2 + j2 * a_dim1], lda, &work[j3 - j1 + 1 + k1 * *n], n, &c_b15, &a[j2 + j3 * a_dim1] , lda);
 }
 /* Recover T( J, J+1 ) */
 i__2 = j + (j + 1) * a_dim1;
 a[i__2].r = alpha.r; a[i__2].i = alpha.i; // , expr subst  
 }
 ...
 */
 ...
 i__2 = 2 - k1;
 i__1 = *n - j;
 clasyf_aa_(uplo, &i__2, &i__1, &jb, &a[j + 1 + max(1,j) * a_dim1], lda, &ipiv[j + 1], &work[1], n, &work[*n * nb + 1]) ;
 /* Adjust IPIV and apply it back (J-th step picks (J+1)-th pivot) */
 /* Computing MIN */
 ...
 /* Merge rank-1 update with BLAS-3 update */
 i__2 = j + 1 + j * a_dim1;
 alpha.r = a[i__2].r; alpha.i = a[i__2].i; // , expr subst  
 i__2 = j + 1 + j * a_dim1;
 a[i__2].r = 1.f; a[i__2].i = 0.f; // , expr subst  
 i__2 = *n - j;
 ...
 --mj) {
 i__3 = jb + 1;
 cgemv_("No transpose", &mj, &i__3, &c_b19, &work[j3 - j1 + 1 + k1 * *n], n, &a[j3 + (j1 - k2) * a_dim1], lda, &c_b15, &a[j3 + j3 * a_dim1], & c__1);
 ++j3;
 }
 /* Update off-diagonal block in J2-th block column with CGEMM */
 i__3 = *n - j3 + 1;
 i__4 = jb + 1;
 cgemm_("No transpose", "Transpose", &i__3, &nj, &i__4, & c_b19, &work[j3 - j1 + 1 + k1 * *n], n, &a[j2 + ( j1 - k2) * a_dim1], lda, &c_b15, &a[j3 + j2 * a_dim1], lda);
 }
 /* Recover T( J+1, J ) */
 i__1 = j + 1 + j * a_dim1;
 a[i__1].r = alpha.r; a[i__1].i = alpha.i; // , expr subst  
 }
 /* WORK(J+1, 1) stores H(J+1, 1) */
 i__1 = *n - j;
