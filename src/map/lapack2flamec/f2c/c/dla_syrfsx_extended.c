...
/* ../netlib/dla_syrfsx_extended.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 on Linux or Unix systems, link with .../path/to/libf2c.a -lm or, if you install libf2c.a in a standard place, with -lf2c -lm -- in that order, at the end of the command line, as in cc *.o -lf2c -lm Source for libf2c is in /netlib/f2c/libf2c.zip, e.g., http://www.netlib.org/f2c/libf2c.zip */
 #include "FLA_f2c.h" /* Table of constant values */
 static integer c__1 = 1;
 static doublereal c_b12 = -1.;
 static doublereal c_b14 = 1.;
 /* > \brief \b DLA_SYRFSX_EXTENDED improves the computed solution to a system of linear equations for symmetri c indefinite matrices by performing extra-precise iterative refinement and provides error bounds and b ackward error estimates for the solution. */
 /* =========== DOCUMENTATION =========== */
 /* Online html documentation available at */
 /* http://www.netlib.org/lapack/explore-html/ */
 /* > \htmlonly */
 /* > Download DLA_SYRFSX_EXTENDED + dependencies */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/dla_syr fsx_extended.f"> */
 /* > [TGZ]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/dla_syr fsx_extended.f"> */
 /* > [ZIP]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/dla_syr fsx_extended.f"> */
 /* > [TXT]</a> */
 /* > \endhtmlonly */
 /* Definition: */
 /* =========== */
...
i+1}
 ...
 /* ===================================================================== */
 /* Subroutine */
 int dla_syrfsx_extended_(integer *prec_type__, char *uplo, integer *n, integer *nrhs, doublereal *a, integer *lda, doublereal * af, integer *ldaf, integer *ipiv, logical *colequ, doublereal *c__, doublereal *b, integer *ldb, doublereal *y, integer *ldy, doublereal * berr_out__, integer *n_norms__, doublereal *err_bnds_norm__, doublereal *err_bnds_comp__, doublereal *res, doublereal *ayb, doublereal *dy, doublereal *y_tail__, doublereal *rcond, integer * ithresh, doublereal *rthresh, doublereal *dz_ub__, logical * ignore_cwise__, integer *info) {
 AOCL_DTL_TRACE_ENTRY(AOCL_DTL_LEVEL_TRACE_5);
#if AOCL_DTL_LOG_ENABLE 
 char buffer[256]; 
 snprintf(buffer, 256,"dla_syrfsx_extended inputs: prec_type__ %" FLA_IS ", uplo %c, n %" FLA_IS ", nrhs %" FLA_IS ", lda %" FLA_IS ", ldaf %" FLA_IS ", ipiv %" FLA_IS ", ldb %" FLA_IS ", ldy %" FLA_IS ", n_norms__ %" FLA_IS ", ithresh %" FLA_IS "",*prec_type__, *uplo, *n, *nrhs, *lda, *ldaf, *ipiv, *ldb, *ldy, *n_norms__, *ithresh);
 ...
 logical upper;
 extern /* Subroutine */
 int dsymv_(char *, integer *, doublereal *, doublereal *, integer *, doublereal *, integer *, doublereal *, doublereal *, integer *);
 ...
 *info = -15;
 ...
 if (*info != 0) {
 i__1 = -(*info);
 xerbla_("DLA_SYRFSX_EXTENDED", &i__1);
 ...
 if (y_prec_state__ == 0) {
 dsymv_(uplo, n, &c_b12, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1, &c_b14, &res[1], &c__1);
 }
 else if (y_prec_state__ == 1) {
 blas_dsymv_x_(&uplo2, n, &c_b12, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1, &c_b14, &res[1], &c__1, prec_type__);
 }
 else {
 blas_dsymv2_x_(&uplo2, n, &c_b12, &a[a_offset], lda, &y[j * y_dim1 + 1], &y_tail__[1], &c__1, &c_b14, &res[1], & c__1, prec_type__);
 }
 /* XXX: RES is no longer needed. */
 dcopy_(n, &res[1], &c__1, &dy[1], &c__1);
 dsytrs_(uplo, n, &c__1, &af[af_offset], ldaf, &ipiv[1], &dy[1], n, info);
 ...
 if (yk != 0.) {
 /* Computing MAX */
 d__1 = dz_z__; d__2 = dyk / yk; // , expr subst  
 ...
 if (*colequ) {
 /* Computing MAX */
 d__1 = normx; d__2 = yk * c__[i__]; // , expr subst  
 normx = max(d__1,d__2);
 /* Computing MAX */
 d__1 = normdx; d__2 = dyk * c__[i__]; // , expr subst  
 ...
 dla_syamv_(&uplo2, n, &c_b14, &a[a_offset], lda, &y[j * y_dim1 + 1], &c__1, &c_b14, &ayb[1], &c__1);
