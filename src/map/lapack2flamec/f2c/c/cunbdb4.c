/* ../netlib/cunbdb4.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
    ...
    *info = 0;
            ...
            ++i__)
    {
        if (i__ == 1)
        {
            ...
            s = sin(theta[i__]);
            phantom[1].r = 1.f;
            phantom[1].i = 0.f; // , expr subst
            i__2 = *p + 1;
            phantom[i__2].r = 1.f;
            phantom[i__2].i = 0.f; // , expr subst
            r_cnjg(&q__1, &taup1[1]);
