/* ../netlib/v3.9.0/dggsvp3.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 char buffer[256]; 
 snprintf(buffer, 256,"dggsvp3 inputs: jobu %c, jobv %c, jobq %c, m %" FLA_IS ", p %" FLA_IS ", n %" FLA_IS ", lda %" FLA_IS ", ldb %" FLA_IS ", ldu %" FLA_IS ", ldv %" FLA_IS ", ldq %" FLA_IS ", lwork %" FLA_IS "",*jobu, *jobv, *jobq, *m, *p, *n, *lda, *ldb, *ldu, *ldv, *ldq, *lwork);
 ...
 extern /* Subroutine */
 int dgeqp3_(integer *, integer *, doublereal *, integer *, integer *, doublereal *, doublereal *, integer *, integer *), dgeqr2_(integer *, integer *, doublereal *, integer *, doublereal *, doublereal *, integer *), dgerq2_(integer *, integer *, doublereal *, integer *, doublereal *, doublereal *, integer *), dorg2r_(integer *, integer *, integer *, doublereal *, integer *, doublereal *, doublereal *, integer *), dorm2r_(char * , char *, integer *, integer *, integer *, doublereal *, integer * , doublereal *, doublereal *, integer *, doublereal *, integer *), dormr2_(char *, char *, integer *, integer *, integer *, doublereal *, integer *, doublereal *, doublereal *, integer *, doublereal *, integer *), dlacpy_(char *, integer *, integer *, doublereal *, integer *, doublereal *, integer *), dlaset_(char *, integer *, integer *, doublereal *, doublereal *, doublereal *, integer *), xerbla_(char *, integer *), dlapmt_(logical *, integer *, integer *, doublereal *, integer *, integer *);
 logical forwrd;
 integer lwkopt;
 logical lquery;
 ...
 *info = -24;
 ...
 /* Compute workspace */
 if (*info == 0) {
 dgeqp3_(p, n, &b[b_offset], ldb, &iwork[1], &tau[1], &work[1], &c_n1, info);
 lwkopt = (integer) work[1];
 if (wantv) {
 lwkopt = max(lwkopt,*p);
 }
 /* Computing MAX */
 i__1 = lwkopt; i__2 = min(*n,*p); // , expr subst  
 lwkopt = max(i__1,i__2);
 lwkopt = max(lwkopt,*m);
 if (wantq) {
 lwkopt = max(lwkopt,*n);
 }
 dgeqp3_(m, n, &a[a_offset], lda, &iwork[1], &tau[1], &work[1], &c_n1, info);
 /* Computing MAX */
 i__1 = lwkopt; i__2 = (integer) work[1]; // , expr subst  
 lwkopt = max(i__1,i__2);
 lwkopt = max(1,lwkopt);
 ...
 dgeqp3_(p, n, &b[b_offset], ldb, &iwork[1], &tau[1], &work[1], lwork, info);
 /* Update A := A*P */
 ...
 *l = 0;
 ...
 i__ <= i__1;
 ++i__) {
 if ((d__1 = b[i__ + i__ * b_dim1], f2c_dabs(d__1)) > *tolb) {
 ++(*l);
 }
 /* L20: */
 }
 ...
 dlaset_("Full", p, p, &c_b14, &c_b14, &v[v_offset], ldv);
 if (*p > 1) {
 i__1 = *p - 1;
 ...
 dlaset_("Full", n, n, &c_b14, &c_b24, &q[q_offset], ldq);
 ...
 /* Clean up B */
 i__1 = *n - *l;
 dlaset_("Full", l, &i__1, &c_b14, &c_b14, &b[b_offset], ldb);
 i__1 = *n;
 for (j = *n - *l + 1;
 j <= i__1;
 ++j) {
 ...
 i__1 = *n - *l;
 dgeqp3_(m, &i__1, &a[a_offset], lda, &iwork[1], &tau[1], &work[1], lwork, info);
 /* Determine the effective rank of A11 */
 *k = 0;
 /* Computing MIN */
 i__2 = *m; i__3 = *n - *l; // , expr subst  
 i__1 = min(i__2,i__3);
 for (i__ = 1;
 i__ <= i__1;
 ++i__) {
 if ((d__1 = a[i__ + i__ * a_dim1], f2c_dabs(d__1)) > *tola) {
 ++(*k);
 }
 /* L80: */
 }
 ...
 dlaset_("Full", m, m, &c_b14, &c_b14, &u[u_offset], ldu);
 if (*m > 1) {
 i__1 = *m - 1;
 i__2 = *n - *l;
 ...
 i__1 = *m - *k;
 i__2 = *n - *l;
 dlaset_("Full", &i__1, &i__2, &c_b14, &c_b14, &a[*k + 1 + a_dim1], lda);
 }
 if (*n - *l > *k) {
 ...
 /* Clean up A */
 i__1 = *n - *l - *k;
 dlaset_("Full", k, &i__1, &c_b14, &c_b14, &a[a_offset], lda);
 i__1 = *n - *l;
 for (j = *n - *l - *k + 1;
 j <= i__1;
 ++j) {
