/* ../netlib/v3.9.0/zggbal.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *ihi = *n;
 ...
 i__2 = i__ + j * a_dim1;
 i__3 = i__ + j * b_dim1;
 if (a[i__2].r != 0. || a[i__2].i != 0. || (b[i__3].r != 0. || b[ i__3].i != 0.)) {
 goto L50;
 }
 /* L40: */
 }
 ...
 *ihi = l;
 ...
 ta = d_lg10(&d__3) / basl;
 L210: i__3 = i__ + j * b_dim1;
 if (b[i__3].r == 0. && b[i__3].i == 0.) {
 tb = 0.;
 goto L220;
 }
 i__3 = i__ + j * b_dim1;
 d__3 = (d__1 = b[i__3].r, abs(d__1)) + (d__2 = d_imag(&b[i__ + j * b_dim1]), abs(d__2));
 tb = d_lg10(&d__3) / basl;
 L220: work[i__ + (*n << 2)] = work[i__ + (*n << 2)] - ta - tb;
 ...
 L250: gamma = ddot_(&nr, &work[*ilo + (*n << 2)], &c__1, &work[*ilo + (*n << 2)] , &c__1) + ddot_(&nr, &work[*ilo + *n * 5], &c__1, &work[*ilo + * n * 5], &c__1);
 ew = 0.;
 ewc = 0.;
 i__1 = *ihi;
 for (i__ = *ilo;
 ...
 /* Computing MAX */
 d__1 = rab; d__2 = z_abs(&b[i__ + (irab + *ilo - 1) * b_dim1]); // , expr subst  
 rab = max(d__1,d__2);
 d__1 = rab + sfmin;
 lrab = (integer) (d_lg10(&d__1) / basl + 1.);
 ir = (integer) (lscale[i__] + d_sign(&c_b72, &lscale[i__]));
 /* Computing MIN */
 i__2 = max(ir,lsfmin); i__2 = min(i__2,lsfmax); i__3 = lsfmax - lrab; // ; expr subst  
 ir = min(i__2,i__3);
 ...
 /* Computing MAX */
 d__1 = cab; d__2 = z_abs(&b[icab + i__ * b_dim1]); // , expr subst  
 cab = max(d__1,d__2);
 d__1 = cab + sfmin;
 lcab = (integer) (d_lg10(&d__1) / basl + 1.);
 jc = (integer) (rscale[i__] + d_sign(&c_b72, &rscale[i__]));
 /* Computing MIN */
 i__2 = max(jc,lsfmin); i__2 = min(i__2,lsfmax); i__3 = lsfmax - lcab; // ; expr subst  
 jc = min(i__2,i__3);
