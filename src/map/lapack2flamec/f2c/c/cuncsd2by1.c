/* ../netlib/v3.9.0/cuncsd2by1.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
;
 ...
 extern /* Subroutine */
 int cunbdb1_(integer *, integer *, integer *, complex *, integer *, complex *, integer *, real *, real *, complex *, complex *, complex *, complex *, integer *, integer *), cunbdb2_(integer *, integer *, integer *, complex *, integer *, complex *, integer *, real *, real *, complex *, complex *, complex *, complex *, integer *, integer *);
 logical lquery;
 extern /* Subroutine */
 ...
 *info = -17;
 ...
 /* Computing MIN */
 i__1 = *p, i__2 = *m - *p, i__1 = min(i__1,i__2); i__1 = min(i__1,*q); i__2 = *m - *q; // ; expr subst  
 r__ = min(i__1,i__2);
 /* Compute workspace */
 /* WORK layout: */
 /* |-----------------------------------------| */
 ...
 lorglqopt = 1;
 if (r__ == *q) {
 cunbdb1_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &theta[1], dum, cdum, cdum, cdum, &work[1], &c_n1, &childinfo);
 lorbdb = (integer) work[1].r;
 if (wantu1 && *p > 0) {
 cungqr_(p, p, q, &u1[u1_offset], ldu1, cdum, &work[1], &c_n1, &childinfo);
 lorgqrmin = max(lorgqrmin,*p);
 /* Computing MAX */
 i__1 = lorgqropt; i__2 = (integer) work[1].r; // , expr subst  
 ...
 /* Computing MAX */
 i__1 = lorgqrmin; i__2 = *m - *p; // , expr subst  
 ...
 i__2 = *q - 1;
 i__3 = *q - 1;
 cunglq_(&i__1, &i__2, &i__3, &v1t[v1t_offset], ldv1t, cdum, & work[1], &c_n1, &childinfo);
 /* Computing MAX */
 i__1 = lorglqmin; i__2 = *q - 1; // , expr subst  
 lorglqmin = max(i__1,i__2);
 /* Computing MAX */
 i__1 = lorglqopt; i__2 = (integer) work[1].r; // , expr subst  
 ...
 cunbdb2_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &theta[1], dum, cdum, cdum, cdum, &work[1], &c_n1, &childinfo);
 lorbdb = (integer) work[1].r;
 if (wantu1 && *p > 0) {
 i__1 = *p - 1;
 i__2 = *p - 1;
 i__3 = *p - 1;
 cungqr_(&i__1, &i__2, &i__3, &u1[(u1_dim1 << 1) + 2], ldu1, cdum, &work[1], &c_n1, &childinfo);
 /* Computing MAX */
 i__1 = lorgqrmin; i__2 = *p - 1; // , expr subst  
 lorgqrmin = max(i__1,i__2);
 /* Computing MAX */
 i__1 = lorgqropt; i__2 = (integer) work[1].r; // , expr subst  
 lorgqropt = max(i__1,i__2);
 ...
 cunbdb3_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &theta[1], dum, cdum, cdum, cdum, &work[1], &c_n1, &childinfo);
 lorbdb = (integer) work[1].r;
 if (wantu1 && *p > 0) {
 cungqr_(p, p, q, &u1[u1_offset], ldu1, cdum, &work[1], &c_n1, &childinfo);
 lorgqrmin = max(lorgqrmin,*p);
 ...
 i__2 = *m - *p - 1;
 i__3 = *m - *p - 1;
 cungqr_(&i__1, &i__2, &i__3, &u2[(u2_dim1 << 1) + 2], ldu2, cdum, &work[1], &c_n1, &childinfo);
 /* Computing MAX */
 i__1 = lorgqrmin; i__2 = *m - *p - 1; // , expr subst  
 lorgqrmin = max(i__1,i__2);
 /* Computing MAX */
 i__1 = lorgqropt; i__2 = (integer) work[1].r; // , expr subst  
 lorgqropt = max(i__1,i__2);
 ...
 i__1 = *m - *q;
 i__2 = *m - *p;
 cbbcsd_("N", jobv1t, jobu2, jobu1, "T", m, &i__1, &i__2, &theta[1] , dum, cdum, &c__1, &v1t[v1t_offset], ldv1t, &u2[ u2_offset], ldu2, &u1[u1_offset], ldu1, dum, dum, dum, dum, dum, dum, dum, dum, &rwork[1], &c_n1, &childinfo);
 lbbcsd = (integer) rwork[1];
 }
 else {
 cunbdb4_(m, p, q, &x11[x11_offset], ldx11, &x21[x21_offset], ldx21, &theta[1], dum, cdum, cdum, cdum, cdum, &work[1], & c_n1, &childinfo);
 ...
 i__2 = *m - *p;
 i__3 = *m - *q;
 cungqr_(&i__1, &i__2, &i__3, &u2[u2_offset], ldu2, cdum, & work[1], &c_n1, &childinfo);
 /* Computing MAX */
 i__1 = lorgqrmin; i__2 = *m - *p; // , expr subst  
 lorgqrmin = max(i__1,i__2);
 /* Computing MAX */
 ...
 i__1 = *m - *p;
 i__2 = *m - *q;
 cbbcsd_(jobu2, jobu1, "N", jobv1t, "N", m, &i__1, &i__2, &theta[1] , dum, &u2[u2_offset], ldu2, &u1[u1_offset], ldu1, cdum, & c__1, &v1t[v1t_offset], ldv1t, dum, dum, dum, dum, dum, dum, dum, dum, &rwork[1], &c_n1, &childinfo);
 lbbcsd = (integer) rwork[1];
 }
 ...
 /* Computing MAX */
 i__1 = iorbdb + lorbdb - 1, i__2 = iorgqr + lorgqrmin - 1; i__1 = max( i__1,i__2); i__2 = iorglq + lorglqmin - 1; // ; expr subst  
 lworkmin = max(i__1,i__2);
 /* Computing MAX */
 i__1 = iorbdb + lorbdb - 1, i__2 = iorgqr + lorgqropt - 1; i__1 = max( i__1,i__2); i__2 = iorglq + lorglqopt - 1; // ; expr subst  
 ...
 *info = -19;
 ...
 if (wantv1t && *q > 0) {
 i__1 = v1t_dim1 + 1;
 v1t[i__1].r = 1.f; v1t[i__1].i = 0.f; // , expr subst  
 i__1 = *q;
 for (j = 2;
 j <= i__1;
 ++j) {
 i__2 = j * v1t_dim1 + 1;
 v1t[i__2].r = 0.f; v1t[i__2].i = 0.f; // , expr subst  
 i__2 = j + v1t_dim1;
 v1t[i__2].r = 0.f; v1t[i__2].i = 0.f; // , expr subst  
 }
 i__1 = *q - 1;
 ...
 if (wantu1 && *p > 0) {
 i__1 = u1_dim1 + 1;
 u1[i__1].r = 1.f; u1[i__1].i = 0.f; // , expr subst  
 i__1 = *p;
 for (j = 2;
 j <= i__1;
 ++j) {
 i__2 = j * u1_dim1 + 1;
 u1[i__2].r = 0.f; u1[i__2].i = 0.f; // , expr subst  
 i__2 = j + u1_dim1;
 u1[i__2].r = 0.f; u1[i__2].i = 0.f; // , expr subst  
 }
 i__1 = *p - 1;
 ...
 /* Simultaneously diagonalize X11 and X21. */
 cbbcsd_(jobv1t, "N", jobu1, jobu2, "T", m, q, p, &theta[1], &rwork[ iphi], &v1t[v1t_offset], ldv1t, cdum, &c__1, &u1[u1_offset], ldu1, &u2[u2_offset], ldu2, &rwork[ib11d], &rwork[ib11e], & rwork[ib12d], &rwork[ib12e], &rwork[ib21d], &rwork[ib21e], & rwork[ib22d], &rwork[ib22e], &rwork[ibbcsd], &lbbcsd, & childinfo);
 ...
 if (wantu2 && *m - *p > 0) {
 i__1 = u2_dim1 + 1;
 u2[i__1].r = 1.f; u2[i__1].i = 0.f; // , expr subst  
 i__1 = *m - *p;
 for (j = 2;
 j <= i__1;
 ++j) {
 i__2 = j * u2_dim1 + 1;
 u2[i__2].r = 0.f; u2[i__2].i = 0.f; // , expr subst  
 i__2 = j + u2_dim1;
 u2[i__2].r = 0.f; u2[i__2].i = 0.f; // , expr subst  
 }
 i__1 = *m - *p - 1;
 ...
 i__1 = *m - *q;
 i__2 = *m - *p;
 cbbcsd_("N", jobv1t, jobu2, jobu1, "T", m, &i__1, &i__2, &theta[1], & rwork[iphi], cdum, &c__1, &v1t[v1t_offset], ldv1t, &u2[ u2_offset], ldu2, &u1[u1_offset], ldu1, &rwork[ib11d], &rwork[ ib11e], &rwork[ib12d], &rwork[ib12e], &rwork[ib21d], &rwork[ ib21e], &rwork[ib22d], &rwork[ib22e], &rwork[ibbcsd], &lbbcsd, &childinfo);
 /* Permute rows and columns to place identity submatrices in */
 /* preferred positions */
 if (*q > r__) {
 i__1 = r__;
 ...
 i__1 = *m - *p;
 i__2 = *m - *q;
 cbbcsd_(jobu2, jobu1, "N", jobv1t, "N", m, &i__1, &i__2, &theta[1], & rwork[iphi], &u2[u2_offset], ldu2, &u1[u1_offset], ldu1, cdum, &c__1, &v1t[v1t_offset], ldv1t, &rwork[ib11d], &rwork[ib11e], &rwork[ib12d], &rwork[ib12e], &rwork[ib21d], &rwork[ib21e], & rwork[ib22d], &rwork[ib22e], &rwork[ibbcsd], &lbbcsd, & childinfo);
 /* Permute rows and columns to place identity submatrices in */
 /* preferred positions */
 if (*p > r__) {
 i__1 = r__;
