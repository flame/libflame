...
void Read_EIG_params( const char *file_name )
{
   ...
   fscanf(fp, "%s", &line[0]); // Range_start
      ...
      for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &eig_paramslist[i].n_range_start);
      eig_paramslist[i].num_ranges = num_ranges;
      eig_paramslist[i].mode=mode;
   }

   fscanf(fp, "%s", &line[0]); // Range_end
   for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &eig_paramslist[i].n_range_end);
   }

   fscanf(fp, "%s", &line[0]); // Range_step_size
   for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &eig_paramslist[i].n_range_step_size);
   }

   fscanf(fp, "%s", &line[0]);
   fscanf(fp, "%d", &num_tests);
   for (i=0; i<num_tests; i++){
      eig_paramslist[i].num_tests = num_tests;
   }
...
/* Read the mode */
   ...
   fscanf(fp, "%s", &line[0]); // Range_start
      ...
      for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &eig_non_sym_paramslist[i].n_range_start);
      eig_non_sym_paramslist[i].num_ranges = num_ranges;
      eig_non_sym_paramslist[i].mode=mode;
   }

   fscanf(fp, "%s", &line[0]); // Range_end
   for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &eig_non_sym_paramslist[i].n_range_end);
   }

   fscanf(fp, "%s", &line[0]); // Range_step_size
   for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &eig_non_sym_paramslist[i].n_range_step_size);
   }

   fscanf(fp, "%s", &line[0]);
   fscanf(fp, "%d", &num_tests);
   for (i=0; i<num_tests; i++){
      eig_non_sym_paramslist[i].num_tests = num_tests;
   }
   fscanf(fp, "%s", &line[0]);//n
   for (i=0; i<num_tests; i++){
      fscanf(fp, "%d", &eig_non_sym_paramslist[i].n);
   }
   fscanf(fp, "%s", &line[0]);
   for (i=0; i<num_tests; i++){
      fscanf(fp, "%s", str);
...
void Read_SVD_param ( const char *file_name )
{
   ...
   for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &svd_paramslist[i].m_range_start);
      svd_paramslist[i].num_ranges = num_ranges;
      svd_paramslist[i].mode = mode;
   }

   fscanf(fp, "%s", &line[0]); // Range_end
   for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &svd_paramslist[i].m_range_end);
   }

   fscanf(fp, "%s", &line[0]); // Range_step_size
   for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &svd_paramslist[i].m_range_step_size);
   }

   fscanf(fp, "%s", &line[0]); // Range_start

   for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &svd_paramslist[i].n_range_start);
   }

   fscanf(fp, "%s", &line[0]); // Range_end
   for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &svd_paramslist[i].n_range_end);
   }

   fscanf(fp, "%s", &line[0]); // Range_step_size
   for (i=0; i<num_ranges; i++){
      fscanf(fp, "%d", &svd_paramslist[i].n_range_step_size);
   }

   fscanf(fp, "%s", &line[0]);
   fscanf(fp, "%d", &num_tests);
   for (i=0; i<num_tests; i++){
      svd_paramslist[i].num_tests = num_tests;
   }
