...
/*! @file libflame_heev_2stage.cc
 *  @brief Test application to validate heev_2stage() using CPP template
           interface.
 *  */
...
/*! @brief  heev_2stage_test is function template for heev_2stage() functions.
			T can be scomplex, dcomplex
			Ta can be float, double.
 * @details
 * \b Purpose:
    \verbatim
	  heev_2stage_test is function template for heev_2stage() functions.
	  T can be scomplex, dcomplex
	  Ta can be float, double.
	  
	  heev_2stage_test() function template calls C and CPP based lbrary APIs with
    ...
    Fails the test case if difference > threshold or INFO < 0.
	  ...
	  Complex reference:
	  http://www.netlib.org/lapack/explore-html/d9/de3/group__complex_h_eeigen_gaf52d306acb40ea108aa4b06bb0fbae43.html#gaf52d306acb40ea108aa4b06bb0fbae43
	  Complex double reference:
	  http://www.netlib.org/lapack/explore-html/df/d9a/group__complex16_h_eeigen_gacb5dc319f9656b8f390480115d83795c.html#gacb5dc319f9656b8f390480115d83795c
    \endverbatim
...
void heev_2stage_test(int ip)
{
  typedef int (*fptr_NL_LAPACK_heev_2stage)(char* jobz, char* uplo,
                  integer* n, T* a, integer* lda, Ta* w, T* work,
                  integer* lwork, Ta* rwork, integer* info);
  fptr_NL_LAPACK_heev_2stage heev_2stage_ref;
  ...
  // RWORK is REAL or DOUBLE PRECISION array, dimension fla_max(1,3*N-2)
  ...
  if (lwork == -1) {
    ...
    // Call CPP function
    libflame::heev_2stage<T, Ta>(&jobz, &uplo, &n, abuff, &lda, wbuff,
                &worksize, &lwork, rworkbuff, &info_cpp);
  ...
  if ((lwork_size < 1) && (n <= 1)) {
    PRINTF("lwork < 1 but it should be: lwork >= 1 when n <= 1. Please " \
           "correct the input data.");
  }
  ...
  info_cpp = -1;
  // Call CPP function
  libflame::heev_2stage<T, Ta>(&jobz, &uplo, &n, abuff, &lda, wbuff, workbuff,
              &lwork_size, rworkbuff, &info_cpp);

  /* Check the typename T passed to this function template and call respective
     function.*/
  if (typeid(T) == typeid(scomplex)) {
    heev_2stage_ref = (fptr_NL_LAPACK_heev_2stage)dlsym(lapackModule,
                        "cheev_2stage_");
  ...
  // Call C function
  integer info_ref = -1;
  heev_2stage_ref(&jobz, &uplo, &n, arefbuff, &lda, wrefbuff, workrefbuff,
...
TEST(LAPACKCPP_heev_2stage, CHEEV_2STAGE) {
  for (short int index = 0; index < NUM_SUB_TESTS; index++) {
	  PRINTF("index: %d\n", index);
    heev_2stage_test<scomplex, float> (index);
  }
}

/* Use TEST macro and call C++ test function template with
   dcomplex and double as typenames.*/
TEST(LAPACKCPP_heev_2stage, ZHEEV_2STAGE) {
  for (short int index = 0; index < NUM_SUB_TESTS; index++) {
	  PRINTF("index: %d\n", index);
    heev_2stage_test<dcomplex, double> (index);
  }
}