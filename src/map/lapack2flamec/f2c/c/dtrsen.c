/* ../netlib/dtrsen.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int dtrsen_(char *job, char *compq, logical *select, integer *n, doublereal *t, integer *ldt, doublereal *q, integer *ldq, doublereal *wr, doublereal *wi, integer *m, doublereal *s, doublereal *sep, doublereal *work, integer *lwork, integer *iwork, integer * liwork, integer *info)
{
    ...
    char buffer[256]; 
    snprintf(buffer, 256,"dtrsen inputs: job %c, compq %c, n %" FLA_IS ", ldt %" FLA_IS ", ldq %" FLA_IS ", lwork %" FLA_IS ", liwork %" FLA_IS "",*job, *compq, *n, *ldt, *ldq, *lwork, *liwork);
    ...
    *info = 0;
            ...
            ++k)
    {
        ...
        else
        {
            ...
            if (swap)
            {
                ...
                if (k != ks)
                {
                    dtrexc_(compq, n, &t[t_offset], ldt, &q[q_offset], ldq, & kk, &ks, &work[1], &ierr);
                }
                if (ierr == 1 || ierr == 2)
                {
    ...
    if (wants)
    {
        ...
        dtrsyl_("N", "N", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 1 + (n1 + 1) * t_dim1], ldt, &work[1], &n1, &scale, &ierr);
...
L30:
        ...
        if (kase != 0)
        {
            ...
            else
            {
                ...
                dtrsyl_("T", "T", &c_n1, &n1, &n2, &t[t_offset], ldt, &t[n1 + 1 + (n1 + 1) * t_dim1], ldt, &work[1], &n1, &scale, & ierr);
            }
            goto L30;
        }
        *sep = scale / est;
...
L40: /* Store the output eigenvalues in WR and WI. */
            ...
            ++k)
    {
        if (t[k + 1 + k * t_dim1] != 0.)
        {
            wi[k] = sqrt((d__1 = t[k + (k + 1) * t_dim1], f2c_dabs(d__1))) * sqrt(( d__2 = t[k + 1 + k * t_dim1], f2c_dabs(d__2)));
            wi[k + 1] = -wi[k];
        }
        /* L60: */
    }
