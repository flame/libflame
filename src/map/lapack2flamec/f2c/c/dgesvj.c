/* ../netlib/dgesvj.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -6;
 ...
 /* Computing MIN */
 d__1 = big; d__2 = temp1 / aapp; // , expr subst  
 temp1 = min(d__1,d__2);
 /* AAQQ = AAQQ*TEMP1 */
 /* AAPP = AAPP*TEMP1 */
 }
 ...
 /* Computing MIN */
 d__1 = sn / aaqq; d__2 = big / (aapp * sqrt((doublereal) (*n))); // , expr subst  
 temp1 = min(d__1,d__2);
 /* AAQQ = AAQQ*TEMP1 */
 /* AAPP = AAPP*TEMP1 */
 }
 ...
 /* Computing MAX */
 d__1 = sn / aaqq; d__2 = temp1 / aapp; // , expr subst  
 temp1 = max(d__1,d__2);
 /* AAQQ = AAQQ*TEMP1 */
 /* AAPP = AAPP*TEMP1 */
 }
 ...
 /* Computing MIN */
 d__1 = sn / aaqq; d__2 = big / (sqrt((doublereal) (*n)) * aapp); // , expr subst  
 temp1 = min(d__1,d__2);
 /* AAQQ = AAQQ*TEMP1 */
 /* AAPP = AAPP*TEMP1 */
 }
 ...
 i__2 = *n - n34;
 i__3 = *lwork - *n;
 dgsvj0_(jobv, &i__1, &i__2, &a[n34 + 1 + (n34 + 1) * a_dim1], lda, &work[n34 + 1], &sva[n34 + 1], &mvl, &v[n34 * q + 1 + ( n34 + 1) * v_dim1], ldv, &epsln, &sfmin, &tol, &c__2, & work[*n + 1], &i__3, &ierr);
 i__1 = *m - n2;
 i__2 = n34 - n2;
 i__3 = *lwork - *n;
 dgsvj0_(jobv, &i__1, &i__2, &a[n2 + 1 + (n2 + 1) * a_dim1], lda, & work[n2 + 1], &sva[n2 + 1], &mvl, &v[n2 * q + 1 + (n2 + 1) * v_dim1], ldv, &epsln, &sfmin, &tol, &c__2, &work[*n + 1], &i__3, &ierr);
 i__1 = *m - n2;
 i__2 = *n - n2;
 i__3 = *lwork - *n;
 dgsvj1_(jobv, &i__1, &i__2, &n4, &a[n2 + 1 + (n2 + 1) * a_dim1], lda, &work[n2 + 1], &sva[n2 + 1], &mvl, &v[n2 * q + 1 + ( n2 + 1) * v_dim1], ldv, &epsln, &sfmin, &tol, &c__1, & work[*n + 1], &i__3, &ierr);
 i__1 = *m - n4;
 i__2 = n2 - n4;
 i__3 = *lwork - *n;
 dgsvj0_(jobv, &i__1, &i__2, &a[n4 + 1 + (n4 + 1) * a_dim1], lda, & work[n4 + 1], &sva[n4 + 1], &mvl, &v[n4 * q + 1 + (n4 + 1) * v_dim1], ldv, &epsln, &sfmin, &tol, &c__1, &work[*n + 1], &i__3, &ierr);
 i__1 = *lwork - *n;
 dgsvj0_(jobv, m, &n4, &a[a_offset], lda, &work[1], &sva[1], &mvl, &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__1, &work[*n + 1], &i__1, &ierr);
 i__1 = *lwork - *n;
 dgsvj1_(jobv, m, &n2, &n4, &a[a_offset], lda, &work[1], &sva[1], & mvl, &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__1, & work[*n + 1], &i__1, &ierr);
 }
 else if (upper) {
 i__1 = *lwork - *n;
 dgsvj0_(jobv, &n4, &n4, &a[a_offset], lda, &work[1], &sva[1], & mvl, &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__2, & work[*n + 1], &i__1, &ierr);
 i__1 = *lwork - *n;
 dgsvj0_(jobv, &n2, &n4, &a[(n4 + 1) * a_dim1 + 1], lda, &work[n4 + 1], &sva[n4 + 1], &mvl, &v[n4 * q + 1 + (n4 + 1) * v_dim1], ldv, &epsln, &sfmin, &tol, &c__1, &work[*n + 1], &i__1, &ierr);
 i__1 = *lwork - *n;
 dgsvj1_(jobv, &n2, &n2, &n4, &a[a_offset], lda, &work[1], &sva[1], &mvl, &v[v_offset], ldv, &epsln, &sfmin, &tol, &c__1, & work[*n + 1], &i__1, &ierr);
 i__1 = n2 + n4;
 i__2 = *lwork - *n;
 dgsvj0_(jobv, &i__1, &n4, &a[(n2 + 1) * a_dim1 + 1], lda, &work[ n2 + 1], &sva[n2 + 1], &mvl, &v[n2 * q + 1 + (n2 + 1) * v_dim1], ldv, &epsln, &sfmin, &tol, &c__1, &work[*n + 1], &i__2, &ierr);
 }
 }
 ...
 if (aapp < big / aaqq) {
 aapq = ddot_(m, &a[p * a_dim1 + 1], & c__1, &a[q * a_dim1 + 1], & c__1) * work[p] * work[q] / aaqq / aapp;
 }
 else {
 dcopy_(m, &a[p * a_dim1 + 1], &c__1, & work[*n + 1], &c__1);
 dlascl_("G", &c__0, &c__0, &aapp, & work[p], m, &c__1, &work[*n + 1], lda, &ierr);
 aapq = ddot_(m, &work[*n + 1], &c__1, &a[q * a_dim1 + 1], &c__1) * work[q] / aaqq;
 }
 }
 else {
 ...
 dlascl_("G", &c__0, &c__0, &aaqq, & work[q], m, &c__1, &work[*n + 1], lda, &ierr);
 aapq = ddot_(m, &work[*n + 1], &c__1, &a[p * a_dim1 + 1], &c__1) * work[p] / aapp;
 }
 }
 /* Computing MAX */
 d__1 = mxaapq; d__2 = f2c_abs(aapq); // , expr subst  
 ...
 /* Computing MAX */
 d__1 = 0.; d__2 = t * apoaq * aapq + 1.; // , expr subst  
 sva[q] = aaqq * sqrt((max(d__1, d__2)));
 /* Computing MAX */
 d__1 = 0.; d__2 = 1. - t * aqoap * aapq; // , expr subst  
 aapp *= sqrt((max(d__1,d__2)));
 /* Computing MAX */
 d__1 = mxsinj; d__2 = f2c_abs(t); // , expr subst  
 ...
 sn = t * cs;
 /* Computing MAX */
 d__1 = mxsinj; d__2 = f2c_abs(sn); // , expr subst  
 mxsinj = max(d__1,d__2);
 /* Computing MAX */
 d__1 = 0.; d__2 = t * apoaq * aapq + 1.; // , expr subst  
 sva[q] = aaqq * sqrt((max(d__1, d__2)));
 ...
 dcopy_(m, &a[p * a_dim1 + 1], &c__1, & work[*n + 1], &c__1);
 dlascl_("G", &c__0, &c__0, &aapp, & c_b18, m, &c__1, &work[*n + 1] , lda, &ierr);
 dlascl_("G", &c__0, &c__0, &aaqq, & c_b18, m, &c__1, &a[q * a_dim1 + 1], lda, &ierr);
 temp1 = -aapq * work[p] / work[q];
 daxpy_(m, &temp1, &work[*n + 1], & c__1, &a[q * a_dim1 + 1], & c__1);
 dlascl_("G", &c__0, &c__0, &c_b18, & aaqq, m, &c__1, &a[q * a_dim1 + 1], lda, &ierr);
 /* Computing MAX */
 d__1 = 0.; d__2 = 1. - aapq * aapq; // , expr subst  
 sva[q] = aaqq * sqrt((max(d__1,d__2))) ;
 ...
 dcopy_(m, &a[q * a_dim1 + 1], & c__1, &work[*n + 1], & c__1);
 dlascl_("G", &c__0, &c__0, &aaqq, &c_b18, m, &c__1, &work[* n + 1], lda, &ierr);
 dlascl_("G", &c__0, &c__0, &aapp, &c_b18, m, &c__1, &a[p * a_dim1 + 1], lda, &ierr);
 temp1 = -aapq * work[q] / work[p];
 daxpy_(m, &temp1, &work[*n + 1], & c__1, &a[p * a_dim1 + 1], &c__1);
 dlascl_("G", &c__0, &c__0, &c_b18, &aapp, m, &c__1, &a[p * a_dim1 + 1], lda, &ierr);
 /* Computing MAX */
 d__1 = 0.; d__2 = 1. - aapq * aapq; // , expr subst  
