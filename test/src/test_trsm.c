...
// Static variables.
static char* op_str                   = "Triangular solve with multiple rhs";
static char* flash_front_str          = "FLASH_Trsm";
static char* fla_front_str            = "FLA_Trsm";
...
static fla_trsm_t*      trsm_cntl_unb;
static fla_trsm_t*      trsm_cntl_blk;
static fla_blocksize_t* trsm_cntl_bsize;

// Local prototypes.
void libfla_test_trsm_experiment( test_params_t params,
                                  unsigned int  var,
                                  char*         sc_str,
                                  ...
                                  double*       perf,
                                  double*       residual );
void libfla_test_trsm_impl( int         impl,
                            FLA_Side    side,
                            FLA_Uplo    uplo,
                            FLA_Trans   trans,
                            FLA_Diag    diag,
...
void libfla_test_trsm_cntl_create( unsigned int var,
                                   dim_t        b_alg_flat );
void libfla_test_trsm_cntl_free( void );


void libfla_test_trsm( FILE* output_stream, test_params_t params, test_op_t op )
{
	...
	if ( op.flash_front == ENABLE )
	{
		libfla_test_op_driver( flash_front_str, NULL,
		                       ...
		                       params, thresh, libfla_test_trsm_experiment );
	}
                                  ...
                                  double*       residual )
{
	...
	if ( impl == FLA_TEST_FLAT_UNB_VAR ||
		...
		libfla_test_trsm_cntl_create( var, b_alg_flat );
	...
	for ( i = 0; i < n_repeats; ++i )
	{
		...
		libfla_test_trsm_impl( impl, side, uplo, trans, diag, alpha, A_test, B_test );
                                   ...
                                   dim_t        b_alg_flat )
{
	...
	trsm_cntl_bsize = FLA_Blocksize_create( b_alg_flat,
	                                        b_alg_flat,
	                                        b_alg_flat,
	                                        b_alg_flat );

	trsm_cntl_unb   = FLA_Cntl_trsm_obj_create( FLA_FLAT,
	                                            var_unb,
	                                            NULL,
	                                            NULL,
	                                            NULL,
	...
	trsm_cntl_blk   = FLA_Cntl_trsm_obj_create( FLA_FLAT,
	                                            var_blk,
	                                            trsm_cntl_bsize,
	                                            fla_scal_cntl_blas,
	                                            fla_trsm_cntl_blas,
...
void libfla_test_trsm_cntl_free( void )
{
	FLA_Blocksize_free( trsm_cntl_bsize );

	FLA_Cntl_obj_free( trsm_cntl_unb );
	FLA_Cntl_obj_free( trsm_cntl_blk );
}
                            ...
                            FLA_Obj   B )
{
	switch ( impl )
	{
		...
		FLASH_Trsm( side, uplo, trans, diag, alpha, A, B );
		break;
		...
		FLA_Trsm_internal( side, uplo, trans, diag, alpha, A, B, trsm_cntl_unb );
		break;

		case FLA_TEST_FLAT_BLK_VAR:
		FLA_Trsm_internal( side, uplo, trans, diag, alpha, A, B, trsm_cntl_blk );
		break;
