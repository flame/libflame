/* ../netlib/dtrevc.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -11;
 ...
 dlaln2_(&c_false, &c__1, &c__1, &smin, &c_b22, &t[j + j * t_dim1], ldt, &c_b22, &c_b22, &work[j + * n], n, &wr, &c_b25, x, &c__2, &scale, &xnorm, &ierr);
 ...
 dlaln2_(&c_false, &c__2, &c__1, &smin, &c_b22, &t[j - 1 + (j - 1) * t_dim1], ldt, &c_b22, &c_b22, & work[j - 1 + *n], n, &wr, &c_b25, x, &c__2, & scale, &xnorm, &ierr);
 ...
 if (j1 == j2) {
 /* 1-by-1 diagonal block */
 dlaln2_(&c_false, &c__1, &c__2, &smin, &c_b22, &t[j + j * t_dim1], ldt, &c_b22, &c_b22, &work[j + * n], n, &wr, &wi, x, &c__2, &scale, &xnorm, & ierr);
 ...
 /* 2-by-2 diagonal block */
 dlaln2_(&c_false, &c__2, &c__2, &smin, &c_b22, &t[j - 1 + (j - 1) * t_dim1], ldt, &c_b22, &c_b22, & work[j - 1 + *n], n, &wr, &wi, x, &c__2, & scale, &xnorm, &ierr);
 ...
 ++k) {
 /* Computing MAX */
 d__3 = emax; d__4 = (d__1 = vr[k + (is - 1) * vr_dim1] , f2c_abs(d__1)) + (d__2 = vr[k + is * vr_dim1], f2c_abs(d__2)); // , expr subst  
 emax = max(d__3,d__4);
 /* L100: */
 }
 remax = 1. / emax;
 ...
 if (ki > 2) {
 i__1 = ki - 2;
 dgemv_("N", n, &i__1, &c_b22, &vr[vr_offset], ldvr, & work[*n + 1], &c__1, &work[ki - 1 + *n], &vr[( ki - 1) * vr_dim1 + 1], &c__1);
 i__1 = ki - 2;
 dgemv_("N", n, &i__1, &c_b22, &vr[vr_offset], ldvr, & work[n2 + 1], &c__1, &work[ki + n2], &vr[ki * vr_dim1 + 1], &c__1);
 }
 else {
 ...
 ++k) {
 /* Computing MAX */
 d__3 = emax; d__4 = (d__1 = vr[k + (ki - 1) * vr_dim1] , f2c_abs(d__1)) + (d__2 = vr[k + ki * vr_dim1], f2c_abs(d__2)); // , expr subst  
 emax = max(d__3,d__4);
 /* L120: */
 }
 remax = 1. / emax;
 ...
 dlaln2_(&c_true, &c__2, &c__1, &smin, &c_b22, &t[j + j * t_dim1], ldt, &c_b22, &c_b22, &work[j + * n], n, &wr, &c_b25, x, &c__2, &scale, &xnorm, &ierr);
 /* Scale if necessary */
 if (scale != 1.) {
 i__3 = *n - ki + 1;
 dscal_(&i__3, &scale, &work[ki + *n], &c__1);
 ...
 work[j + 1 + *n] = x[1];
 /* Computing MAX */
 d__3 = (d__1 = work[j + *n], f2c_abs(d__1)); d__4 = (d__2 = work[j + 1 + *n], f2c_abs(d__2)); d__3 = max( d__3,d__4); // ; expr subst  
 vmax = max(d__3,vmax);
 vcrit = bignum / vmax;
 }
 L170: ;
 ...
 d__1 = -wi;
 dlaln2_(&c_false, &c__1, &c__2, &smin, &c_b22, &t[j + j * t_dim1], ldt, &c_b22, &c_b22, &work[j + * n], n, &wr, &d__1, x, &c__2, &scale, &xnorm, & ierr);
 /* Scale if necessary */
 if (scale != 1.) {
 i__3 = *n - ki + 1;
 dscal_(&i__3, &scale, &work[ki + *n], &c__1);
 ...
 work[j + n2] = x[2];
 /* Computing MAX */
 d__3 = (d__1 = work[j + *n], f2c_abs(d__1)); d__4 = (d__2 = work[j + n2], f2c_abs(d__2)); d__3 = max(d__3, d__4); // ; expr subst  
 vmax = max(d__3,vmax);
 vcrit = bignum / vmax;
 }
 else {
 ...
 /* ([T(j+1,j) T(j+1,j+1)] ) */
 d__1 = -wi;
 dlaln2_(&c_true, &c__2, &c__2, &smin, &c_b22, &t[j + j * t_dim1], ldt, &c_b22, &c_b22, &work[j + * n], n, &wr, &d__1, x, &c__2, &scale, &xnorm, & ierr);
 /* Scale if necessary */
 if (scale != 1.) {
 i__3 = *n - ki + 1;
 dscal_(&i__3, &scale, &work[ki + *n], &c__1);
 ...
 work[j + 1 + n2] = x[3];
 /* Computing MAX */
 d__1 = f2c_abs(x[0]), d__2 = f2c_abs(x[2]), d__1 = max(d__1, d__2), d__2 = f2c_abs(x[1]), d__1 = max(d__1,d__2) ; d__2 = f2c_abs(x[3]); d__1 = max(d__1,d__2); // ; expr subst  
 vmax = max(d__1,vmax);
 vcrit = bignum / vmax;
 }
 L200: ;
 ...
 ++k) {
 /* Computing MAX */
 d__3 = emax; d__4 = (d__1 = vl[k + is * vl_dim1], f2c_abs( d__1)) + (d__2 = vl[k + (is + 1) * vl_dim1], f2c_abs(d__2)); // , expr subst  
 emax = max(d__3,d__4);
 /* L220: */
 }
 remax = 1. / emax;
 ...
 if (ki < *n - 1) {
 i__2 = *n - ki - 1;
 dgemv_("N", n, &i__2, &c_b22, &vl[(ki + 2) * vl_dim1 + 1], ldvl, &work[ki + 2 + *n], &c__1, &work[ ki + *n], &vl[ki * vl_dim1 + 1], &c__1);
 i__2 = *n - ki - 1;
 dgemv_("N", n, &i__2, &c_b22, &vl[(ki + 2) * vl_dim1 + 1], ldvl, &work[ki + 2 + n2], &c__1, &work[ ki + 1 + n2], &vl[(ki + 1) * vl_dim1 + 1], & c__1);
 }
 else {
 ...
 ++k) {
 /* Computing MAX */
 d__3 = emax; d__4 = (d__1 = vl[k + ki * vl_dim1], f2c_abs( d__1)) + (d__2 = vl[k + (ki + 1) * vl_dim1], f2c_abs(d__2)); // , expr subst  
 emax = max(d__3,d__4);
 /* L240: */
 }
 remax = 1. / emax;
