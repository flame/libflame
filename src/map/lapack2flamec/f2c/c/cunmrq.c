/* ../netlib/cunmrq.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 char buffer[256]; 
 snprintf(buffer, 256,"cunmrq inputs: side %c, trans %c, m %" FLA_IS ", n %" FLA_IS ", k %" FLA_IS ", lda %" FLA_IS ", ldc %" FLA_IS ", lwork %" FLA_IS "",*side, *trans, *m, *n, *k, *lda, *ldc, *lwork);
 ...
 *info = -12;
 ...
 /* Computing MIN */
 i__1 = 64; i__2 = ilaenv_(&c__1, "CUNMRQ", ch__1, m, n, k, &c_n1); // , expr subst  
 nb = min(i__1,i__2);
 lwkopt = nw * nb + 4160;
 }
 ...
 /* Computing MAX */
 i__1 = 2; i__2 = ilaenv_(&c__2, "CUNMRQ", ch__1, m, n, k, &c_n1); // , expr subst  
 nbmin = max(i__1,i__2);
 }
 }
 if (nb < nbmin || nb >= *k) {
