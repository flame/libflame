/* ../netlib/v3.9.0/dlasq2.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -(k + 201);
 ...
 e += z__[k + 1];
 /* Computing MAX */
 d__1 = qmax; d__2 = z__[k]; // , expr subst  
 qmax = max(d__1,d__2);
 /* Computing MIN */
 d__1 = emin; d__2 = z__[k + 1]; // , expr subst  
 emin = min(d__1,d__2);
 /* Computing MAX */
 d__1 = max(qmax,zmax); d__2 = z__[k + 1]; // , expr subst  
 zmax = max(d__1,d__2);
 /* L10: */
 }
 if (z__[(*n << 1) - 1] < 0.) {
 *info = -((*n << 1) + 199);
 ...
 d__ += z__[(*n << 1) - 1];
 /* Computing MAX */
 d__1 = qmax; d__2 = z__[(*n << 1) - 1]; // , expr subst  
 qmax = max(d__1,d__2);
 zmax = max(qmax,zmax);
 ...
 /* Computing MIN */
 d__1 = emin; d__2 = z__[i4 - (pp << 1)]; // , expr subst  
 emin = min(d__1,d__2);
 /* L60: */
 }
 z__[(n0 << 2) - pp - 2] = d__;
 ...
 i4 += 4) {
 /* Computing MAX */
 d__1 = qmax; d__2 = z__[i4]; // , expr subst  
 qmax = max(d__1,d__2);
 /* L70: */
 }
 ...
 *info = 1;
 ...
 if (qmin >= emax * 4.) {
 /* Computing MIN */
 d__1 = qmin; d__2 = z__[i4 - 3]; // , expr subst  
 qmin = min(d__1,d__2);
 /* Computing MAX */
 d__1 = emax; d__2 = z__[i4 - 5]; // , expr subst  
 emax = max(d__1,d__2);
 }
 /* Computing MAX */
 d__1 = qmax; d__2 = z__[i4 - 7] + z__[i4 - 5]; // , expr subst  
 qmax = max(d__1,d__2);
 /* Computing MIN */
 d__1 = emin; d__2 = z__[i4 - 5]; // , expr subst  
 emin = min(d__1,d__2);
 /* L90: */
 }
 i4 = 4;
 ...
 /* Computing MAX */
 d__1 = 0.; d__2 = qmin - sqrt(qmin) * 2. * sqrt(emax); // , expr subst  
 ...
 /* Computing MAX */
 d__1 = qmax; d__2 = z__[i4 + 1]; // , expr subst  
 qmax = max(d__1,d__2);
 /* Computing MIN */
 d__1 = emin; d__2 = z__[i4 - 1]; // , expr subst  
 emin = min(d__1,d__2);
 /* Computing MIN */
 d__1 = oldemn; d__2 = z__[i4]; // , expr subst  
