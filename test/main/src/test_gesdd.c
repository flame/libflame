	...
	double* residual)
{
	...
	prepare_gesdd_run("N", m, n, A_test, s, U, V, datatype, n_repeats, &time_min);
	...
	double* time_min_)
{
	...
	for (i = 0; i < n_repeats; ++i)
	{
		...
		// call to API
		//**invoke_gesdd(datatype, &blas_jobz, &m_A, &n_A, A, &cs_A, s_test, U_test, &cs_U, V_test, &cs_V, work, &lwork, rwork, iwork, &info);
		invoke_gesdd(datatype, jobz, &m_A, &n_A, A, &cs_A, s_test, NULL, &cs_U, NULL, &cs_V, work, &lwork, rwork, iwork, &info);
