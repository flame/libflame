/* ../netlib/dlanst.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 on Linux or Unix systems, link with .../path/to/libf2c.a -lm or, if you install libf2c.a in a standard place, with -lf2c -lm -- in that order, at the end of the command line, as in cc *.o -lf2c -lm Source for libf2c is in /netlib/f2c/libf2c.zip, e.g., http://www.netlib.org/f2c/libf2c.zip */
#include "FLA_f2c.h" /* Table of constant values */
static integer c__1 = 1;
/* > \brief \b DLANST returns the value of the 1-norm, or the Frobenius norm, or the infinity norm, or the ele ment of largest absolute value of a real symmetric tridiagonal matrix. */
/* =========== DOCUMENTATION =========== */
...
/* > \verbatim */
/* > */
/* > DLANST = ( max(f2c_abs(A(i,j))), NORM = 'M' or 'm' */
/* > ( */
...
doublereal dlanst_(char *norm, integer *n, doublereal *d__, doublereal *e)
{
    ...
    else if (lsame_(norm, "O") || *(unsigned char *) norm == '1' || lsame_(norm, "I"))
    {
        ...
        else
        {
            ...
            sum = (d__1 = e[*n - 1], f2c_abs(d__1)) + (d__2 = d__[*n], f2c_abs(d__2));
            if (anorm < sum || disnan_(&sum))
            {
                anorm = sum;
            }
            ...
            for (i__ = 2;
                    ...
                    ++i__)
            {
                sum = (d__1 = d__[i__], f2c_abs(d__1)) + (d__2 = e[i__], f2c_abs(d__2) ) + (d__3 = e[i__ - 1], f2c_abs(d__3));
                if (anorm < sum || disnan_(&sum))
                {
                    anorm = sum;
                }
