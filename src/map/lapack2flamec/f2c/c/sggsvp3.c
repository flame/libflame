/* ../netlib/v3.9.0/sggsvp3.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 /* ===================================================================== */
 /* Subroutine */
 int sggsvp3_(char *jobu, char *jobv, char *jobq, integer *m, integer *p, integer *n, real *a, integer *lda, real *b, integer *ldb, real *tola, real *tolb, integer *k, integer *l, real *u, integer *ldu, real *v, integer *ldv, real *q, integer *ldq, integer *iwork, real * tau, real *work, integer *lwork, integer *info) {
 ...
 *info = -24;
 ...
 /* Compute workspace */
 if (*info == 0) {
 sgeqp3_(p, n, &b[b_offset], ldb, &iwork[1], &tau[1], &work[1], &c_n1, info);
 ...
 sgeqp3_(m, n, &a[a_offset], lda, &iwork[1], &tau[1], &work[1], &c_n1, info);
 /* Computing MAX */
 ...
 sgeqp3_(p, n, &b[b_offset], ldb, &iwork[1], &tau[1], &work[1], lwork, info);
 /* Update A := A*P */
 ...
 *l = 0;
 ...
 i__1 = *n - *l;
 sgeqp3_(m, &i__1, &a[a_offset], lda, &iwork[1], &tau[1], &work[1], lwork, info);
 ...
 *k = 0;
 ...
 i__1 = *m - *k;
 i__2 = *n - *l;
 slaset_("Full", &i__1, &i__2, &c_b14, &c_b14, &a[*k + 1 + a_dim1], lda);
 }
 if (*n - *l > *k) {
