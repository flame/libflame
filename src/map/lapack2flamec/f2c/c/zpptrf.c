...
int zpptrf_(char *uplo, integer *n, doublecomplex *ap, integer *info)
{
    ...
    char buffer[256]; 
    snprintf(buffer, 256,"zpptrf inputs: uplo %c, n %d",*uplo, *n);
    ...
    logical upper;
    extern /* Subroutine */
    int ztpsv_(char *, char *, char *, integer *, doublecomplex *, doublecomplex *, integer *), xerbla_(char *, integer *), zdscal_(integer *, doublereal *, doublecomplex *, integer *);
    ...
    else
    {
                ...
                ++j)
        {
            ...
            if (j < *n)
            {
                ...
                i__2 = *n - j;
                zhpr_("Lower", &i__2, &c_b16, &ap[jj + 1], &c__1, &ap[jj + *n - j + 1]);
                jj = jj + *n - j + 1;
            }
            /* L20: */
        }
