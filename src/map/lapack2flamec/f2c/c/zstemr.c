/* ../netlib/v3.9.0/zstemr.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -19;
 ...
 if (zquery && *info == 0) {
 i__1 = z_dim1 + 1;
 z__[i__1].r = (doublereal) nzcmin; z__[i__1].i = 0.; // , expr subst  
 }
 ...
 *m = 1;
 ...
 i__1 = *m * z_dim1 + 1;
 d__1 = -sn;
 z__[i__1].r = d__1; z__[i__1].i = 0.; // , expr subst  
 i__1 = *m * z_dim1 + 2;
 z__[i__1].r = cs; z__[i__1].i = 0.; // , expr subst  
 ...
 z__[i__1].r = cs; z__[i__1].i = 0.; // , expr subst  
 i__1 = *m * z_dim1 + 2;
 z__[i__1].r = sn; z__[i__1].i = 0.; // , expr subst  
 /* Note: At most one of SN and CS can be zero. */
 if (sn != 0.) {
 if (cs != 0.) {
 isuppz[(*m << 1) - 1] = 1;
 ...
 *tryrac = FALSE_;
 ...
 rtol1 = sqrt(eps);
 /* Computing MAX */
 d__1 = sqrt(eps) * .005; d__2 = eps * 4.; // , expr subst  
 rtol2 = max(d__1,d__2);
 }
