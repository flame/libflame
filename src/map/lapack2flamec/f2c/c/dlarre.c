/* ../netlib/dlarre.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 on Linux or Unix systems, link with .../path/to/libf2c.a -lm or, if you install libf2c.a in a standard place, with -lf2c -lm -- in that order, at the end of the command line, as in cc *.o -lf2c -lm Source for libf2c is in /netlib/f2c/libf2c.zip, e.g., http://www.netlib.org/f2c/libf2c.zip */
 #include "FLA_f2c.h" /* Table of constant values */
 static integer c__1 = 1;
 static integer c__2 = 2;
 /* > \brief \b DLARRE given the tridiagonal matrix T, sets small off-diagonal elements to zero and for each un reduced block Ti, finds base representations and eigenvalues. */
 /* =========== DOCUMENTATION =========== */
 ...
 *m = 1;
 ...
 /* Computing MIN */
 d__1 = gl; d__2 = gers[(i__ << 1) - 1]; // , expr subst  
 gl = min(d__1,d__2);
 gers[i__ * 2] = d__[i__] + tmp1;
 /* Computing MAX */
 d__1 = gu; d__2 = gers[i__ * 2]; // , expr subst  
 gu = max(d__1,d__2);
 eold = eabs;
 /* L5: */
 }
 ...
 /* Computing 2nd power */
 d__3 = emax;
 d__1 = 1.; d__2 = d__3 * d__3; // , expr subst  
 ...
 *info = -1;
 ...
 ++i__) {
 /* Computing MAX */
 d__1 = 0.; d__2 = w[i__ + 1] - werr[i__ + 1] - (w[i__] + werr[i__]); // , expr subst  
 wgap[i__] = max(d__1,d__2);
 /* L30: */
 }
 /* Computing MAX */
 d__1 = 0.; d__2 = *vu - sigma - (w[wend] + werr[wend]); // , expr subst  
 ...
 *info = -1;
 ...
 /* Computing MAX */
 d__2 = gl; d__3 = tmp - tmp1 - eps * 100. * (d__1 = tmp - tmp1, f2c_abs(d__1)); // , expr subst  
 ...
 *info = -1;
 ...
 /* Computing MIN */
 d__2 = gu; d__3 = tmp + tmp1 + eps * 100. * (d__1 = tmp + tmp1, f2c_abs(d__1)); // , expr subst  
 ...
 /* Computing MAX */
 d__2 = gl; d__3 = w[wbegin] - werr[wbegin] - eps * 100. * (d__1 = w[wbegin] - werr[wbegin], f2c_abs(d__1)); // , expr subst  
 isleft = max(d__2,d__3);
 /* Computing MIN */
 d__2 = gu; d__3 = w[wend] + werr[wend] + eps * 100. * (d__1 = w[ wend] + werr[wend], f2c_abs(d__1)); // , expr subst  
 isrght = min(d__2,d__3);
 }
 ...
 /* Computing MAX */
 d__1 = tau; d__2 = eps * 2. * f2c_abs(sigma); // , expr subst  
 tau = max(d__1,d__2);
 }
 else {
 if (mb > 1) {
 clwdth = w[wend] + werr[wend] - w[wbegin] - werr[wbegin];
 avgap = (d__1 = clwdth / (doublereal) (wend - wbegin), f2c_abs( d__1));
 if (sgndef == 1.) {
 /* Computing MAX */
 d__1 = wgap[wbegin];
 tau = max(d__1,avgap) * .5;
 /* Computing MAX */
 d__1 = tau; d__2 = werr[wbegin]; // , expr subst  
 tau = max(d__1,d__2);
 }
 else {
 /* Computing MAX */
 ...
 tau = max(d__1,avgap) * .5;
 /* Computing MAX */
 d__1 = tau; d__2 = werr[wend]; // , expr subst  
 tau = max(d__1,d__2);
 }
 }
 else {
 ...
 work[i__ + 1] = dpivot;
 /* Computing MAX */
 d__1 = dmax__; d__2 = f2c_abs(dpivot); // , expr subst  
 ...
 *info = 2;
 ...
 ++j) {
 w[j] -= sigma;
 werr[j] += (d__1 = w[j], f2c_abs(d__1)) * eps;
 /* L134: */
 }
 ...
 *info = -4;
 ...
 i__ <= i__2;
 ++i__) {
 work[(i__ << 1) - 1] = (d__1 = d__[j], f2c_abs(d__1));
 ...
 *info = -6;
 ...
 werr[i__] = rtol * (d__1 = w[i__], f2c_abs(d__1));
 /* L165: */
 }
 i__2 = *m - 1;
 for (i__ = *m - mb + 1;
 ...
 /* Computing MAX */
 d__1 = 0.; d__2 = *vu - sigma - (w[*m] + werr[*m]); // , expr subst  
