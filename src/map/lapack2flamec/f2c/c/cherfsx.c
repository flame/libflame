...
/* ../netlib/v3.9.0/cherfsx.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
,inf) * norm(Z,inf)) for some */
 ...
 extern real cla_hercond_c_(char *, integer *, complex *, integer *, complex *, integer *, integer *, real *, logical *, integer *, complex *, real *);
 real anorm;
 logical rcequ;
 ...
 *(unsigned char *)norm = 'I';
 ...
 if (rcequ) {
 rcond_tmp__ = cla_hercond_c_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &ipiv[1], &s[1], &c_true, info, &work[1] , &rwork[1]);
 }
 else {
 rcond_tmp__ = cla_hercond_c_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &ipiv[1], &s[1], &c_false, info, &work[ 1], &rwork[1]);
 }
 i__1 = *nrhs;
 for (j = 1;
 j <= i__1;
 ...
 *info = *n + j;
 ...
 rcond_tmp__ = cla_hercond_x_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &ipiv[1], &x[j * x_dim1 + 1], info, &work[1], &rwork[1]);
 }
 else {
 rcond_tmp__ = 0.f;
 }
