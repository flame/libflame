/* ../netlib/chetrs_rook.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
/* > \brief \b CHETRS_ROOK computes the solution to a system of linear equations A * X = B for HE matrices us ing factorization obtained with one of the bounded diagonal pivoting methods (max 2 interchanges) */
/* =========== DOCUMENTATION =========== */
/* Online html documentation available at */
/* http://www.netlib.org/lapack/explore-html/ */
/* > \htmlonly */
/* > Download CHETRS_ROOK + dependencies */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/chetrs_ rook.f"> */
/* > [TGZ]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/chetrs_ rook.f"> */
/* > [ZIP]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/chetrs_ rook.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
/* =========== */
...
/* ===================================================================== */
/* Subroutine */
int chetrs_rook_(char *uplo, integer *n, integer *nrhs, complex *a, integer *lda, integer *ipiv, complex *b, integer *ldb, integer *info)
{
    AOCL_DTL_TRACE_ENTRY(AOCL_DTL_LEVEL_TRACE_5);
#if LF_AOCL_DTL_LOG_ENABLE
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,"chetrs_rook inputs: uplo %c, n %lld, nrhs %lld, lda %lld, ldb %lld",*uplo, *n, *nrhs, *lda, *ldb);
#else
    snprintf(buffer, 256,"chetrs_rook inputs: uplo %c, n %d, nrhs %d, lda %d, ldb %d",*uplo, *n, *nrhs, *lda, *ldb);
#endif
...
L90: /* If K < 1, exit from loop. */
        ...
        if (ipiv[k] > 0)
        {
            ...
            if (k < *n)
            {
                ...
                q__1.r = -1.f;
                q__1.i = -0.f; // , expr subst
                cgemv_("Conjugate transpose", &i__1, nrhs, &q__1, &b[k + 1 + b_dim1], ldb, &a[k + 1 + k * a_dim1], &c__1, &c_b1, & b[k + b_dim1], ldb);
                clacgv_(nrhs, &b[k + b_dim1], ldb);
            }
            /* Interchange rows K and IPIV(K). */
            kp = ipiv[k];
        ...
        else
        {
            ...
            if (k < *n)
            {
                ...
                q__1.r = -1.f;
                q__1.i = -0.f; // , expr subst
                cgemv_("Conjugate transpose", &i__1, nrhs, &q__1, &b[k + 1 + b_dim1], ldb, &a[k + 1 + (k - 1) * a_dim1], &c__1, & c_b1, &b[k - 1 + b_dim1], ldb);
                clacgv_(nrhs, &b[k - 1 + b_dim1], ldb);
            }
            /* Interchange rows K and -IPIV(K), then K-1 and -IPIV(K-1) */
            kp = -ipiv[k];
