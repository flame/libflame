/* ../netlib/v3.9.0/cgehrd.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -8;
 ...
 /* Computing MIN */
 i__1 = 64; i__2 = ilaenv_(&c__1, "CGEHRD", " ", n, ilo, ihi, &c_n1); // , expr subst  
 nb = min(i__1,i__2);
 lwkopt = *n * nb + 4160;
 ...
 ++i__) {
 i__2 = i__;
 tau[i__2].r = 0.f; tau[i__2].i = 0.f; // , expr subst  
 /* L10: */
 }
 i__1 = *n - 1;
 for (i__ = max(1,*ihi);
 ...
 /* Computing MAX */
 i__1 = nb; i__2 = ilaenv_(&c__3, "CGEHRD", " ", n, ilo, ihi, &c_n1); // , expr subst  
 nx = max(i__1,i__2);
 if (nx < nh) {
 ...
 /* unblocked code */
 /* Computing MAX */
 i__1 = 2; i__2 = ilaenv_(&c__2, "CGEHRD", " ", n, ilo, ihi, & c_n1); // , expr subst  
 nbmin = max(i__1,i__2);
 ...
 i__3 = i__ + ib + (i__ + ib - 1) * a_dim1;
 a[i__3].r = 1.f; a[i__3].i = 0.f; // , expr subst  
 i__3 = *ihi - i__ - ib + 1;
