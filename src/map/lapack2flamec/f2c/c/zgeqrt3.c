/* ../netlib/v3.9.0/zgeqrt3.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -6;
 ...
 if (*info != 0) {
 i__1 = -(*info);
 xerbla_("ZGEQRT3", &i__1);
 return 0;
 }
 if (*n == 1) {
 /* Compute Householder transform when N=1 */
 zlarfg_(m, &a[a_dim1 + 1], &a[min(2,*m) + a_dim1], &c__1, &t[t_dim1 + 1]);
 }
 else {
