/* ../netlib/v3.9.0/chetrf_rk.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/chetrf_ rk.f"> */
 /* > [TGZ]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/chetrf_ rk.f"> */
 /* > [ZIP]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/chetrf_ rk.f"> */
 /* > [TXT]</a> */
 /* > \endhtmlonly */
 /* Definition: */
 /* =========== */
 ...
 */
 ...
 char buffer[256]; 
#if FLA_ENABLE_ILP64 
 snprintf(buffer, 256,"chetrf_rk inputs: uplo %c, n %lld, lda %lld, lwork %lld",*uplo, *n, *lda, *lwork);
#else 
 snprintf(buffer, 256,"chetrf_rk inputs: uplo %c, n %d, lda %d, lwork %d",*uplo, *n, *lda, *lwork);
#endif
 ...
 *info = -8;
 ...
 nb = max(i__1,1);
 /* Computing MAX */
 i__1 = 2; i__2 = ilaenv_(&c__2, "CHETRF_RK", uplo, n, &c_n1, & c_n1, &c_n1); // , expr subst  
 nbmin = max(i__1,i__2);
 }
 }
 else {
 ...
 */
 ...
 clahef_rk_(uplo, &k, &nb, &kb, &a[a_offset], lda, &e[1], &ipiv[1] , &work[1], &ldwork, &iinfo);
 }
 else {
 ...
 */
 ...
 i__1 = *n - k + 1;
 clahef_rk_(uplo, &i__1, &nb, &kb, &a[k + k * a_dim1], lda, &e[k], &ipiv[k], &work[1], &ldwork, &iinfo);
 }
 else {
 /* Use unblocked code to factorize columns k:n of A */
 i__1 = *n - k + 1;
 chetf2_rk_(uplo, &i__1, &a[k + k * a_dim1], lda, &e[k], &ipiv[k], &iinfo);
 kb = *n - k + 1;
 }
 /* Set INFO on the first occurrence of a zero pivot */
 if (*info == 0 && iinfo > 0) {
