/* ../netlib/zunbdb3.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int zunbdb3_(integer *m, integer *p, integer *q, doublecomplex *x11, integer *ldx11, doublecomplex *x21, integer * ldx21, doublereal *theta, doublereal *phi, doublecomplex *taup1, doublecomplex *taup2, doublecomplex *tauq1, doublecomplex *work, integer *lwork, integer *info)
{
    ...
    AOCL_DTL_SNPRINTF("zunbdb3 inputs: m %" FLA_IS ", p %" FLA_IS ", q %" FLA_IS ", ldx11 %" FLA_IS ", ldx21 %" FLA_IS ", lwork %" FLA_IS "", *m, *p, *q, *ldx11, *ldx21, *lwork);
    ...
    extern /* Subroutine */
    int zlarf_(char *, integer *, integer *, doublecomplex *, integer *, doublecomplex *, doublecomplex *, integer *, doublecomplex *), zdrot_(integer *, doublecomplex *, integer *, doublecomplex *, integer *, doublereal *, doublereal *);
    ...
    *info = 0;
            ...
            ++i__)
    {
        ...
        {
            i__2 = *q - i__ + 1;
            zdrot_(&i__2, &x11[i__ - 1 + i__ * x11_dim1], ldx11, &x21[i__ + i__ * x21_dim1], ldx11, &c__, &s);
        }
        i__2 = *q - i__ + 1;
        ...
        i__2 = *p - i__ + 1;
        i__3 = *q - i__ + 1;
        zlarf_("R", &i__2, &i__3, &x21[i__ + i__ * x21_dim1], ldx21, &tauq1[ i__], &x11[i__ + i__ * x11_dim1], ldx11, &work[ilarf]);
        i__2 = *m - *p - i__;
        i__3 = *q - i__ + 1;
        ...
        i__3 = *m - *p - i__;
        i__4 = *q - i__;
        zunbdb5_(&i__2, &i__3, &i__4, &x11[i__ + i__ * x11_dim1], &c__1, &x21[ i__ + 1 + i__ * x21_dim1], &c__1, &x11[i__ + (i__ + 1) * x11_dim1], ldx11, &x21[i__ + 1 + (i__ + 1) * x21_dim1], ldx21, &work[iorbdb5], &lorbdb5, &childinfo);
        i__2 = *p - i__ + 1;
        zlarfgp_(&i__2, &x11[i__ + i__ * x11_dim1], &x11[i__ + 1 + i__ * x11_dim1], &c__1, &taup1[i__]);
        if (i__ < *m - *p)
        {
            i__2 = *m - *p - i__;
            zlarfgp_(&i__2, &x21[i__ + 1 + i__ * x21_dim1], &x21[i__ + 2 + i__ * x21_dim1], &c__1, &taup2[i__]);
            phi[i__] = atan2((doublereal) x21[i__ + 1 + i__ * x21_dim1].r, ( doublereal) x11[i__ + i__ * x11_dim1].r);
            c__ = cos(phi[i__]);
            s = sin(phi[i__]);
            ...
            zlarf_("L", &i__2, &i__3, &x21[i__ + 1 + i__ * x21_dim1], &c__1, & z__1, &x21[i__ + 1 + (i__ + 1) * x21_dim1], ldx21, &work[ ilarf]);
        }
