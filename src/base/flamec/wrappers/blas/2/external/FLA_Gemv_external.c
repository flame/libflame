...
FLA_Error FLA_Gemv_external_ts( FLA_cntl_init_s *FLA_cntl_init_i, FLA_Trans transa, FLA_Obj alpha, FLA_Obj A, FLA_Obj x, FLA_Obj beta, FLA_Obj y )
{
  ...
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING ) 
    FLA_Gemv_check_ts( FLA_cntl_init_i, transa, alpha, A, x, beta, y );

  if ( FLA_Obj_has_zero_dim( A ) )
  {
    FLA_Scal_external_ts( FLA_cntl_init_i, beta, y );
    return FLA_SUCCESS;
  }
