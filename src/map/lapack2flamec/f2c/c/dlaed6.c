...
int dlaed6_(integer *kniter, logical *orgati, doublereal * rho, doublereal *d__, doublereal *z__, doublereal *finit, doublereal * tau, integer *info)
{
    ...
    *tau = 0.;
    if (*kniter == 2)
    {
        ...
        /* Computing MAX */
        d__1 = f2c_dabs(a), d__2 = f2c_dabs(b);
        d__1 = max(d__1,d__2);
        ...
        else
        {
            ...
            if (f2c_dabs(*finit) <= f2c_dabs(temp))
            {
                *tau = 0.;
            }
        }
    ...
    {
        /* Computing MIN */
        d__3 = (d__1 = d__[2] - *tau, f2c_dabs(d__1));
        d__4 = (d__2 = d__[3] - * tau, f2c_dabs(d__2)); // , expr subst
        temp = min(d__3,d__4);
    }
    else
    {
        /* Computing MIN */
        d__3 = (d__1 = d__[1] - *tau, f2c_dabs(d__1));
        d__4 = (d__2 = d__[2] - * tau, f2c_dabs(d__2)); // , expr subst
        temp = min(d__3,d__4);
    }
    scale = FALSE_;
    if (temp <= small1)
    ...
    */
            ...
            ++niter)
    {
        ...
        *tau += eta;
        ...
        f = *finit + *tau * fc;
        erretm = (f2c_dabs(*finit) + f2c_dabs(*tau) * erretm) * 8. + f2c_dabs(*tau) * df;
        if (f2c_dabs(f) <= eps * erretm)
        {
            goto L60;
        }
        if (f <= 0.)
