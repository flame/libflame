/* dsytrf_aa.f -- translated by f2c (version 20190311). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int dsytrf_aa_(char *uplo, integer *n, doublereal *a, integer *lda, integer *ipiv, doublereal *work, integer *lwork, integer *info)
{
    ...
    AOCL_DTL_SNPRINTF("dsytrf_aa inputs: uplo %c, n %" FLA_IS ", lda %" FLA_IS ", lwork %" FLA_IS "",*uplo, *n, *lda, *lwork);
...
L10:
        ...
        i__1 = 2 - k1;
        i__2 = *n - j;
        dlasyf_aa_(uplo, &i__1, &i__2, &jb, &a[fla_max(1,j) + (j + 1) * a_dim1], lda, &ipiv[j + 1], &work[1], n, &work[*n * nb + 1]) ;
...
L11:
        ...
        i__2 = 2 - k1;
        i__1 = *n - j;
        dlasyf_aa_(uplo, &i__2, &i__1, &jb, &a[j + 1 + fla_max(1,j) * a_dim1], lda, &ipiv[j + 1], &work[1], n, &work[*n * nb + 1]) ;
        /* Adjust IPIV and apply it back (J-th step picks (J+1)-th pivot) */
        /* Computing MIN */
        i__1 = *n;
        i__3 = j + jb + 1; // , expr subst
