/* ../netlib/v3.9.0/sgehrd.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -8;
 ...
 /* Computing MIN */
 i__1 = 64; i__2 = ilaenv_(&c__1, "SGEHRD", " ", n, ilo, ihi, &c_n1); // , expr subst  
 nb = min(i__1,i__2);
 lwkopt = *n * nb + 4160;
 work[1] = (real) lwkopt;
 }
 ...
 /* Computing MAX */
 i__1 = nb; i__2 = ilaenv_(&c__3, "SGEHRD", " ", n, ilo, ihi, &c_n1); // , expr subst  
 nx = max(i__1,i__2);
 if (nx < nh) {
 ...
 /* unblocked code */
 /* Computing MAX */
 i__1 = 2; i__2 = ilaenv_(&c__2, "SGEHRD", " ", n, ilo, ihi, & c_n1); // , expr subst  
 nbmin = max(i__1,i__2);
 ...
 slahr2_(ihi, &i__, &ib, &a[i__ * a_dim1 + 1], lda, &tau[i__], & work[iwt], &c__65, &work[1], &ldwork);
 ...
 i__3 = *ihi - i__ - ib + 1;
 sgemm_("No transpose", "Transpose", ihi, &i__3, &ib, &c_b25, & work[1], &ldwork, &a[i__ + ib + i__ * a_dim1], lda, & c_b26, &a[(i__ + ib) * a_dim1 + 1], lda);
 a[i__ + ib + (i__ + ib - 1) * a_dim1] = ei;
 /* Apply the block reflector H to A(1:i,i+1:i+ib-1) from the */
 /* right */
 i__3 = ib - 1;
 strmm_("Right", "Lower", "Transpose", "Unit", &i__, &i__3, &c_b26, &a[i__ + 1 + i__ * a_dim1], lda, &work[1], &ldwork);
 i__3 = ib - 2;
 for (j = 0;
 j <= i__3;
 ++j) {
 saxpy_(&i__, &c_b25, &work[ldwork * j + 1], &c__1, &a[(i__ + j + 1) * a_dim1 + 1], &c__1);
 /* L30: */
 }
 ...
 i__3 = *ihi - i__;
 i__4 = *n - i__ - ib + 1;
 slarfb_("Left", "Transpose", "Forward", "Columnwise", &i__3, & i__4, &ib, &a[i__ + 1 + i__ * a_dim1], lda, &work[iwt], & c__65, &a[i__ + 1 + (i__ + ib) * a_dim1], lda, &work[1], & ldwork);
 /* L40: */
 }
 }
