/* ../netlib/slansb.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
        ...
        if (*k > 0)
        {
            if (lsame_(uplo, "U"))
            {
                        ...
                        ++j)
                {
                    ...
                    /* Computing MAX */
                    i__2 = *k + 2 - j;
                    slassq_(&i__4, &ab[max(i__2,1) + j * ab_dim1], &c__1, & scale, &sum);
                    /* L110: */
                }
                l = *k + 1;
            }
