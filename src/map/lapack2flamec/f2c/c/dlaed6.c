 ...
 */
 ...
 char buffer[256]; 
 snprintf(buffer, 256,"dlaed6 inputs: kniter %" FLA_IS "",*kniter);
 ...
 *tau = 0.;
 ...
 /* Computing MAX */
 d__1 = f2c_abs(a), d__2 = f2c_abs(b); d__1 = max(d__1,d__2); d__2 = f2c_abs(c__); // ; expr subst  
 temp = max(d__1,d__2);
 a /= temp;
 b /= temp;
 c__ /= temp;
 ...
 *tau = 0.;
 ...
 if (*orgati) {
 /* Computing MIN */
 d__3 = (d__1 = d__[2] - *tau, f2c_abs(d__1)); d__4 = (d__2 = d__[3] - * tau, f2c_abs(d__2)); // , expr subst  
 temp = min(d__3,d__4);
 }
 else {
 /* Computing MIN */
 d__3 = (d__1 = d__[1] - *tau, f2c_abs(d__1)); d__4 = (d__2 = d__[2] - * tau, f2c_abs(d__2)); // , expr subst  
 temp = min(d__3,d__4);
 }
 scale = FALSE_;
 if (temp <= small1) {
