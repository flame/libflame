/* ../netlib/v3.9.0/slasyf_aa.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/slasyf_ aa.f"> */
/* > [TGZ]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/slasyf_ aa.f"> */
/* > [ZIP]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/slasyf_ aa.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
/* =========== */
...
int slasyf_aa_(char *uplo, integer *j1, integer *m, integer *nb, real *a, integer *lda, integer *ipiv, real *h__, integer *ldh, real *work)
{
    ...
    char buffer[256];
    snprintf(buffer, 256,"slasyf_aa inputs: uplo %c, j1 %" FLA_IS ", m %" FLA_IS ", nb %" FLA_IS ", lda %" FLA_IS ", ldh %" FLA_IS "",*uplo, *j1, *m, *nb, *lda, *ldh);
...
L10:
        ...
        if (k > 2)
        {
            ...
            /* first column */
            i__1 = j - k1;
            sgemv_("No transpose", &mj, &i__1, &c_b6, &h__[j + k1 * h_dim1], ldh, &a[j * a_dim1 + 1], &c__1, &c_b8, &h__[j + j * h_dim1], &c__1);
        }
        ...
        if (j < *m)
        {
            ...
            if (i2 != 2 && piv != 0.f)
            {
                ...
                i2 = i2 + j - 1;
                i__1 = i2 - i1 - 1;
                sswap_(&i__1, &a[*j1 + i1 - 1 + (i1 + 1) * a_dim1], lda, &a[* j1 + i1 + i2 * a_dim1], &c__1);
                /* Swap A(I1, I2+1:M) with A(I2, I2+1:M) */
                if (i2 < *m)
                {
                    i__1 = *m - i2;
                    sswap_(&i__1, &a[*j1 + i1 - 1 + (i2 + 1) * a_dim1], lda, & a[*j1 + i2 - 1 + (i2 + 1) * a_dim1], lda);
                }
            ...
            if (j < *nb)
            {
                ...
                i__1 = *m - j;
                scopy_(&i__1, &a[k + 1 + (j + 1) * a_dim1], lda, &h__[j + 1 + (j + 1) * h_dim1], &c__1);
            }
...
L30:
        ...
        if (k > 2)
        {
            ...
            /* first column */
            i__1 = j - k1;
            sgemv_("No transpose", &mj, &i__1, &c_b6, &h__[j + k1 * h_dim1], ldh, &a[j + a_dim1], lda, &c_b8, &h__[j + j * h_dim1], & c__1);
        }
        ...
        if (j < *m)
        {
            ...
            if (i2 != 2 && piv != 0.f)
            {
                ...
                i2 = i2 + j - 1;
                i__1 = i2 - i1 - 1;
                sswap_(&i__1, &a[i1 + 1 + (*j1 + i1 - 1) * a_dim1], &c__1, &a[ i2 + (*j1 + i1) * a_dim1], lda);
                /* Swap A(I2+1:M, I1) with A(I2+1:M, I2) */
                if (i2 < *m)
                {
                    i__1 = *m - i2;
                    sswap_(&i__1, &a[i2 + 1 + (*j1 + i1 - 1) * a_dim1], &c__1, &a[i2 + 1 + (*j1 + i2 - 1) * a_dim1], &c__1);
                }
                /* Swap A(I1, I1) with A(I2, I2) */
            ...
            if (j < *nb)
            {
                ...
                i__1 = *m - j;
                scopy_(&i__1, &a[j + 1 + (k + 1) * a_dim1], &c__1, &h__[j + 1 + (j + 1) * h_dim1], &c__1);
            }
            /* Compute L(J+2, J+1) = WORK( 3:M ) / T(J, J+1), */
            /* where A(J, J+1) = T(J, J+1) and A(J+2:M, J) = L(J+2:M, J+1) */
            if (j < *m - 1)
