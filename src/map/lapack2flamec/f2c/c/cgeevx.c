/* ../netlib/cgeevx.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
...
#else 
 snprintf(buffer, 256,"cgeevx inputs: balanc %c, jobvl %c, jobvr %c, sense %c, n %d, lda %d, ldvl %d, ldvr %d, lwork %d",*balanc, *jobvl, *jobvr, *sense, *n, *lda, *ldvl, *ldvr, *lwork);
#endif
 ...
 extern /* Subroutine */
 int chseqr_(char *, char *, integer *, integer *, integer *, complex *, integer *, complex *, complex *, integer *, complex *, integer *, integer *), cunghr_(integer *, integer *, integer *, complex *, integer *, complex *, complex *, integer *, integer *), ctrsna_(char *, char *, logical *, integer *, complex *, integer *, complex *, integer *, complex *, integer *, real *, real *, integer *, integer *, complex *, integer *, real *, integer *);
 ...
 extern /* Subroutine */
 int ctrevc3_(char *, char *, logical *, integer *, complex *, integer *, complex *, integer *, complex *, integer *, integer *, integer *, complex *, integer *, real *, integer *, integer *);
 ...
 *info = -12;
 ...
 chseqr_("S", "N", n, &c__1, n, &a[a_offset], lda, &w[1], & vr[vr_offset], ldvr, &work[1], &c_n1, info);
 }
 }
 ...
 if (! (wntsnn || wntsne)) {
 /* Computing MAX */
 i__1 = minwrk; i__2 = *n * *n + (*n << 1); // , expr subst  
 minwrk = max(i__1,i__2);
 }
 maxwrk = max(maxwrk,hswork);
 if (! (wntsnn || wntsne)) {
 /* Computing MAX */
 i__1 = maxwrk; i__2 = *n * *n + (*n << 1); // , expr subst  
 ...
 maxwrk = max(maxwrk,hswork);
 /* Computing MAX */
 i__1 = maxwrk; i__2 = *n + (*n - 1) * ilaenv_(&c__1, "CUNGHR", " ", n, &c__1, n, &c_n1); // , expr subst  
 maxwrk = max(i__1,i__2);
 if (! (wntsnn || wntsne)) {
 /* Computing MAX */
 i__1 = maxwrk; i__2 = *n * *n + (*n << 1); // , expr subst  
 ...
 /* Computing MAX */
 i__1 = maxwrk; i__2 = *n << 1; // , expr subst  
 maxwrk = max(i__1,i__2);
 }
 ...
 *(unsigned char *)job = 'S';
 ...
 iwrk = itau;
 i__1 = *lwork - iwrk + 1;
 chseqr_(job, "N", n, ilo, ihi, &a[a_offset], lda, &w[1], &vr[ vr_offset], ldvr, &work[iwrk], &i__1, info);
 }
 ...
 /* (RWorkspace: need N) */
 i__1 = *lwork - iwrk + 1;
 ctrevc3_(side, "B", select, n, &a[a_offset], lda, &vl[vl_offset], ldvl, &vr[vr_offset], ldvr, n, &nout, &work[iwrk], &i__1, & rwork[1], n, &ierr);
 }
 /* Compute condition numbers if desired */
 /* (CWorkspace: need N*N+2*N unless SENSE = 'E') */
 /* (RWorkspace: need 2*N unless SENSE = 'E') */
 if (! wntsnn) {
 ctrsna_(sense, "A", select, n, &a[a_offset], lda, &vl[vl_offset], ldvl, &vr[vr_offset], ldvr, &rconde[1], &rcondv[1], n, &nout, &work[iwrk], n, &rwork[1], &icond);
 }
 if (wantvl) {
 /* Undo balancing of left eigenvectors */
 cgebak_(balanc, "L", n, ilo, ihi, &scale[1], n, &vl[vl_offset], ldvl, &ierr);
 ...
 tmp.r = q__1.r; tmp.i = q__1.i; // , expr subst  
 cscal_(n, &tmp, &vl[i__ * vl_dim1 + 1], &c__1);
 i__2 = k + i__ * vl_dim1;
 i__3 = k + i__ * vl_dim1;
 r__1 = vl[i__3].r;
 q__1.r = r__1; q__1.i = 0.f; // , expr subst  
 vl[i__2].r = q__1.r; vl[i__2].i = q__1.i; // , expr subst  
 /* L20: */
 }
 }
 if (wantvr) {
 /* Undo balancing of right eigenvectors */
 cgebak_(balanc, "R", n, ilo, ihi, &scale[1], n, &vr[vr_offset], ldvr, &ierr);
 ...
 r__1 = vr[i__3].r;
 q__1.r = r__1; q__1.i = 0.f; // , expr subst  
 vr[i__2].r = q__1.r; vr[i__2].i = q__1.i; // , expr subst  
 /* L40: */
 }
 }
