/* ../netlib/chetri2.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 /* ===================================================================== */
 /* Subroutine */
 int chetri2_(char *uplo, integer *n, complex *a, integer * lda, integer *ipiv, complex *work, integer *lwork, integer *info) {
 AOCL_DTL_TRACE_ENTRY(AOCL_DTL_LEVEL_TRACE_5);
#if AOCL_DTL_LOG_ENABLE 
 char buffer[256]; 
#if FLA_ENABLE_ILP64 
 snprintf(buffer, 256,"chetri2 inputs: uplo %c, n %lld, lda %lld, ipiv %lld, lwork %lld",*uplo, *n, *lda, *ipiv, *lwork);
#else 
 snprintf(buffer, 256,"chetri2 inputs: uplo %c, n %d, lda %d, ipiv %d, lwork %d",*uplo, *n, *lda, *ipiv, *lwork);
#endif
 ...
 /* Local variables */
 extern /* Subroutine */
 int chetri2x_(char *, integer *, complex *, integer *, integer *, complex *, integer *, integer *);
 ...
 *info = -7;
 ...
 work[1].r = (real) minsize; work[1].i = 0.f; // , expr subst  
 AOCL_DTL_TRACE_EXIT(AOCL_DTL_LEVEL_TRACE_5);
 return 0;
 }
 if (*n == 0) {
