/* ../netlib/v3.9.0/zgeqp3.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -8;
 ...
 /* Computing MAX */
 i__1 = iws; i__2 = (integer) work[1].r; // , expr subst  
 iws = max(i__1,i__2);
 if (na < *n) {
 ...
 /* Computing MAX */
 i__1 = 0; i__2 = ilaenv_(&c__3, "ZGEQRF", " ", &sm, &sn, &c_n1, & c_n1); // , expr subst  
 nx = max(i__1,i__2);
 if (nx < sminmn) {
 ...
 nb = *lwork / (sn + 1);
 /* Computing MAX */
 i__1 = 2; i__2 = ilaenv_(&c__2, "ZGEQRF", " ", &sm, &sn, & c_n1, &c_n1); // , expr subst  
 nbmin = max(i__1,i__2);
 }
 }
 }
 ...
 L30: if (j <= topbmn) {
 /* Computing MIN */
 i__1 = nb; i__2 = topbmn - j + 1; // , expr subst  
 jb = min(i__1,i__2);
