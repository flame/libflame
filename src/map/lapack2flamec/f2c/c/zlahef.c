/* ../netlib/zlahef.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
/* > \brief \b ZLAHEF computes a partial factorization of a complex Hermitian indefinite matrix using the Bunc h-Kaufman diagonal pivoting method (blocked algorithm, calling Level 3 BLAS). */
/* =========== DOCUMENTATION =========== */
        ...
        else
        {
            ...
            else
            {
                ...
                if (imax > 1)
                {
                    ...
                    i__1 = jmax + (kw - 1) * w_dim1;
                    d__3 = rowmax;
                    d__4 = (d__1 = w[i__1].r, abs(d__1)) + ( d__2 = d_imag(&w[jmax + (kw - 1) * w_dim1]), abs( d__2)); // , expr subst
        ...
        else
        {
            ...
            else
            {
                ...
                if (imax < *n)
                {
                    ...
                    i__1 = jmax + (k + 1) * w_dim1;
                    d__3 = rowmax;
                    d__4 = (d__1 = w[i__1].r, abs(d__1)) + ( d__2 = d_imag(&w[jmax + (k + 1) * w_dim1]), abs( d__2)); // , expr subst
                    rowmax = max(d__3,d__4);
                }
                /* Case(2) */
                if (absakk >= alpha * colmax * (colmax / rowmax))
