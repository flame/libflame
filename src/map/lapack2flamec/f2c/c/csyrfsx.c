...
/* ../netlib/csyrfsx.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
,inf) * norm(Z,inf)) for some */
 ...
 char buffer[256]; 
#if FLA_ENABLE_ILP64 
 snprintf(buffer, 256,"csyrfsx inputs: uplo %c, equed %c, n %lld, nrhs %lld, lda %lld, ldaf %lld, ipiv %lld, ldb %lld, ldx %lld, n_err_bnds__ %lld, nparams %lld",*uplo, *equed, *n, *nrhs, *lda, *ldaf, *ipiv, *ldb, *ldx, *n_err_bnds__, *nparams);
#else 
 snprintf(buffer, 256,"csyrfsx inputs: uplo %c, equed %c, n %d, nrhs %d, lda %d, ldaf %d, ipiv %d, ldb %d, ldx %d, n_err_bnds__ %d, nparams %d",*uplo, *equed, *n, *nrhs, *lda, *ldaf, *ipiv, *ldb, *ldx, *n_err_bnds__, *nparams);
#endif
 ...
 *(unsigned char *)norm = 'I';
 ...
 if (rcequ) {
 rcond_tmp__ = cla_syrcond_c_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &ipiv[1], &s[1], &c_true, info, &work[1] , &rwork[1]);
 }
 else {
 rcond_tmp__ = cla_syrcond_c_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &ipiv[1], &s[1], &c_false, info, &work[ 1], &rwork[1]);
 }
 i__1 = *nrhs;
 for (j = 1;
 j <= i__1;
 ...
 *info = *n + j;
 ...
 rcond_tmp__ = cla_syrcond_x_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &ipiv[1], &x[j * x_dim1 + 1], info, &work[1], &rwork[1]);
 }
 else {
 rcond_tmp__ = 0.f;
 }
