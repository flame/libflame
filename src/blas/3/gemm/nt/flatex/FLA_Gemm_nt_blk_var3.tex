...
\renewcommand{\invariant}{
  \FlaOneByTwo{C_L}{C_R}  = 
  \FlaOneByTwo{ A B_T^T + \hat{C}_L }{ \hat{C}_R } 
}
...
=
  \FlaOneByThreeR{A B_0^T + \hat{C}_0}{\hat{C}_1}{\hat{C}_2}
}
...
=
  \FlaOneByThreeL{A B_0^T + \hat{C}_0}{A B_1^T + \hat{C}_1}{\hat{C}_2}
}
...
%  \ref{FLA_Gemm_nt:ws_blk_var1} in the \LaTeX source 
 
\label{FLA_Gemm_nt:ws_blk_var1} 
...
% The following command creates a label so that the figure can 
% be referenced by its number by inserting  
%  \ref{FLA_Gemm_nt:alg_blk_var1} in the \LaTeX source 
 
\label{FLA_Gemm_nt:alg_blk_var1} 
