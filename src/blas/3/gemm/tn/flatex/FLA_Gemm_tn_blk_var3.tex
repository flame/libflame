...
\renewcommand{\invariant}{
  \FlaOneByTwo{C_L}{C_R}  = 
  \FlaOneByTwo{ A^T B_{L} + \hat{C}_L }{\hat{C}_R} 
}
...
=
  \FlaOneByThreeR{ A^T B_{0} + \hat{C}_0 }{\hat{C}_1}{\hat{C}_2}
}
...
=
  \FlaOneByThreeL{ A^T B_{0} + \hat{C}_0 }{ A^T B_{1} + \hat{C}_1 }{\hat{C}_2}
}
...
%  \ref{FLA_Gemm_tn:ws_blk_var3} in the \LaTeX source 
 
\label{FLA_Gemm_tn:ws_blk_var3} 
...
% The following command creates a label so that the figure can 
% be referenced by its number by inserting  
%  \ref{FLA_Gemm_tn:alg_blk_var3} in the \LaTeX source 
 
\label{FLA_Gemm_tn:alg_blk_var3} 
