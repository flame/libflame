...
{
	char buffer[ INPUT_BUFFER_SIZE ], temp[ INPUT_BUFFER_SIZE ];
	integer i, op;
	FILE* input_stream;
										   ...
										   double* ) )       // residual
{
	...
	switch (api_type)
	{
		...
		case LIN:
			...
			thresh              = params->lin_solver_paramslist[0].solver_threhold;
			break;
		...
		case EIG_SYM:
			...
			thresh              = params->eig_sym_paramslist[0].threshold_value;
			break;
		...
		case EIG_NSYM:
			...
			thresh              = params->eig_non_sym_paramslist[0].gghrd_threshold;
			break;
		...
		case SVD:
			...
			thresh              = params->svd_paramslist[0].svd_threshold;
			break;
	...
	for ( dt = 0; dt < n_datatypes; ++dt )
	{
		...
		for ( p_cur = p_first, q_cur = q_first; (p_cur <= p_max && q_cur <= q_max); p_cur += p_inc, q_cur += q_inc )
		{
			...
			for ( pci = 0; pci < n_pc; ++pci )
			{
				...
				fla_test_output_info( "   %s%s  %c  %14"FS" x %-8"FS" %-10.3lf  %6.2lf %-6s  %-7.2le   %9s\n",
