...
void FLASH_Chol_cntl_init_ts(FLA_Cntl_init_flash_s *FLA_cntl_flash_init_i)
{
	// Set blocksize for hierarchical storage.
	FLA_cntl_flash_init_i->flash_chol_bsize       = FLA_Blocksize_create( 1, 1, 1, 1 );

	// Create a control tree that assumes A is a b x b block.
	FLA_cntl_flash_init_i->flash_chol_cntl_leaf   = FLA_Cntl_chol_obj_create( FLA_HIER,
	                                                   FLA_SUBPROBLEM,
	                                                   NULL,
	                                                   NULL,
	                                                   NULL,
	...
	FLA_cntl_flash_init_i->flash_chol_cntl        = FLA_Cntl_chol_obj_create( FLA_HIER,
	                                                   FLA_BLOCKED_VARIANT3, 
	                                                   FLA_cntl_flash_init_i->flash_chol_bsize,
	                                                   FLA_cntl_flash_init_i->flash_chol_cntl_leaf,
	                                                   FLA_cntl_flash_init_i->flash_herk_cntl_op,
	                                                   FLA_cntl_flash_init_i->flash_trsm_cntl_bp,
	                                                   NULL );
}

void FLASH_Chol_cntl_finalize_ts(FLA_Cntl_init_flash_s *FLA_cntl_flash_init_i)
{
	FLA_Cntl_obj_free( FLA_cntl_flash_init_i->flash_chol_cntl_leaf );
	FLA_Cntl_obj_free( FLA_cntl_flash_init_i->flash_chol_cntl );

	FLA_Blocksize_free( FLA_cntl_flash_init_i->flash_chol_bsize );
}
