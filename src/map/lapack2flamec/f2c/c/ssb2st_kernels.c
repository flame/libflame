/* ../netlib/v3.9.0/ssb2st_kernels.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/ssb2st_ kernels.f"> */
 /* > [TGZ]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/ssb2st_ kernels.f"> */
 /* > [ZIP]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/ssb2st_ kernels.f"> */
 /* > [TXT]</a> */
 /* > \endhtmlonly */
 /* Definition: */
 /* =========== */
 ...
 r__1 = tau[taupos];
 i__1 = *lda - 1;
 slarfx_("Left", &ln, &lm, &v[vpos], &r__1, &a[dpos - *nb + j1 * a_dim1], &i__1, &work[1]);
 if (*wantz) {
 vpos = (*sweep - 1) % 2 * *n + j1;
 ...
 i__1 = ln - 1;
 i__2 = *lda - 1;
 slarfx_("Right", &i__1, &lm, &v[vpos], &tau[taupos], &a[dpos - *nb + 1 + j1 * a_dim1], &i__2, &work[1]);
 }
 }
 /* Lower case */
 }
 ...
 if (lm > 0) {
 i__1 = *lda - 1;
 slarfx_("Right", &lm, &ln, &v[vpos], &tau[taupos], &a[dpos + * nb + *st * a_dim1], &i__1, &work[1]);
 if (*wantz) {
 vpos = (*sweep - 1) % 2 * *n + j1;
 taupos = (*sweep - 1) % 2 * *n + j1;
 }
 ...
 r__1 = tau[taupos];
 i__2 = *lda - 1;
 slarfx_("Left", &lm, &i__1, &v[vpos], &r__1, &a[dpos + *nb - 1 + (*st + 1) * a_dim1], &i__2, &work[1]);
 }
 }
 }
 return 0;
