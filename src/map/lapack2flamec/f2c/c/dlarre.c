/* ../netlib/dlarre.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
L21:
        ...
        if (irange == 1 && ! forceb || usedqd)
        {
            ...
            /* Computing MAX */
            d__2 = gl;
            d__3 = tmp - tmp1 - eps * 100. * (d__1 = tmp - tmp1, f2c_dabs(d__1)); // , expr subst
        ...
        else
        {
            ...
            /* Computing MAX */
            d__2 = gl;
            d__3 = w[wbegin] - werr[wbegin] - eps * 100. * (d__1 = w[wbegin] - werr[wbegin], f2c_dabs(d__1)); // , expr subst
            isleft = max(d__2,d__3);
            /* Computing MIN */
            d__2 = gu;
            d__3 = w[wend] + werr[wend] + eps * 100. * (d__1 = w[ wend] + werr[wend], f2c_dabs(d__1)); // , expr subst
            isrght = min(d__2,d__3);
        }
        ...
        if (usedqd)
        {
            ...
            /* Computing MAX */
            d__1 = tau;
            d__2 = eps * 2. * f2c_dabs(sigma); // , expr subst
            tau = max(d__1,d__2);
        }
        else
        {
            if (mb > 1)
            {
                ...
                avgap = (d__1 = clwdth / (doublereal) (wend - wbegin), f2c_dabs( d__1));
                if (sgndef == 1.)
                {
                    /* Computing MAX */
                    d__1 = wgap[wbegin];
                ...
                ++idum)
        {
                    ...
                    ++i__)
            {
                ...
                /* Computing MAX */
                d__1 = dmax__;
                d__2 = f2c_dabs(dpivot); // , expr subst
...
L83: /* At this point, we have found an initial base representation */
        ...
        if (! usedqd)
        {
            ...
            {
                w[j] -= sigma;
                werr[j] += (d__1 = w[j], f2c_dabs(d__1)) * eps;
                /* L134: */
            }
        ...
        else
        {
            ...
            for (i__ = 1;
                    ...
                    ++i__)
            {
                work[(i__ << 1) - 1] = (d__1 = d__[j], f2c_dabs(d__1));
                work[i__ * 2] = e[j] * e[j] * work[(i__ << 1) - 1];
                ++j;
                /* L140: */
            }
            work[(in << 1) - 1] = (d__1 = d__[iend], f2c_dabs(d__1));
            work[in * 2] = 0.;
                    ...
                    ++i__)
            {
                ...
                werr[i__] = rtol * (d__1 = w[i__], f2c_dabs(d__1));
                /* L165: */
            }
            i__2 = *m - 1;
            for (i__ = *m - mb + 1;
