/* ../netlib/dladiv.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int dladiv_(doublereal *a, doublereal *b, doublereal *c__, doublereal *d__, doublereal *p, doublereal *q)
{
    ...
    /* Computing MAX */
    d__1 = f2c_dabs(*a);
    d__2 = f2c_dabs(*b); // , expr subst
    ab = max(d__1,d__2);
    /* Computing MAX */
    d__1 = f2c_dabs(*c__);
    d__2 = f2c_dabs(*d__); // , expr subst
    cd = max(d__1,d__2);
    s = 1.;
