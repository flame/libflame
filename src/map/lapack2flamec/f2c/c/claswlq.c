/* ../netlib/v3.9.0/claswlq.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int claswlq_(integer *m, integer *n, integer *mb, integer * nb, complex *a, integer *lda, complex *t, integer *ldt, complex *work, integer *lwork, integer *info)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,"claswlq inputs: m %lld, n %lld, mb %lld, nb %lld, lda %lld, ldt %lld, lwork %lld",*m, *n, *mb, *nb, *lda, *ldt, *lwork);
#else
    snprintf(buffer, 256,"claswlq inputs: m %d, n %d, mb %d, nb %d, lda %d, ldt %d, lwork %d",*m, *n, *mb, *nb, *lda, *ldt, *lwork);
#endif
    ...
    *info = 0;
            ...
            i__ += i__2)
    {
        ...
        i__3 = *nb - *m;
        ctplqt_(m, &i__3, &c__0, mb, &a[a_dim1 + 1], lda, &a[i__ * a_dim1 + 1], lda, &t[(ctr * *m + 1) * t_dim1 + 1], ldt, &work[1], info);
        ++ctr;
    }
    /* Compute the QR factorization of the last block A(1:M,II:N) */
    if (ii <= *n)
    {
        ctplqt_(m, &kk, &c__0, mb, &a[a_dim1 + 1], lda, &a[ii * a_dim1 + 1], lda, &t[(ctr * *m + 1) * t_dim1 + 1], ldt, &work[1], info);
    }
    i__2 = *m * *mb;
    work[1].r = (real) i__2;
    work[1].i = 0.f; // , expr subst
