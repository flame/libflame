 ...
 */
 ...
 char buffer[256]; 
#if FLA_ENABLE_ILP64 
 snprintf(buffer, 256,"csyconvf_rook inputs: uplo %c, way %c, n %lld, lda %lld, ipiv %lld",*uplo, *way, *n, *lda, *ipiv);
#else 
 snprintf(buffer, 256,"csyconvf_rook inputs: uplo %c, way %c, n %d, lda %d, ipiv %d",*uplo, *way, *n, *lda, *ipiv);
#endif
