/* ../netlib/ctgsen.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
...
#else 
 snprintf(buffer, 256,"ctgsen inputs: ijob %d, n %d, lda %d, ldb %d, ldq %d, ldz %d, lwork %d, liwork %d",*ijob, *n, *lda, *ldb, *ldq, *ldz, *lwork, *liwork);
#endif
 ...
 real safmin;
 extern /* Subroutine */
 int ctgexc_(logical *, logical *, integer *, complex *, integer *, complex *, integer *, complex *, integer *, complex *, integer *, integer *, integer *, integer *), xerbla_( char *, integer *), classq_(integer *, complex *, integer *, real *, real *);
 integer liwmin;
 extern /* Subroutine */
 int ctgsyl_(char *, integer *, integer *, integer *, complex *, integer *, complex *, integer *, complex *, integer *, complex *, integer *, complex *, integer *, complex *, integer *, real *, real *, complex *, integer *, integer *, integer *);
 logical lquery;
 ...
 *m = 0;
 ...
 i__2 = k;
 i__3 = k + k * a_dim1;
 alpha[i__2].r = a[i__3].r; alpha[i__2].i = a[i__3].i; // , expr subst  
 i__2 = k;
 i__3 = k + k * b_dim1;
 beta[i__2].r = b[i__3].r; beta[i__2].i = b[i__3].i; // , expr subst  
 if (k < *n) {
 if (select[k]) {
 ++(*m);
 }
 ...
 /* Computing MAX */
 i__1 = 1; i__2 = (*m << 1) * (*n - *m); // , expr subst  
 lwmin = max(i__1,i__2);
 /* Computing MAX */
 i__1 = 1; i__2 = *n + 2; // , expr subst  
 liwmin = max(i__1,i__2);
 }
 else if (*ijob == 3 || *ijob == 5) {
 /* Computing MAX */
 i__1 = 1; i__2 = (*m << 2) * (*n - *m); // , expr subst  
 lwmin = max(i__1,i__2);
 /* Computing MAX */
 i__1 = 1, i__2 = (*m << 1) * (*n - *m); i__1 = max(i__1,i__2); i__2 = *n + 2; // ; expr subst  
 liwmin = max(i__1,i__2);
 }
 else {
 lwmin = 1;
 ...
 *pr = 1.f;
 ...
 if (k != ks) {
 ctgexc_(wantq, wantz, n, &a[a_offset], lda, &b[b_offset], ldb, &q[q_offset], ldq, &z__[z_offset], ldz, &k, &ks, & ierr);
 }
 if (ierr > 0) {
 ...
 *pr = 0.f;
 ...
 ijb = 0;
 i__1 = *lwork - (n1 << 1) * n2;
 ctgsyl_("N", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + i__ * a_dim1] , lda, &work[1], &n1, &b[b_offset], ldb, &b[i__ + i__ * b_dim1], ldb, &work[n1 * n2 + 1], &n1, &dscale, &dif[1], & work[(n1 * n2 << 1) + 1], &i__1, &iwork[1], &ierr);
 ...
 *pr = dscale / (sqrt(dscale * dscale / *pr + *pr) * sqrt(*pr));
 ...
 i__1 = *lwork - (n1 << 1) * n2;
 ctgsyl_("N", &ijb, &n2, &n1, &a[i__ + i__ * a_dim1], lda, &a[ a_offset], lda, &work[1], &n2, &b[i__ + i__ * b_dim1], ldb, &b[b_offset], ldb, &work[n1 * n2 + 1], &n2, &dscale, &dif[2], &work[(n1 * n2 << 1) + 1], &i__1, &iwork[1], & ierr);
 }
 else {
 ...
 L40: clacn2_(&mn2, &work[mn2 + 1], &work[1], &dif[1], &kase, isave);
 if (kase != 0) {
 if (kase == 1) {
 ...
 i__1 = *lwork - (n1 << 1) * n2;
 ctgsyl_("C", &ijb, &n1, &n2, &a[a_offset], lda, &a[i__ + i__ * a_dim1], lda, &work[1], &n1, &b[b_offset], ldb, &b[i__ + i__ * b_dim1], ldb, &work[n1 * n2 + 1], &n1, &dscale, &dif[1], &work[(n1 * n2 << 1) + 1], &i__1, &iwork[1], &ierr);
 }
 goto L40;
 }
 dif[1] = dscale / dif[1];
 /* 1-norm-based estimate of Difl. */
 L50: clacn2_(&mn2, &work[mn2 + 1], &work[1], &dif[2], &kase, isave);
 if (kase != 0) {
 if (kase == 1) {
 /* Solve generalized Sylvester equation */
 i__1 = *lwork - (n1 << 1) * n2;
 ...
 /* Solve the transposed variant. */
 i__1 = *lwork - (n1 << 1) * n2;
 ctgsyl_("C", &ijb, &n2, &n1, &a[i__ + i__ * a_dim1], lda, &a[a_offset], lda, &work[1], &n2, &b[b_offset], ldb, &b[i__ + i__ * b_dim1], ldb, &work[n1 * n2 + 1], &n2, &dscale, &dif[2], &work[(n1 * n2 << 1) + 1], &i__1, &iwork[1], &ierr);
 }
 goto L50;
 }
 dif[2] = dscale / dif[2];
 ...
 if (dscale > safmin) {
 i__2 = k + k * b_dim1;
 q__2.r = b[i__2].r / dscale; q__2.i = b[i__2].i / dscale; // , expr subst  
 r_cnjg(&q__1, &q__2);
 temp1.r = q__1.r; temp1.i = q__1.i; // , expr subst  
 i__2 = k + k * b_dim1;
 q__1.r = b[i__2].r / dscale; q__1.i = b[i__2].i / dscale; // , expr subst  
 temp2.r = q__1.r; temp2.i = q__1.i; // , expr subst  
 i__2 = k + k * b_dim1;
 b[i__2].r = dscale; b[i__2].i = 0.f; // , expr subst  
 i__2 = *n - k;
 ...
 i__2 = k + k * b_dim1;
 b[i__2].r = 0.f; b[i__2].i = 0.f; // , expr subst  
 }
 i__2 = k;
 i__3 = k + k * a_dim1;
 alpha[i__2].r = a[i__3].r; alpha[i__2].i = a[i__3].i; // , expr subst  
 ...
 L70: work[1].r = (real) lwmin; work[1].i = 0.f; // , expr subst  
 iwork[1] = liwmin;
 AOCL_DTL_TRACE_EXIT(AOCL_DTL_LEVEL_TRACE_5);
 return 0;
 /* End of CTGSEN */
