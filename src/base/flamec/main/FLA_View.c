                        ...
                        dim_t  mb,  dim_t     nb, FLA_Quadrant quadrant )
{
  ...
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
    FLA_Part_2x2_check_ts( FLA_cntl_init_i, A,    A11, A12,
...
{ 
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
    FLA_Part_2x1_check_ts( FLA_cntl_init_i, A,    A1,
                              A2,     mb, side );
...
{
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
    FLA_Part_1x2_check_ts( FLA_cntl_init_i, A,    A1,  A2,      nb, side );
...
{
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
    FLA_Repart_2x2_to_3x3_check_ts( FLA_cntl_init_i, ATL, ATR,       A00, A01, A02,
                                                 A10, A11, A12,
                                 ABL, ABR,       A20, A21, A22,
                                 mb, nb, quadrant );
  ...
  if ( quadrant == FLA_BR )
  { 
    FLA_Part_2x2_ts( FLA_cntl_init_i, ABR,    A11, A12,
                          A21, A22,    mb, nb, FLA_TL );
 
    FLA_Part_1x2_ts( FLA_cntl_init_i, ATR,    A01, A02,    nb, FLA_LEFT );
 
    FLA_Part_2x1_ts( FLA_cntl_init_i, ABL,    A10, 
                          A20,         mb, FLA_TOP );
  ...
  {
    FLA_Part_2x2_ts( FLA_cntl_init_i, ABL,    A10, A11,
                          A20, A21,    mb, nb, FLA_TR );
 
    FLA_Part_1x2_ts( FLA_cntl_init_i, ATL,    A00, A01,    nb, FLA_RIGHT );
 
    FLA_Part_2x1_ts( FLA_cntl_init_i, ABR,    A12, 
                          A22,         mb, FLA_TOP );
  ...
  {
    FLA_Part_2x2_ts( FLA_cntl_init_i, ATL,    A00, A01,
                          A10, A11,    mb, nb, FLA_BR );
 
    FLA_Part_1x2_ts( FLA_cntl_init_i, ABL,    A20, A21,    nb, FLA_RIGHT );
 
    FLA_Part_2x1_ts( FLA_cntl_init_i, ATR,    A02, 
                          A12,         mb, FLA_BOTTOM );
  ...
  {
    FLA_Part_2x2_ts( FLA_cntl_init_i, ATR,    A01, A02,
                          A11, A12,    mb, nb, FLA_BL );

    FLA_Part_1x2_ts( FLA_cntl_init_i, ABR,    A21, A22,    nb, FLA_LEFT );

    FLA_Part_2x1_ts( FLA_cntl_init_i, ATL,    A00,
                          A10,         mb, FLA_BOTTOM );
...
{
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
    FLA_Repart_2x1_to_3x1_check_ts( FLA_cntl_init_i, AT,     A0, 
                                         A1, 
                                 AB,     A2,     mb, side );

  if ( side == FLA_TOP )
  {
    FLA_Part_2x1_ts ( FLA_cntl_init_i, AT,    A0, 
                          A1,    mb, FLA_BOTTOM );
  ...
  else
  {
    ...
    FLA_Part_2x1_ts( FLA_cntl_init_i, AB,    A1, 
                          A2,    mb, FLA_TOP );
  }
...
{
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
    FLA_Repart_1x2_to_1x3_check_ts( FLA_cntl_init_i, AL, AR,        A0, A1, A2,
                                 nb, side );

  if ( side == FLA_LEFT )
  {
    FLA_Part_1x2_ts( FLA_cntl_init_i, AL,    A0, A1,    nb, FLA_RIGHT );
  ...
  else
  {
    ...
    FLA_Part_1x2_ts( FLA_cntl_init_i, AR,    A1, A2,    nb, FLA_LEFT );
  }
...
{
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
    FLA_Cont_with_3x3_to_2x2_check_ts( FLA_cntl_init_i, ATL, ATR,       A00, A01, A02,
                                                    A10, A11, A12,
                                    ABL, ABR,       A20, A21, A22,
                                    quadrant );
...
{
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
    FLA_Cont_with_3x1_to_2x1_check_ts( FLA_cntl_init_i, 
                                       AT,     A0, 
                                            A1, 
                                       AB,     A2,     side );
...
{
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i ) == FLA_FULL_ERROR_CHECKING )
    FLA_Cont_with_1x3_to_1x2_check_ts( FLA_cntl_init_i, AL, AR,        A0, A1, A2,
                                    side );
...
{
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
    FLA_Merge_2x1_check_ts( FLA_cntl_init_i, AT,
                         AB,   A );
