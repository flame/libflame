/* ../netlib/v3.9.0/sgejsv.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -15;
 ...
 {
 /* Computing MAX */
 i__1 = 7, i__2 = (*n << 2) + 1; i__1 = max(i__1,i__2); i__2 = (*m << 1) + *n; // ; expr subst  
 /* Computing MAX */
 i__3 = 7, i__4 = (*n << 2) + *n * *n; i__3 = max(i__3,i__4); i__4 = (* m << 1) + *n; // ; expr subst  
 /* Computing MAX */
 i__5 = 7, i__6 = (*m << 1) + *n; i__5 = max(i__5,i__6); i__6 = (*n << 2) + 1; // ; expr subst  
 /* Computing MAX */
 i__7 = 7, i__8 = (*m << 1) + *n; i__7 = max(i__7,i__8); i__8 = (*n << 2) + 1; // ; expr subst  
 /* Computing MAX */
 i__9 = (*m << 1) + *n; i__10 = *n * 6 + (*n << 1) * *n; // , expr subst  
 /* Computing MAX */
 i__11 = (*m << 1) + *n, i__12 = (*n << 2) + *n * *n; i__11 = max( i__11,i__12); i__12 = (*n << 1) + *n * *n + 6; // ; expr subst  
 ...
 *info = -9;
 ...
 /* Computing MAX */
 r__1 = aatmax; r__2 = work[*n + p]; // , expr subst  
 aatmax = max(r__1,r__2);
 if (work[*n + p] != 0.f) {
 /* Computing MIN */
 r__1 = aatmin; r__2 = work[*n + p]; // , expr subst  
 ...
 p <= i__1;
 ++p) {
 work[*m + *n + p] = scalem * (r__1 = a[p + isamax_(n, &a[p + a_dim1], lda) * a_dim1], abs(r__1));
 /* Computing MAX */
 r__1 = aatmax; r__2 = work[*m + *n + p]; // , expr subst  
 aatmax = max(r__1,r__2);
 /* Computing MIN */
 r__1 = aatmin; r__2 = work[*m + *n + p]; // , expr subst  
 aatmin = min(r__1,r__2);
 /* L1904: */
 }
 }
 ...
 p <= i__1;
 ++p) {
 if ((r__2 = a[p + p * a_dim1], abs(r__2)) >= temp1 * (r__1 = a[ a_dim1 + 1], abs(r__1))) {
 ++nr;
 }
 else {
 goto L3002;
 ...
 p <= i__1;
 ++p) {
 if ((r__2 = a[p + p * a_dim1], abs(r__2)) < epsln * (r__1 = a[p - 1 + (p - 1) * a_dim1], abs(r__1)) || (r__3 = a[p + p * a_dim1], abs(r__3)) < small || l2kill && (r__4 = a[p + p * a_dim1], abs(r__4)) < temp1) {
 goto L3402;
 }
 ++nr;
 /* L3401: */
 ...
 p <= i__1;
 ++p) {
 if ((r__1 = a[p + p * a_dim1], abs(r__1)) < small || l2kill && ( r__2 = a[p + p * a_dim1], abs(r__2)) < temp1) {
 goto L3302;
 }
 ++nr;
 /* L3301: */
 ...
 spocon_("U", n, &v[v_offset], ldv, &c_b35, &temp1, &work[*n + 1], &iwork[(*n << 1) + *m + 1], &ierr);
 }
 else if (lsvec) {
 ...
 spocon_("U", n, &u[u_offset], ldu, &c_b35, &temp1, &work[*n + 1], &iwork[(*n << 1) + *m + 1], &ierr);
 }
 else {
 ...
 spocon_("U", n, &work[*n + 1], n, &c_b35, &temp1, &work[*n + * n * *n + 1], &iwork[(*n << 1) + *m + 1], &ierr);
 }
 ...
 l2pert = l2pert && (r__1 = a[a_dim1 + 1] / a[nr + nr * a_dim1], abs(r__1)) > sqrt(big1);
 ...
 p <= i__2;
 ++p) {
 if (p > q && (r__1 = a[p + q * a_dim1], abs(r__1)) <= temp1 || p < q) {
 ...
 i__1 = *lwork - *n;
 sormlq_("Left", "Transpose", n, n, &nr, &a[a_offset], lda, &work[ 1], &v[v_offset], ldv, &work[*n + 1], &i__1, &ierr);
 }
 i__1 = *n;
 for (p = 1;
 p <= i__1;
 ...
 i__1 = *lwork - *n;
 sormqr_("Left", "No Tr", m, &n1, n, &a[a_offset], lda, &work[1], &u[ u_offset], ldu, &work[*n + 1], &i__1, &ierr);
 if (rowpiv) {
 i__1 = *m - 1;
 ...
 p <= i__2;
 ++p) {
 if (p > q && (r__1 = v[p + q * v_dim1], abs(r__1)) <= temp1 || p < q) {
 v[p + q * v_dim1] = r_sign(&temp1, &v[p + q * v_dim1]);
 }
 if (p < q) {
 ...
 spocon_("Lower", &nr, &work[(*n << 1) + 1], &nr, &c_b35, & temp1, &work[(*n << 1) + nr * nr + 1], &iwork[*m + (* n << 1) + 1], &ierr);
 ...
 ++q) {
 /* Computing MIN */
 r__3 = (r__1 = v[p + p * v_dim1], abs(r__1)); r__4 = (r__2 = v[q + q * v_dim1], abs( r__2)); // , expr subst  
 temp1 = xsc * min(r__3,r__4);
 if ((r__1 = v[q + p * v_dim1], abs(r__1)) <= temp1) {
 ...
 i__1 = *lwork - (*n << 1);
 sgeqp3_(n, &nr, &v[v_offset], ldv, &iwork[*n + 1], &work[* n + 1], &work[(*n << 1) + 1], &i__1, &ierr);
 ...
 sgelqf_(&nr, &nr, &v[v_offset], ldv, &work[(*n << 1) + *n * nr + 1], &work[(*n << 1) + *n * nr + nr + 1], & i__1, &ierr);
 /* .. and estimate the condition number */
 slacpy_("L", &nr, &nr, &v[v_offset], ldv, &work[(*n << 1) + *n * nr + nr + 1], &nr);
 i__1 = nr;
 for (p = 1;
 p <= i__1;
 ++p) {
 ...
 spocon_("L", &nr, &work[(*n << 1) + *n * nr + nr + 1], & nr, &c_b35, &temp1, &work[(*n << 1) + *n * nr + nr + nr * nr + 1], &iwork[*m + (*n << 1) + 1], & ierr);
 ...
 strsm_("L", "U", "N", "N", &nr, &nr, &c_b35, &a[ a_offset], lda, &v[v_offset], ldv);
 }
 else {
 ...
 strsm_("L", "U", "T", "N", &nr, &nr, &c_b35, &work[(* n << 1) + 1], n, &v[v_offset], ldv);
 if (nr < *n) {
 i__1 = *n - nr;
 slaset_("A", &i__1, &nr, &c_b34, &c_b34, &v[nr + 1 + v_dim1], ldv);
 i__1 = *n - nr;
 ...
 i__1 = *lwork - (*n << 1) - *n * nr - nr;
 sormqr_("L", "N", n, n, &nr, &work[(*n << 1) + 1], n, &work[*n + 1], &v[v_offset], ldv, &work[(*n << 1) + *n * nr + nr + 1], &i__1, &ierr);
 }
 }
 ...
 strsm_("L", "U", "N", "N", &nr, &nr, &c_b35, &work[(*n << 1) + 1], n, &u[u_offset], ldu);
 ...
 sormqr_("L", "N", n, n, &nr, &work[(*n << 1) + 1], n, & work[*n + 1], &v[v_offset], ldv, &work[(*n << 1) + *n * nr + nr + 1], &i__1, &ierr);
 i__1 = *lwork - (*n << 1) - *n * nr - nr;
 sormlq_("L", "T", &nr, &nr, &nr, &work[(*n << 1) + 1], n, &work[(*n << 1) + *n * nr + 1], &u[u_offset], ldu, &work[(*n << 1) + *n * nr + nr + 1], &i__1, & ierr);
 i__1 = nr;
 for (q = 1;
 q <= i__1;
 ++q) {
 ...
 i__1 = *lwork - *n;
 sormqr_("Left", "No_Tr", m, &n1, n, &a[a_offset], lda, &work[ 1], &u[u_offset], ldu, &work[*n + 1], &i__1, &ierr);
 ...
 q <= i__2;
 ++q) {
 work[*n + (q - 1) * *n + p] = -r_sign(&temp1, & work[*n + (p - 1) * *n + q]);
 /* L5971: */
 }
 /* L5970: */
 }
 ...
 strsm_("Left", "Upper", "NoTrans", "No UD", n, n, &c_b35, &a[ a_offset], lda, &work[*n + 1], n);
 i__1 = *n;
 for (p = 1;
 p <= i__1;
 ++p) {
 ...
 i__1 = *m - *n;
 i__2 = n1 - *n;
 slaset_("A", &i__1, &i__2, &c_b34, &c_b35, &u[*n + 1 + (*n + 1) * u_dim1], ldu);
 }
 }
 i__1 = *lwork - *n;
 sormqr_("Left", "No Tr", m, &n1, n, &a[a_offset], lda, &work[ 1], &u[u_offset], ldu, &work[*n + 1], &i__1, &ierr);
 ...
 ++p) {
 /* Computing MIN */
 r__3 = (r__1 = u[p + p * u_dim1], abs(r__1)); r__4 = ( r__2 = u[q + q * u_dim1], abs(r__2)); // , expr subst  
 temp1 = xsc * min(r__3,r__4);
