/* dorglq.f -- translated by f2c (version 20000121). You must link the resulting object file with the libraries: -lf2c -lm (in that order) */
/*
...
#include "FLA_f2c.h" /* Table of constant values */
 ...
 /* ===================================================================== */
 /* Subroutine */
 int lapack_dorglq(integer *m, integer *n, integer *k, doublereal * a, integer *lda, doublereal *tau, doublereal *work, integer *lwork, integer *info) {
 /* System generated locals */
 integer a_dim1, a_offset, i__1, i__2, i__3;
 /* Local variables */
 integer i__, j, l, nbmin, iinfo;
 extern /* Subroutine */
 int lapack_dorgl2(integer *, integer *, integer *, doublereal *, integer *, doublereal *, doublereal *, integer *);
 integer ib, nb, ki, kk;
 extern /* Subroutine */
 ...
 *info = -8;
 ...
 /* Computing MAX */
 i__1 = 0; i__2 = ilaenv_(&c__3, "DORGLQ", " ", m, n, k, &c_n1); // , expr subst  
 nx = max(i__1,i__2);
 if (nx < *k) {
 ...
 nb = *lwork / ldwork;
 /* Computing MAX */
 i__1 = 2; i__2 = ilaenv_(&c__2, "DORGLQ", " ", m, n, k, &c_n1); // , expr subst  
 nbmin = max(i__1,i__2);
 }
 }
 }
 ...
 i__2 = *n - kk;
 i__3 = *k - kk;
 lapack_dorgl2(&i__1, &i__2, &i__3, &a[kk + 1 + (kk + 1) * a_dim1], lda, & tau[kk + 1], &work[1], &iinfo);
 }
 if (kk > 0) {
 /* Use blocked code */
 i__1 = -nb;
 ...
 i__2 = *m - i__ - ib + 1;
 i__3 = *n - i__ + 1;
 dlarfb_("Right", "Transpose", "Forward", "Rowwise", &i__2, & i__3, &ib, &a[i__ + i__ * a_dim1], lda, &work[1], & ldwork, &a[i__ + ib + i__ * a_dim1], lda, &work[ib + 1], &ldwork);
 }
 /* Apply H**T to columns i:n of current block */
 i__2 = *n - i__ + 1;
 lapack_dorgl2(&ib, &i__2, &ib, &a[i__ + i__ * a_dim1], lda, &tau[i__], & work[1], &iinfo);
