/* ../netlib/v3.9.0/slarrd.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *m = 1;
 ...
 /* Computing MAX */
 r__1 = abs(gl); r__2 = abs(gu); // , expr subst  
 tnorm = max(r__1,r__2);
 ...
 */
 ...
 ++j) {
 /* Computing MIN */
 r__1 = gl; r__2 = gers[(j << 1) - 1]; // , expr subst  
 gl = min(r__1,r__2);
 /* Computing MAX */
 r__1 = gu; r__2 = gers[j * 2]; // , expr subst  
 gu = max(r__1,r__2);
 /* L40: */
 }
 /* [JAN/28/2009] */
 ...
 *info = iinfo;
 ...
 tmp2 = (r__1 = work[j + *n] - work[j + in + *n], abs(r__1)) * .5f;
 if (j > iout - iinfo) {
