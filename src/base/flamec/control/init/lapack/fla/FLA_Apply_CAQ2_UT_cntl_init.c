...
void FLA_Apply_CAQ2_UT_cntl_init_ts(FLA_Cntl_init_flamec_s *FLA_cntl_flamec_init_i)
{
	...
	// but scaled down.
	FLA_cntl_flamec_init_i->fla_apcaq2ut_var1_bsize = FLA_Query_blocksizes( FLA_DIMENSION_MIN );
	FLA_Blocksize_scale_ts( FLA_cntl_flamec_init_i, FLA_cntl_flamec_init_i->fla_apcaq2ut_var1_bsize, FLA_CAQR_INNER_TO_OUTER_B_RATIO );

	// Create a control tree to invoke variant 1.
	FLA_cntl_flamec_init_i->fla_apcaq2ut_cntl_leaf = FLA_Cntl_apcaq2ut_obj_create( FLA_FLAT,
	                                                       FLA_BLOCKED_VARIANT1, 
	                                                       FLA_cntl_flamec_init_i->fla_apcaq2ut_var1_bsize,
	                                                       NULL,
...
{
	FLA_Cntl_obj_free( FLA_cntl_flamec_init_i->fla_apcaq2ut_cntl_leaf );

	FLA_Blocksize_free( FLA_cntl_flamec_init_i->fla_apcaq2ut_var1_bsize );
}
#endif
