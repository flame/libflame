 ...
 */
 ...
 char buffer[256]; 
#if FLA_ENABLE_ILP64 
 snprintf(buffer, 256,"clasyf_rk inputs: uplo %c, n %lld, nb %lld, lda %lld, ldw %lld",*uplo, *n, *nb, *lda, *ldw);
#else 
 snprintf(buffer, 256,"clasyf_rk inputs: uplo %c, n %d, nb %d, lda %d, ldw %d",*uplo, *n, *nb, *lda, *ldw);
#endif
