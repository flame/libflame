/* ../netlib/v3.9.0/dgehrd.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -8;
 ...
 /* Computing MIN */
 i__1 = 64; i__2 = ilaenv_(&c__1, "DGEHRD", " ", n, ilo, ihi, &c_n1); // , expr subst  
 nb = min(i__1,i__2);
 lwkopt = *n * nb + 4160;
 ...
 /* Computing MAX */
 i__1 = nb; i__2 = ilaenv_(&c__3, "DGEHRD", " ", n, ilo, ihi, &c_n1); // , expr subst  
 nx = max(i__1,i__2);
 if (nx < nh) {
 ...
 /* unblocked code */
 /* Computing MAX */
 i__1 = 2; i__2 = ilaenv_(&c__2, "DGEHRD", " ", n, ilo, ihi, & c_n1); // , expr subst  
 nbmin = max(i__1,i__2);
 ...
 dlahr2_(ihi, &i__, &ib, &a[i__ * a_dim1 + 1], lda, &tau[i__], & work[iwt], &c__65, &work[1], &ldwork);
 ...
 i__3 = *ihi - i__;
 i__4 = *n - i__ - ib + 1;
 dlarfb_("Left", "Transpose", "Forward", "Columnwise", &i__3, & i__4, &ib, &a[i__ + 1 + i__ * a_dim1], lda, &work[iwt], & c__65, &a[i__ + 1 + (i__ + ib) * a_dim1], lda, &work[1], & ldwork);
 /* L40: */
 }
 }
