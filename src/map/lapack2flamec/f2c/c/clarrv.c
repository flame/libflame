/* ../netlib/v3.9.0/clarrv.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
;
 ...
 /* > \brief \b CLARRV computes the eigenvectors of the tridiagonal matrix T = L D LT given L, D and the eigenv alues of L D LT. */
 /* =========== DOCUMENTATION =========== */
 ...
 *rtol2 = eps * 4.f;
 ...
 z__[i__2].r = 1.f; z__[i__2].i = 0.f; // , expr subst  
 ...
 *info = -2;
 ...
 claset_("Full", &in, &c__2, &c_b1, &c_b1, &z__[ibegin + j * z_dim1], ldz);
 }
 ...
 *info = -1;
 ...
 if (oldfst > 1) {
 /* Computing MAX */
 r__1 = wgap[wbegin + oldfst - 2]; r__2 = w[wbegin + oldfst - 1] - werr[wbegin + oldfst - 1] - w[ wbegin + oldfst - 2] - werr[wbegin + oldfst - 2]; // , expr subst  
 wgap[wbegin + oldfst - 2] = max(r__1,r__2);
 }
 if (wbegin + oldlst - 1 < wend) {
 /* Computing MAX */
 r__1 = wgap[wbegin + oldlst - 1]; r__2 = w[wbegin + oldlst] - werr[wbegin + oldlst] - w[wbegin + oldlst - 1] - werr[wbegin + oldlst - 1]; // , expr subst  
 ...
 else if (wgap[wbegin + j - 1] >= *minrgp * (r__1 = work[ wbegin + j - 1], abs(r__1))) {
 ...
 if (newfst == 1) {
 /* Computing MAX */
 r__1 = 0.f; r__2 = w[wbegin] - werr[wbegin] - *vl; // , expr subst  
 lgap = max(r__1,r__2);
 }
 else {
 ...
 slarrf_(&in, &d__[ibegin], &l[ibegin], &work[indld + ibegin - 1], &newfst, &newlst, &work[wbegin], &wgap[wbegin], &werr[wbegin], &spdiam, &lgap, &rgap, pivmin, &tau, &work[indin1], &work[ indin2], &work[indwrk], &iinfo);
 ...
 i__6 = indin1 + k - 1;
 q__1.r = work[i__6]; q__1.i = 0.f; // , expr subst  
 z__[i__5].r = q__1.r; z__[i__5].i = q__1.i; // , expr subst  
 ...
 i__5 = indin1 + in - 1;
 q__1.r = work[i__5]; q__1.i = 0.f; // , expr subst  
 ...
 q__1.r = ssigma; q__1.i = 0.f; // , expr subst  
 z__[i__4].r = q__1.r; z__[i__4].i = q__1.i; // , expr subst  
 ...
 k <= i__4;
 ++k) {
 fudge = eps * 3.f * (r__1 = work[wbegin + k - 1], abs(r__1));
 work[wbegin + k - 1] -= tau;
 fudge += eps * 4.f * (r__1 = work[wbegin + k - 1], abs(r__1));
 /* Fudge errors */
 ...
 *info = -2;
 ...
 /* Computing MAX */
 r__1 = abs(left); r__2 = abs(right); // , expr subst  
 ...
 *info = -3;
 ...
 L__1 = ! usedbs;
 clar1v_(&in, &c__1, &in, &lambda, &d__[ibegin], &l[ ibegin], &work[indld + ibegin - 1], &work[ indlld + ibegin - 1], pivmin, &gaptol, &z__[ ibegin + windex * z_dim1], &L__1, &negcnt, & ztz, &mingma, &iwork[iindr + windex], &isuppz[ (windex << 1) - 1], &nrminv, &resid, &rqcorr, &work[indwrk]);
 if (iter == 0) {
 bstres = resid;
 bstw = lambda;
 }
 ...
 *info = 5;
 ...
 z__[i__5].r = 0.f; z__[i__5].i = 0.f; // , expr subst  
 /* L122: */
 }
 }
 if (isupmx > zto) {
 ...
 i__4 = zto - zfrom + 1;
 csscal_(&i__4, &nrminv, &z__[zfrom + windex * z_dim1], &c__1);
 L125: /* Update W */
 ...
 if (k > 1) {
 /* Computing MAX */
 r__1 = wgap[windmn]; r__2 = w[windex] - werr[ windex] - w[windmn] - werr[windmn]; // , expr subst  
 wgap[windmn] = max(r__1,r__2);
 }
 if (windex < wend) {
 /* Computing MAX */
 r__1 = savgap; r__2 = w[windpl] - werr[windpl] - w[windex] - werr[windex]; // , expr subst  
