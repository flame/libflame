...
// Static variables.
static char* op_str                   = "LU factorization with incremental pivoting";
static char* flash_front_str          = "FLASH_LU_incpiv";
...
// Local prototypes.
void libfla_test_lu_incpiv_experiment( test_params_t params,
                                       unsigned int  var,
                                       char*         sc_str,
                                       ...
                                       double*       perf,
                                       double*       residual );
void libfla_test_lu_incpiv_impl( int     impl,
                                 FLA_Obj A,
                                 FLA_Obj p,
                                 FLA_Obj L );


void libfla_test_lu_incpiv( FILE* output_stream, test_params_t params, test_op_t op )
{
	...
	if ( op.flash_front == ENABLE )
	{
		...
		libfla_test_op_driver( flash_front_str, NULL,
		                       ...
		                       params, thresh, libfla_test_lu_incpiv_experiment );
	}
                                       ...
                                       double*       residual )
{
	...
	FLASH_LU_incpiv_create_hier_matrices( A, 1, &b_flash, b_alg_hier, &A_test, &p_test, &L_test );
	...
	for ( i = 0; i < n_repeats; ++i )
	{
		...
		libfla_test_lu_incpiv_impl( impl, A_test, p_test, L_test );
