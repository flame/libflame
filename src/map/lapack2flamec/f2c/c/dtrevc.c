/* ../netlib/v3.9.0/dtrevc.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -11;
 ...
 work[j + 1 + *n] = x[1];
 /* Computing MAX */
 d__3 = (d__1 = work[j + *n], abs(d__1)); d__4 = (d__2 = work[j + 1 + *n], abs(d__2)); d__3 = max( d__3,d__4); // ; expr subst  
 vmax = max(d__3,vmax);
 vcrit = bignum / vmax;
 }
 L170: ;
 ...
 work[j + n2] = x[2];
 /* Computing MAX */
 d__3 = (d__1 = work[j + *n], abs(d__1)); d__4 = (d__2 = work[j + n2], abs(d__2)); d__3 = max(d__3, d__4); // ; expr subst  
 vmax = max(d__3,vmax);
 vcrit = bignum / vmax;
 }
 else {
