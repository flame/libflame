/* ../netlib/clahef.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
/* > \brief \b CLAHEF computes a partial factorization of a complex Hermitian indefinite matrix using the Bunc h-Kaufman diagonal pivoting method (blocked algorithm, calling Level 3 BLAS). */
/* =========== DOCUMENTATION =========== */
...
L30: /* Update the upper triangle of A11 (= A(1:k,1:k)) as */
        ...
        j = k + 1;
L60: /* Undo the interchanges (if any) of rows J and JP */
        /* at each step J */
...
L90: /* Update the lower triangle of A22 (= A(k:n,k:n)) as */
        ...
        j = k - 1;
L120: /* Undo the interchanges (if any) of rows J and JP */
        /* at each step J */
        /* (Here, J is a diagonal index) */
        jj = j;
        jp = ipiv[j];
