/* ../netlib/cggrqf.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
#else 
 snprintf(buffer, 256,"cggrqf inputs: m %d, p %d, n %d, lda %d, ldb %d",*m, *p, *n, *lda, *ldb);
#endif
 ...
 *info = -8;
 ...
 {
 /* Computing MAX */
 i__1 = max(1,*m); i__1 = max(i__1,*p); // , expr subst  
 ...
 *info = -11;
 ...
 i__1 = min(*m,*n);
 /* Computing MAX */
 i__2 = 1; i__3 = *m - *n + 1; // , expr subst  
 cunmrq_("Right", "Conjugate Transpose", p, n, &i__1, &a[max(i__2,i__3) + a_dim1], lda, &taua[1], &b[b_offset], ldb, &work[1], lwork, info);
 /* Computing MAX */
 i__1 = lopt; i__2 = (integer) work[1].r; // , expr subst  
 lopt = max(i__1,i__2);
 /* QR factorization of P-by-N matrix B: B = Z*T */
 cgeqrf_(p, n, &b[b_offset], ldb, &taub[1], &work[1], lwork, info);
 /* Computing MAX */
 i__2 = lopt; i__3 = (integer) work[1].r; // , expr subst  
 i__1 = max(i__2,i__3);
