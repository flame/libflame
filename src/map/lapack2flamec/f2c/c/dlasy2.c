/* ../netlib/v3.9.0/dlasy2.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = 0;
 ...
 L10: tau1 = tl[tl_dim1 + 1] + sgn * tr[tr_dim1 + 1];
 bet = abs(tau1);
 if (bet <= smlnum) {
 tau1 = smlnum;
 bet = smlnum;
 ...
 *xnorm = (d__1 = x[x_dim1 + 1], abs(d__1));
 ...
 L20: /* Computing MAX */
 /* Computing MAX */
 d__7 = (d__1 = tl[tl_dim1 + 1], abs(d__1)), d__8 = (d__2 = tr[tr_dim1 + 1] , abs(d__2)), d__7 = max(d__7,d__8), d__8 = (d__3 = tr[(tr_dim1 << 1) + 1], abs(d__3)), d__7 = max(d__7,d__8), d__8 = (d__4 = tr[ tr_dim1 + 2], abs(d__4)); d__7 = max(d__7,d__8); d__8 = (d__5 = tr[(tr_dim1 << 1) + 2], abs(d__5)); // ; expr subst  
 ...
 L30: /* Computing MAX */
 /* Computing MAX */
 d__7 = (d__1 = tr[tr_dim1 + 1], abs(d__1)), d__8 = (d__2 = tl[tl_dim1 + 1] , abs(d__2)), d__7 = max(d__7,d__8), d__8 = (d__3 = tl[(tl_dim1 << 1) + 1], abs(d__3)), d__7 = max(d__7,d__8), d__8 = (d__4 = tl[ tl_dim1 + 2], abs(d__4)); d__7 = max(d__7,d__8); d__8 = (d__5 = tl[(tl_dim1 << 1) + 2], abs(d__5)); // ; expr subst  
 d__6 = eps * max(d__7,d__8);
 smin = max(d__6,smlnum);
 tmp[0] = tl[tl_dim1 + 1] + sgn * tr[tr_dim1 + 1];
 ...
 *scale = 1.;
 ...
 /* Computing MAX */
 d__1 = abs(btmp[0]); d__2 = abs(btmp[1]); // , expr subst  
 ...
 *xnorm = (d__1 = x[x_dim1 + 1], abs(d__1)) + (d__2 = x[(x_dim1 << 1) + 1], abs(d__2));
 ...
 x[x_dim1 + 2] = x2[1];
 /* Computing MAX */
 d__3 = (d__1 = x[x_dim1 + 1], abs(d__1)); d__4 = (d__2 = x[x_dim1 + 2] , abs(d__2)); // , expr subst  
 *xnorm = max(d__3,d__4);
 }
 return 0;
 /* 2 by 2: */
 ...
 /* Set pivots less than SMIN to SMIN. */
 L50: /* Computing MAX */
 d__5 = (d__1 = tr[tr_dim1 + 1], abs(d__1)), d__6 = (d__2 = tr[(tr_dim1 << 1) + 1], abs(d__2)), d__5 = max(d__5,d__6), d__6 = (d__3 = tr[ tr_dim1 + 2], abs(d__3)); d__5 = max(d__5,d__6); d__6 = (d__4 = tr[(tr_dim1 << 1) + 2], abs(d__4)); // ; expr subst  
 smin = max(d__5,d__6);
 /* Computing MAX */
 d__5 = smin, d__6 = (d__1 = tl[tl_dim1 + 1], abs(d__1)), d__5 = max(d__5, d__6), d__6 = (d__2 = tl[(tl_dim1 << 1) + 1], abs(d__2)), d__5 = max(d__5,d__6), d__6 = (d__3 = tl[tl_dim1 + 2], abs(d__3)); d__5 = max(d__5,d__6); d__6 = (d__4 = tl[(tl_dim1 << 1) + 2], abs(d__4)) ; // ; expr subst  
 smin = max(d__5,d__6);
 /* Computing MAX */
 d__1 = eps * smin;
 ...
 *scale = 1.;
 if (smlnum * 8. * abs(btmp[0]) > abs(t16[0]) || smlnum * 8. * abs(btmp[1]) > abs(t16[5]) || smlnum * 8. * abs(btmp[2]) > abs(t16[10]) || smlnum * 8. * abs(btmp[3]) > abs(t16[15])) {
 /* Computing MAX */
 d__1 = abs(btmp[0]), d__2 = abs(btmp[1]), d__1 = max(d__1,d__2), d__2 = abs(btmp[2]); d__1 = max(d__1,d__2); d__2 = abs(btmp[3]); // ; expr subst  
 *scale = .125 / max(d__1,d__2);
 ...
 x[(x_dim1 << 1) + 2] = tmp[3];
 /* Computing MAX */
 d__1 = abs(tmp[0]) + abs(tmp[2]); d__2 = abs(tmp[1]) + abs(tmp[3]); // , expr subst  
 *xnorm = max(d__1,d__2);
 return 0;
 /* End of DLASY2 */
 }
