...
/*! @file libflame_hesv.cc
 *  @brief Test application to validate hesv() using CPP template interface.
 *  */
...
/*! @brief  hesv_test is function template for hesv() functions.
			T can be scomplex, dcomplex
 * @details
 * \b Purpose:
    \verbatim
	  hesv_test is function template for hesv() functions.
	  T can be scomplex, dcomplex
	  
	  hesv_test() function template calls C and CPP based lbrary APIs with
    ...
    Fails the test case if difference > threshold or INFO < 0.
 	  ...
 	  Complex reference:
    http://www.netlib.org/lapack/explore-html/d8/d8e/group__complex_h_esolve_gaf8cd44fa8c6239031e5a6a1c3694a75b.html#gaf8cd44fa8c6239031e5a6a1c3694a75b
    Complex double reference:
    http://www.netlib.org/lapack/explore-html/d3/d9d/group__complex16_h_esolve_gac2def3254215d3a7c56bc162483193d4.html#gac2def3254215d3a7c56bc162483193d4
    \endverbatim
 ...
 * */
template<typename T>
void hesv_test(int ip)
{
  typedef int (*fptr_NL_LAPACK_hesv)(char* uplo, integer* n, integer* nrhs,
  ...
  /* If lwork is -1, then call this API with default work
     ...
     needed.*/
  if (lwork == -1) {
    PRINTF("lwork is -1, so call hesv() to get the array sizes.\n");
    T worksize = {0};
    
    // Call CPP function
    libflame::hesv<T>(&uplo, &n, &nrhs, abuff, &lda, ipivbuff, bbuff, &ldb,
  ...
  if (typeid(T) == typeid(scomplex)) {
    hesv_ref = (fptr_NL_LAPACK_hesv)dlsym(lapackModule, "chesv_");
  } else if (typeid(T) == typeid(dcomplex)) {
    hesv_ref = (fptr_NL_LAPACK_hesv)dlsym(lapackModule, "zhesv_");
  } else {
...
TEST(LAPACKCPP_hesv, CHESV) {
  for (short int index = 0; index < NUM_SUB_TESTS; index++) {
	  PRINTF("index: %d\n", index);
    hesv_test<scomplex> (index);
  }
}

/* Use TEST macro and call C++ test function template with
   dcomplex as typenames.*/
TEST(LAPACKCPP_hesv, ZHESV) {
  for (short int index = 0; index < NUM_SUB_TESTS; index++) {
	  PRINTF("index: %d\n", index);
    hesv_test<dcomplex> (index);
  }
}