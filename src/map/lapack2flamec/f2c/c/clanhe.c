/* ../netlib/clanhe.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
real clanhe_(char *norm, char *uplo, integer *n, complex *a, integer *lda, real *work)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,"clanhe inputs: norm %c, uplo %c, n %lld, lda %lld",*norm, *uplo, *n, *lda);
#else
    snprintf(buffer, 256,"clanhe inputs: norm %c, uplo %c, n %d, lda %d",*norm, *uplo, *n, *lda);
#endif
