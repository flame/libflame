/* ../netlib/v3.9.0/slarrb.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = 0;
 ...
 back = werr[ii];
 L20: negcnt = slaneg_(n, &d__[1], &lld[1], &left, pivmin, &r__);
 if (negcnt > i__ - 1) {
 left -= back;
 back *= 2.f;
 goto L20;
 ...
 back = werr[ii];
 L50: negcnt = slaneg_(n, &d__[1], &lld[1], &right, pivmin, &r__);
 if (negcnt < i__) {
 right += back;
 back *= 2.f;
 goto L50;
 ...
 tmp = max(r__1,r__2);
 /* Computing MAX */
 r__1 = *rtol1 * gap; r__2 = *rtol2 * tmp; // , expr subst  
 cvrgd = max(r__1,r__2);
 ...
 ii = i__ - *offset;
 /* Computing MAX */
 r__1 = 0.f; r__2 = w[ii] - werr[ii] - w[ii - 1] - werr[ii - 1]; // , expr subst  
