/* ../netlib/v3.9.0/dgesvdq.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 doublereal big;
 integer lwrk_dgeqp3__, lwrk_dgelqf__, lwrk_dgeqrf__, lwrk_dgesvd__, lwrk_dormlq__, lwrk_dormqr__, lwrk_dgesvd2__, ierr, lwrk_dormqr2__;
 doublereal rtmp;
 ...
 if (conda) {
 /* Computing MAX */
 i__1 = 1; i__2 = *n + *m - 1 + *n; // , expr subst  
 ...
 *info = -17;
 ...
 lwrk_dgeqp3__ = (integer) rdummy[0];
 if (wntus || wntur) {
 dormqr_("L", "N", m, n, n, &a[a_offset], lda, rdummy, &u[ u_offset], ldu, rdummy, &c_n1, &ierr);
 lwrk_dormqr__ = (integer) rdummy[0];
 }
 else if (wntua) {
 dormqr_("L", "N", m, m, n, &a[a_offset], lda, rdummy, &u[ u_offset], ldu, rdummy, &c_n1, &ierr);
 lwrk_dormqr__ = (integer) rdummy[0];
 }
 else {
 lwrk_dormqr__ = 0;
 ...
 if (lquery) {
 dgesvd_("N", "N", n, n, &a[a_offset], lda, &s[1], &u[u_offset] , ldu, &v[v_offset], ldv, rdummy, &c_n1, &ierr);
 lwrk_dgesvd__ = (integer) rdummy[0];
 if (conda) {
 /* Computing MAX */
 i__1 = *n + lwrk_dgeqp3__; i__2 = *n + lwcon; i__1 = max( i__1,i__2); // ; expr subst  
 ...
 if (lquery) {
 if (rtrans) {
 dgesvd_("N", "O", n, n, &a[a_offset], lda, &s[1], &u[ u_offset], ldu, &v[v_offset], ldv, rdummy, &c_n1, &ierr);
 }
 else {
 dgesvd_("O", "N", n, n, &a[a_offset], lda, &s[1], &u[ u_offset], ldu, &v[v_offset], ldv, rdummy, &c_n1, &ierr);
 }
 lwrk_dgesvd__ = (integer) rdummy[0];
 if (conda) {
 /* Computing MAX */
 i__1 = max(lwrk_dgeqp3__,lwcon); i__1 = max(i__1, lwrk_dgesvd__); // , expr subst  
 ...
 lworq2 = max(*n,1);
 /* Computing MAX */
 i__1 = lwqp3, i__2 = *n / 2 + lwqrf, i__1 = max(i__1,i__2) , i__2 = *n / 2 + lwsvd2, i__1 = max(i__1,i__2); i__2 = *n / 2 + lworq2; i__1 = max(i__1,i__2); // ; expr subst  
 ...
 lworlq = max(*n,1);
 /* Computing MAX */
 i__1 = lwqp3, i__2 = *n / 2 + lwlqf, i__1 = max(i__1,i__2) , i__2 = *n / 2 + lwsvd2, i__1 = max(i__1,i__2); i__2 = *n / 2 + lworlq; i__1 = max(i__1,i__2); // ; expr subst  
 minwrk2 = max(i__1,lworq);
 if (conda) {
 minwrk2 = max(minwrk2,lwcon);
 }
 ...
 if (wntva) {
 i__1 = *n / 2;
 dgeqrf_(n, &i__1, &u[u_offset], ldu, rdummy, rdummy, & c_n1, &ierr);
 lwrk_dgeqrf__ = (integer) rdummy[0];
 i__1 = *n / 2;
 i__2 = *n / 2;
 dgesvd_("S", "O", &i__1, &i__2, &v[v_offset], ldv, &s[ 1], &u[u_offset], ldu, &v[v_offset], ldv, rdummy, &c_n1, &ierr);
 lwrk_dgesvd2__ = (integer) rdummy[0];
 i__1 = *n / 2;
 dormqr_("R", "C", n, n, &i__1, &u[u_offset], ldu, rdummy, &v[v_offset], ldv, rdummy, &c_n1, & ierr);
 lwrk_dormqr2__ = (integer) rdummy[0];
 /* Computing MAX */
 i__1 = lwrk_dgeqp3__, i__2 = *n / 2 + lwrk_dgeqrf__, i__1 = max(i__1,i__2), i__2 = *n / 2 + lwrk_dgesvd2__; i__1 = max(i__1,i__2); i__2 = *n / 2 + lwrk_dormqr2__; // ; expr subst  
 ...
 if (wntva) {
 i__1 = *n / 2;
 dgelqf_(&i__1, n, &u[u_offset], ldu, rdummy, rdummy, & c_n1, &ierr);
 lwrk_dgelqf__ = (integer) rdummy[0];
 i__1 = *n / 2;
 i__2 = *n / 2;
 dgesvd_("S", "O", &i__1, &i__2, &v[v_offset], ldv, &s[ 1], &u[u_offset], ldu, &v[v_offset], ldv, rdummy, &c_n1, &ierr);
 lwrk_dgesvd2__ = (integer) rdummy[0];
 ...
 /* Computing MAX */
 i__1 = lwrk_dgeqp3__, i__2 = *n / 2 + lwrk_dgelqf__, i__1 = max(i__1,i__2), i__2 = *n / 2 + lwrk_dgesvd2__; i__1 = max(i__1,i__2); i__2 = *n / 2 + lwrk_dormlq__; // ; expr subst  
 optwrk2 = max(i__1,i__2);
 if (conda) {
 optwrk2 = max(optwrk2,lwcon);
 }
 ...
 *info = -21;
 ...
 rwork[p] = dlange_("M", &c__1, n, &a[p + a_dim1], lda, rdummy);
 ...
 *numrank = 0;
 dlaset_("G", n, &c__1, &c_b72, &c_b72, &s[1], n);
 if (wntus) {
 dlaset_("G", m, n, &c_b72, &c_b76, &u[u_offset], ldu);
 }
 if (wntua) {
 dlaset_("G", m, m, &c_b72, &c_b76, &u[u_offset], ldu);
 }
 if (wntva) {
 dlaset_("G", n, n, &c_b72, &c_b76, &v[v_offset], ldv);
 }
 if (wntuf) {
 dlaset_("G", n, &c__1, &c_b72, &c_b72, &work[1], n) ;
 dlaset_("G", m, n, &c_b72, &c_b76, &u[u_offset], ldu);
 }
 i__1 = *n;
 for (p = 1;
 ...
 dlascl_("G", &c__0, &c__0, &d__1, &c_b76, m, n, &a[a_offset], lda, &ierr);
 ascaled = TRUE_;
 }
 i__1 = *m - 1;
 dlaswp_(n, &a[a_offset], lda, &c__1, &i__1, &iwork[*n + 1], &c__1);
 }
 ...
 if (! rowprm) {
 rtmp = dlange_("M", m, n, &a[a_offset], lda, rdummy);
 ...
 *info = -8;
 ...
 p <= i__1;
 ++p) {
 if ((d__2 = a[p + p * a_dim1], f2c_dabs(d__2)) < rtmp * (d__1 = a[ a_dim1 + 1], f2c_dabs(d__1))) {
 goto L3002;
 }
 ++nr;
 /* L3001: */
 ...
 p <= i__1;
 ++p) {
 if ((d__2 = a[p + p * a_dim1], f2c_dabs(d__2)) < epsln * (d__1 = a[p - 1 + (p - 1) * a_dim1], f2c_dabs(d__1)) || (d__3 = a[p + p * a_dim1], f2c_dabs(d__3)) < sfmin) {
 goto L3402;
 }
 ++nr;
 /* L3401: */
 ...
 */
 ...
 p <= i__1;
 ++p) {
 if ((d__1 = a[p + p * a_dim1], f2c_dabs(d__1)) == 0.) {
 goto L3502;
 }
 ++nr;
 /* L3501: */
 ...
 p <= i__1;
 ++p) {
 rtmp = dnrm2_(&p, &v[p * v_dim1 + 1], &c__1);
 d__1 = 1. / rtmp;
 dscal_(&p, &d__1, &v[p * v_dim1 + 1], &c__1);
 /* L3053: */
 }
 if (! (lsvec || rsvec)) {
 dpocon_("U", &nr, &v[v_offset], ldv, &c_b76, &rtmp, &work[1], &iwork[*n + iwoff], &ierr);
 }
 else {
 dpocon_("U", &nr, &v[v_offset], ldv, &c_b76, &rtmp, &work[*n + 1], &iwork[*n + iwoff], &ierr);
 }
 sconda = 1. / sqrt(rtmp);
 ...
 dgesvd_("N", "N", n, &nr, &a[a_offset], lda, &s[1], &u[u_offset], ldu, &v[v_offset], ldv, &work[1], lwork, info);
 }
 else {
 ...
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("L", &i__1, &i__2, &c_b72, &c_b72, &a[a_dim1 + 2], lda);
 }
 dgesvd_("N", "N", &nr, n, &a[a_offset], lda, &s[1], &u[u_offset], ldu, &v[v_offset], ldv, &work[1], lwork, info);
 }
 }
 else if (lsvec && ! rsvec) {
 /* ....................................................................... */
 ...
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("U", &i__1, &i__2, &c_b72, &c_b72, &u[(u_dim1 << 1) + 1], ldu);
 }
 /* .. the left singular vectors not computed, the NR right singular */
 /* vectors overwrite [U](1:NR,1:NR) as transposed. These */
 /* will be pre-multiplied by Q to build the left singular vectors of A. */
 i__1 = *lwork - *n;
 dgesvd_("N", "O", n, &nr, &u[u_offset], ldu, &s[1], &u[u_offset], ldu, &u[u_offset], ldu, &work[*n + 1], &i__1, info);
 i__1 = nr;
 for (p = 1;
 p <= i__1;
 ++p) {
 ...
 dlacpy_("U", &nr, n, &a[a_offset], lda, &u[u_offset], ldu);
 if (nr > 1) {
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("L", &i__1, &i__2, &c_b72, &c_b72, &u[u_dim1 + 2], ldu);
 }
 /* .. the right singular vectors not computed, the NR left singular */
 /* vectors overwrite [U](1:NR,1:NR) */
 i__1 = *lwork - *n;
 dgesvd_("O", "N", &nr, n, &u[u_offset], ldu, &s[1], &u[u_offset], ldu, &v[v_offset], ldv, &work[*n + 1], &i__1, info);
 ...
 if (nr < *m && ! wntuf) {
 i__1 = *m - nr;
 dlaset_("A", &i__1, &nr, &c_b72, &c_b72, &u[nr + 1 + u_dim1], ldu);
 if (nr < n1) {
 i__1 = n1 - nr;
 dlaset_("A", &nr, &i__1, &c_b72, &c_b72, &u[(nr + 1) * u_dim1 + 1], ldu);
 i__1 = *m - nr;
 i__2 = n1 - nr;
 dlaset_("A", &i__1, &i__2, &c_b72, &c_b76, &u[nr + 1 + (nr + 1) * u_dim1], ldu);
 }
 }
 ...
 if (! wntuf) {
 i__1 = *lwork - *n;
 dormqr_("L", "N", m, &n1, n, &a[a_offset], lda, &work[1], &u[ u_offset], ldu, &work[*n + 1], &i__1, &ierr);
 }
 if (rowprm && ! wntuf) {
 i__1 = *m - 1;
 dlaswp_(&n1, &u[u_offset], ldu, &c__1, &i__1, &iwork[*n + 1], & c_n1);
 }
 }
 else if (rsvec && ! lsvec) {
 /* ....................................................................... */
 ...
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("U", &i__1, &i__2, &c_b72, &c_b72, &v[(v_dim1 << 1) + 1], ldv);
 }
 /* .. the left singular vectors of R**T overwrite V, the right singular */
 /* vectors not computed */
 if (wntvr || nr == *n) {
 i__1 = *lwork - *n;
 dgesvd_("O", "N", n, &nr, &v[v_offset], ldv, &s[1], &u[ u_offset], ldu, &u[u_offset], ldu, &work[*n + 1], & i__1, info);
 i__1 = nr;
 for (p = 1;
 p <= i__1;
 ++p) {
 ...
 dlapmt_(&c_false, &nr, n, &v[v_offset], ldv, &iwork[1]);
 }
 else {
 ...
 i__1 = *n - nr;
 dlaset_("G", n, &i__1, &c_b72, &c_b72, &v[(nr + 1) * v_dim1 + 1], ldv);
 i__1 = *lwork - *n;
 dgesvd_("O", "N", n, n, &v[v_offset], ldv, &s[1], &u[u_offset] , ldu, &u[u_offset], ldu, &work[*n + 1], &i__1, info);
 i__1 = *n;
 for (p = 1;
 p <= i__1;
 ++p) {
 ...
 dlapmt_(&c_false, n, n, &v[v_offset], ldv, &iwork[1]);
 }
 }
 else {
 /* .. aply DGESVD to R */
 /* .. copy R into V and overwrite V with the right singular vectors */
 dlacpy_("U", &nr, n, &a[a_offset], lda, &v[v_offset], ldv);
 if (nr > 1) {
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("L", &i__1, &i__2, &c_b72, &c_b72, &v[v_dim1 + 2], ldv);
 }
 /* .. the right singular vectors overwrite V, the NR left singular */
 /* vectors stored in U(1:NR,1:NR) */
 if (wntvr || nr == *n) {
 i__1 = *lwork - *n;
 dgesvd_("N", "O", &nr, n, &v[v_offset], ldv, &s[1], &u[ u_offset], ldu, &v[v_offset], ldv, &work[*n + 1], & i__1, info);
 dlapmt_(&c_false, &nr, n, &v[v_offset], ldv, &iwork[1]);
 ...
 /* how to implement this, see the " FULL SVD " branch. */
 i__1 = *n - nr;
 dlaset_("G", &i__1, n, &c_b72, &c_b72, &v[nr + 1 + v_dim1], ldv);
 i__1 = *lwork - *n;
 dgesvd_("N", "O", n, n, &v[v_offset], ldv, &s[1], &u[u_offset] , ldu, &v[v_offset], ldv, &work[*n + 1], &i__1, info);
 dlapmt_(&c_false, n, n, &v[v_offset], ldv, &iwork[1]);
 }
 ...
 i__1 = *lwork - *n;
 dgesvd_("O", "A", n, &nr, &v[v_offset], ldv, &s[1], &v[ v_offset], ldv, &u[u_offset], ldu, &work[*n + 1], & i__1, info);
 /* .. assemble V */
 i__1 = nr;
 for (p = 1;
 p <= i__1;
 ...
 i__1 = *n - nr;
 dlaset_("A", n, &i__1, &c_b72, &c_b72, &v[(nr + 1) * v_dim1 + 1], ldv);
 i__1 = *lwork - *n;
 dgesvd_("O", "A", n, n, &v[v_offset], ldv, &s[1], &v[ v_offset], ldv, &u[u_offset], ldu, &work[*n + 1], &i__1, info);
 i__1 = *n;
 for (p = 1;
 p <= i__1;
 ++p) {
 ...
 if (*n < *m && ! wntuf) {
 i__1 = *m - *n;
 dlaset_("A", &i__1, n, &c_b72, &c_b72, &u[*n + 1 + u_dim1], ldu);
 if (*n < n1) {
 i__1 = n1 - *n;
 dlaset_("A", n, &i__1, &c_b72, &c_b72, &u[(*n + 1) * u_dim1 + 1], ldu);
 i__1 = *m - *n;
 i__2 = n1 - *n;
 dlaset_("A", &i__1, &i__2, &c_b72, &c_b76, &u[*n + 1 + (*n + 1) * u_dim1], ldu);
 }
 }
 }
 else {
 ...
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("U", &i__1, &i__2, &c_b72, &c_b72, &u[(nr + 2) * u_dim1 + 1], ldu);
 }
 i__1 = *lwork - *n - nr;
 dgeqrf_(n, &nr, &u[(nr + 1) * u_dim1 + 1], ldu, &work[*n + 1], &work[*n + nr + 1], &i__1, &ierr);
 i__1 = nr;
 for (p = 1;
 p <= i__1;
 ++p) {
 ...
 dlaset_("U", &i__1, &i__2, &c_b72, &c_b72, &v[(v_dim1 << 1) + 1], ldv);
 i__1 = *lwork - *n - nr;
 dgesvd_("S", "O", &nr, &nr, &v[v_offset], ldv, &s[1], &u[ u_offset], ldu, &v[v_offset], ldv, &work[*n + nr + 1], &i__1, info);
 i__1 = *n - nr;
 dlaset_("A", &i__1, &nr, &c_b72, &c_b72, &v[nr + 1 + v_dim1], ldv);
 i__1 = *n - nr;
 dlaset_("A", &nr, &i__1, &c_b72, &c_b72, &v[(nr + 1) * v_dim1 + 1], ldv);
 i__1 = *n - nr;
 i__2 = *n - nr;
 dlaset_("A", &i__1, &i__2, &c_b72, &c_b76, &v[nr + 1 + ( nr + 1) * v_dim1], ldv);
 i__1 = *lwork - *n - nr;
 dormqr_("R", "C", n, n, &nr, &u[(nr + 1) * u_dim1 + 1], ldu, &work[*n + 1], &v[v_offset], ldv, &work[*n + nr + 1], &i__1, &ierr);
 dlapmt_(&c_false, n, n, &v[v_offset], ldv, &iwork[1]);
 /* .. assemble the left singular vector matrix U of dimensions */
 /* (M x NR) or (M x N) or (M x M). */
 if (nr < *m && ! wntuf) {
 ...
 i__1 = *lwork - *n;
 dgesvd_("S", "O", &nr, n, &v[v_offset], ldv, &s[1], &u[ u_offset], ldu, &v[v_offset], ldv, &work[*n + 1], & i__1, info);
 dlapmt_(&c_false, &nr, n, &v[v_offset], ldv, &iwork[1]);
 /* .. now [V](1:NR,1:N) contains V(1:N,1:NR)**T */
 /* .. assemble the left singular vector matrix U of dimensions */
 /* (M x NR) or (M x N) or (M x M). */
 ...
 /* singular vectors of R stored in [U](1:NR,1:NR) */
 i__1 = *n - nr;
 dlaset_("A", &i__1, n, &c_b72, &c_b72, &v[nr + 1 + v_dim1] , ldv);
 i__1 = *lwork - *n;
 dgesvd_("S", "O", n, n, &v[v_offset], ldv, &s[1], &u[ u_offset], ldu, &v[v_offset], ldv, &work[*n + 1], &i__1, info);
 dlapmt_(&c_false, n, n, &v[v_offset], ldv, &iwork[1]);
 ...
 dlacpy_("U", &nr, n, &a[a_offset], lda, &u[nr + 1 + u_dim1], ldu);
 if (nr > 1) {
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("L", &i__1, &i__2, &c_b72, &c_b72, &u[nr + 2 + u_dim1], ldu);
 }
 i__1 = *lwork - *n - nr;
 dgelqf_(&nr, n, &u[nr + 1 + u_dim1], ldu, &work[*n + 1], & work[*n + nr + 1], &i__1, &ierr);
 dlacpy_("L", &nr, &nr, &u[nr + 1 + u_dim1], ldu, &v[ v_offset], ldv);
 if (nr > 1) {
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("U", &i__1, &i__2, &c_b72, &c_b72, &v[(v_dim1 << 1) + 1], ldv);
 ...
 dlaset_("A", &i__1, &i__2, &c_b72, &c_b76, &v[nr + 1 + ( nr + 1) * v_dim1], ldv);
 i__1 = *lwork - *n - nr;
 dormlq_("R", "N", n, n, &nr, &u[nr + 1 + u_dim1], ldu, & work[*n + 1], &v[v_offset], ldv, &work[*n + nr + 1], &i__1, &ierr);
 dlapmt_(&c_false, n, n, &v[v_offset], ldv, &iwork[1]);
 /* .. assemble the left singular vector matrix U of dimensions */
 /* (M x NR) or (M x N) or (M x M). */
 if (nr < *m && ! wntuf) {
 ...
 if (nr < *n) {
 i__1 = *n - nr;
 dlaset_("G", &i__1, &c__1, &c_b72, &c_b72, &s[nr + 1], n);
 }
 /* .. undo scaling;
 ...
 if (ascaled) {
 d__1 = sqrt((doublereal) (*m));
 dlascl_("G", &c__0, &c__0, &c_b76, &d__1, &nr, &c__1, &s[1], n, &ierr);
 }
 if (conda) {
 rwork[1] = sconda;
 }
