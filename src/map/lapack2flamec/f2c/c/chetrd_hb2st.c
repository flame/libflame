 ...
 */
 ...
 integer grsiz, ttype;
 extern /* Subroutine */
 int chb2st_kernels_(char *, logical *, integer *, integer *, integer *, integer *, integer *, integer *, integer *, complex *, integer *, complex *, complex *, integer *, complex *);
 integer abdpos;
 extern /* Subroutine */
 ...
 *info = -13;
 ...
 abstmp = c_abs(&tmp);
 i__2 = abofdpos + (i__ + 1) * ab_dim1;
 ab[i__2].r = abstmp; ab[i__2].i = 0.f; // , expr subst  
 e[i__] = abstmp;
 if (abstmp != 0.f) {
 q__1.r = tmp.r / abstmp; q__1.i = tmp.i / abstmp; // , expr subst  
 tmp.r = q__1.r; tmp.i = q__1.i; // , expr subst  
 }
 else {
 tmp.r = 1.f; tmp.i = 0.f; // , expr subst  
 }
 if (i__ < *n - 1) {
 i__2 = abofdpos + (i__ + 2) * ab_dim1;
 i__3 = abofdpos + (i__ + 2) * ab_dim1;
 q__1.r = ab[i__3].r * tmp.r - ab[i__3].i * tmp.i; q__1.i = ab[i__3].r * tmp.i + ab[i__3].i * tmp.r; // , expr subst  
 ab[i__2].r = q__1.r; ab[i__2].i = q__1.i; // , expr subst  
 }
 /* IF( WANTZ ) THEN */
