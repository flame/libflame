...
FLA_Error FLA_Scal_external_ts( FLA_cntl_init_s *FLA_cntl_init_i, FLA_Obj alpha, FLA_Obj A )
{
  ...
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
    FLA_Scal_check_ts( FLA_cntl_init_i, alpha, A );

  if ( FLA_Obj_has_zero_dim( A ) ) return FLA_SUCCESS;

  if ( FLA_Obj_equals_ts( FLA_cntl_init_i, alpha, FLA_ONE ) )
  {
    return FLA_SUCCESS;
  }
  else if ( FLA_Obj_equals_ts( FLA_cntl_init_i, alpha, FLA_ZERO ) )
  {
    FLA_Set_ts( (void **) &FLA_cntl_init_i, FLA_ZERO, A );
    return FLA_SUCCESS;
  }
  ...
  if ( FLA_Obj_is_constant_ts( FLA_cntl_init_i, alpha ) )
    dt_alpha = datatype;
  else
    dt_alpha = FLA_Obj_datatype_ts( FLA_cntl_init_i, alpha );
