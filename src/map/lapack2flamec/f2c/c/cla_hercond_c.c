...
real cla_hercond_c_(char *uplo, integer *n, complex *a, integer *lda, complex *af, integer *ldaf, integer *ipiv, real *c__, logical *capply, integer *info, complex *work, real *rwork)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,"cla_hercond_c inputs: uplo %c, n %lld, lda %lld, ldaf %lld",*uplo, *n, *lda, *ldaf);
#else
    snprintf(buffer, 256,"cla_hercond_c inputs: uplo %c, n %d, lda %d, ldaf %d",*uplo, *n, *lda, *ldaf);
#endif
