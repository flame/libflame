/* ../netlib/dgetc2.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int dgetc2_(integer *n, doublereal *a, integer *lda, integer *ipiv, integer *jpiv, integer *info)
{
    ...
    *info = 0;
    ...
    if (*n == 1)
    {
        ipiv[1] = 1;
        jpiv[1] = 1;
        if ((d__1 = a[a_dim1 + 1], f2c_abs(d__1)) < smlnum)
        {
            *info = 1;
            a[a_dim1 + 1] = smlnum;
        }
