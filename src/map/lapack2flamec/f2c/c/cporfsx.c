...
/* ../netlib/cporfsx.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
#else 
 snprintf(buffer, 256,"cporfsx inputs: uplo %c, equed %c, n %d, nrhs %d, lda %d, ldaf %d, ldb %d, ldx %d, n_err_bnds__ %d, nparams %d",*uplo, *equed, *n, *nrhs, *lda, *ldaf, *ldb, *ldx, *n_err_bnds__, *nparams);
#endif
 ...
 *(unsigned char *)norm = 'I';
 ...
 cla_porfsx_extended_(&prec_type__, uplo, n, nrhs, &a[a_offset], lda, &af[af_offset], ldaf, &rcequ, &s[1], &b[b_offset], ldb, &x[ x_offset], ldx, &berr[1], &n_norms__, &err_bnds_norm__[ err_bnds_norm_offset], &err_bnds_comp__[err_bnds_comp_offset], &work[1], &rwork[1], &work[*n + 1], &rwork[1], rcond, & ithresh, &rthresh, &unstable_thresh__, &ignore_cwise__, info);
 }
 /* Computing MAX */
 ...
 if (rcequ) {
 rcond_tmp__ = cla_porcond_c_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &s[1], &c_true, info, &work[1], &rwork[ 1]);
 }
 else {
 rcond_tmp__ = cla_porcond_c_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &s[1], &c_false, info, &work[1], &rwork[ 1]);
 }
 i__1 = *nrhs;
 for (j = 1;
 j <= i__1;
 ...
 *info = *n + j;
 ...
 rcond_tmp__ = cla_porcond_x_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &x[j * x_dim1 + 1], info, &work[1], &rwork[1]);
 }
 else {
 rcond_tmp__ = 0.f;
 }
