...
/*! @file liblame_hetf2.cc
 *  @brief Test application to validate hetf2() using CPP template interface.
 *  */
...
/*! @brief  hetf2_test is function template for hetf2() functions.
			T can be scomplex, dcomplex
 * @details
 * \b Purpose:
    \verbatim
	  hetf2_test is function template for hetf2() functions.
	  T can be scomplex, dcomplex
	  
	  hetf2_test() function template calls C and CPP based lbrary APIs with
    ...
    Fails the test case if difference > threshold or INFO < 0.
 	  ...
 	  Complex reference:
    http://www.netlib.org/lapack/explore-html/d4/d74/group__complex_h_ecomputational_ga3f1790e662e89c7ffc788d4e38b33bea.html#ga3f1790e662e89c7ffc788d4e38b33bea
    Complex double reference:
    http://www.netlib.org/lapack/explore-html/d3/d80/group__complex16_h_ecomputational_ga40a7b760050b65451b24ab9176e93192.html#ga40a7b760050b65451b24ab9176e93192
    \endverbatim
 ...
 * */
template<typename T>
void hetf2_test(int ip)
{
  typedef int (*Fptr_NL_LAPACK_hetf2)(char *uplo, integer *n, T *a,
                  integer *lda, integer *ipiv, integer *info);
  Fptr_NL_LAPACK_hetf2 HETF2;
  ...
  // Call CPP function
  integer info_cpp = -1;
  libflame::hetf2<T>(&uplo, &n, abuff, &lda, ipivbuff, &info_cpp);

  // Call C function
  if (typeid(T) == typeid(scomplex)) {
    HETF2 = (Fptr_NL_LAPACK_hetf2)dlsym(lapackModule, "chetf2_");
  } else if (typeid(T) == typeid(dcomplex)) {
    HETF2 = (Fptr_NL_LAPACK_hetf2)dlsym(lapackModule, "zhetf2_");
  } else {
  ...
  integer info_ref = -1;
  HETF2(&uplo, &n, arefbuff, &lda, ipivrefbuff, &info_ref);
...
TEST(LAPACKCPP_hetf2, CHETF2) {
  for (short int index = 0; index < NUM_SUB_TESTS; index++) {
    PRINTF("index: %d\n", index);
    hetf2_test<scomplex> (index);
  }
}

/* Use TEST macro and call C++ test function template with
   dcomplex, double as typenames.*/
TEST(LAPACKCPP_hetf2, ZHETF2) {
  for (short int index = 0; index < NUM_SUB_TESTS; index++) {
    PRINTF("index: %d\n", index);
    hetf2_test<dcomplex> (index);
  }
}