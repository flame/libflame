/* ../netlib/v3.9.0/zsytri_3x.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
;
 ...
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zsytri_ 3x.f"> */
 /* > [TGZ]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zsytri_ 3x.f"> */
 /* > [ZIP]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zsytri_ 3x.f"> */
 /* > [TXT]</a> */
 /* > \endhtmlonly */
 /* Definition: */
 /* =========== */
 ...
 */
 ...
 /* ===================================================================== */
 /* Subroutine */
 int zsytri_3x_(char *uplo, integer *n, doublecomplex *a, integer *lda, doublecomplex *e, integer *ipiv, doublecomplex *work, integer *nb, integer *info) {
 ...
 *info = 0;
 ...
 akp1.r = z__1.r; akp1.i = z__1.i; // , expr subst  
 z_div(&z__1, &work[k + 1 + work_dim1], &t);
 akkp1.r = z__1.r; akkp1.i = z__1.i; // , expr subst  
 z__3.r = ak.r * akp1.r - ak.i * akp1.i; z__3.i = ak.r * akp1.i + ak.i * akp1.r; // , expr subst  
 ...
 /* U11**T * invD1 * U11 -> U11 */
 i__1 = *n + *nb + 1;
 ztrmm_("L", "U", "T", "U", &nnb, &nnb, &c_b1, &a[cut + 1 + (cut + 1) * a_dim1], lda, &work[u11 + 1 + work_dim1], &i__1);
 i__1 = nnb;
 for (i__ = 1;
 i__ <= i__1;
 ++i__) {
 ...
 i__1 = *n + *nb + 1;
 i__2 = *n + *nb + 1;
 zgemm_("T", "N", &nnb, &nnb, &cut, &c_b1, &a[(cut + 1) * a_dim1 + 1], lda, &work[work_offset], &i__1, &c_b2, &work[u11 + 1 + work_dim1], &i__2);
 ...
 /* U01 = U00**T * invD0 * U01 */
 i__1 = *n + *nb + 1;
 ztrmm_("L", uplo, "T", "U", &cut, &nnb, &c_b1, &a[a_offset], lda, &work[work_offset], &i__1);
 /* Update U01 */
 i__1 = cut;
 for (i__ = 1;
 i__ <= i__1;
 ...
 /* L11**T * invD1 * L11 -> L11 */
 i__1 = *n + *nb + 1;
 ztrmm_("L", uplo, "T", "U", &nnb, &nnb, &c_b1, &a[cut + 1 + (cut + 1) * a_dim1], lda, &work[u11 + 1 + work_dim1], &i__1);
 i__1 = nnb;
 for (i__ = 1;
 i__ <= i__1;
 ++i__) {
 ...
 i__2 = *n + *nb + 1;
 i__3 = *n + *nb + 1;
 zgemm_("T", "N", &nnb, &nnb, &i__1, &c_b1, &a[cut + nnb + 1 + (cut + 1) * a_dim1], lda, &work[work_offset], &i__2, & c_b2, &work[u11 + 1 + work_dim1], &i__3);
 ...
 i__1 = *n - nnb - cut;
 i__2 = *n + *nb + 1;
 ztrmm_("L", uplo, "T", "U", &i__1, &nnb, &c_b1, &a[cut + nnb + 1 + (cut + nnb + 1) * a_dim1], lda, &work[ work_offset], &i__2);
 /* Update L21 */
 i__1 = *n - cut - nnb;
 for (i__ = 1;
 i__ <= i__1;
