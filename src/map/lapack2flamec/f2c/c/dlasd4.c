/* ../netlib/v3.9.0/dlasd4.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = 1;
 ...
 temp = sqrt(eps);
 if (d__[*i__] <= temp * d__[ip1] && (d__1 = z__[*i__], abs(d__1)) <= temp && d__[*i__] > 0.) {
 /* Computing MIN */
 d__1 = d__[*i__] * 10.;
 tau = min(d__1,sgub);
 geomavg = TRUE_;
