/* ../netlib/v3.9.0/dbdsqr.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = 0;
 ...
 ++i__) {
 /* Computing MAX */
 d__2 = smax; d__3 = (d__1 = d__[i__], abs(d__1)); // , expr subst  
 smax = max(d__2,d__3);
 /* L20: */
 }
 i__1 = *n - 1;
 ...
 ++i__) {
 /* Computing MAX */
 d__2 = smax; d__3 = (d__1 = e[i__], abs(d__1)); // , expr subst  
 smax = max(d__2,d__3);
 /* L30: */
 }
 sminl = 0.;
 ...
 /* Computing MAX */
 d__1 = abs(tol) * smax; d__2 = *n * (*n * unfl) * 6; // , expr subst  
 thresh = max(d__1,d__2);
 }
