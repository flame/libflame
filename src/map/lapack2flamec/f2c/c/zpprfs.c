/* ../netlib/zpprfs.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int zpprfs_(char *uplo, integer *n, integer *nrhs, doublecomplex *ap, doublecomplex *afp, doublecomplex *b, integer *ldb, doublecomplex *x, integer *ldx, doublereal *ferr, doublereal *berr, doublecomplex *work, doublereal *rwork, integer *info)
{
    ...
    doublereal lstres;
    extern /* Subroutine */
    int zpptrs_(char *, integer *, integer *, doublecomplex *, doublecomplex *, integer *, integer *);
...
L20: /* Loop until stopping criterion is satisfied. */
        ...
        if (upper)
        {
                    ...
                    ++k)
            {
                ...
                {
                    i__4 = ik;
                    rwork[i__] += ((d__1 = ap[i__4].r, abs(d__1)) + (d__2 = d_imag(&ap[ik]), abs(d__2))) * xk;
                    i__4 = ik;
                    i__5 = i__ + j * x_dim1;
                    s += ((d__1 = ap[i__4].r, abs(d__1)) + (d__2 = d_imag(&ap[ ik]), abs(d__2))) * ((d__3 = x[i__5].r, abs(d__3)) + (d__4 = d_imag(&x[i__ + j * x_dim1]), abs(d__4) ));
                    ++ik;
                    /* L40: */
                }
                i__3 = kk + k - 1;
