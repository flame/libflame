 ...
 *info = istop + 1;
 ...
 /* Computing MAX */
 r__1 = smlnum; r__2 = ulp * (c_abs(&a[istop + istop * a_dim1]) + c_abs(&a[istop - 1 + (istop - 1) * a_dim1])); // , expr subst  
 if (c_abs(&a[istop + (istop - 1) * a_dim1]) <= max(r__1,r__2)) {
 i__2 = istop + (istop - 1) * a_dim1;
 a[i__2].r = 0.f; a[i__2].i = 0.f; // , expr subst  
 --istop;
 ld = 0;
 ...
 /* Computing MAX */
 r__1 = smlnum; r__2 = ulp * (c_abs(&a[istart + istart * a_dim1]) + c_abs(&a[istart + 1 + (istart + 1) * a_dim1])); // , expr subst  
 ...
 --k) {
 /* Computing MAX */
 r__1 = smlnum; r__2 = ulp * (c_abs(&a[k + k * a_dim1]) + c_abs(&a[ k - 1 + (k - 1) * a_dim1])); // , expr subst  
 ...
 /* Computing MAX */
 r__1 = smlnum; r__2 = ulp * tempr; // , expr subst  
 ...
 q__2.r = 1.f / r__1; q__2.i = 0.f / r__1; // , expr subst  
 q__1.r = eshift.r + q__2.r; q__1.i = eshift.i + q__2.i; // , expr subst  
 eshift.r = q__1.r; eshift.i = q__1.i; // , expr subst  
 }
 i__2 = shiftpos;
 alpha[i__2].r = 1.f; alpha[i__2].i = 0.f; // , expr subst  
 i__2 = shiftpos;
 ...
 L80: chgeqz_(wants, wantq, wantz, n, ilo, ihi, &a[a_offset], lda, &b[b_offset], ldb, &alpha[1], &beta[1], &q[q_offset], ldq, &z__[z_offset], ldz, &work[1], lwork, &rwork[1], &norm_info__);
 *info = norm_info__;
 return 0;
 }
 /* claqz0_ */
