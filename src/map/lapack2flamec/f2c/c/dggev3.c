/* ../netlib/v3.9.0/dggev3.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 /* > \brief <b> DGGEV3 computes the eigenvalues and, optionally, the left and/or right eigenvectors for GE mat rices (blocked algorithm)</b> */
 /* =========== DOCUMENTATION =========== */
 ...
 */
 ...
 /* ===================================================================== */
 /* Subroutine */
 int dggev3_(char *jobvl, char *jobvr, integer *n, doublereal *a, integer *lda, doublereal *b, integer *ldb, doublereal *alphar, doublereal *alphai, doublereal *beta, doublereal *vl, integer *ldvl, doublereal *vr, integer *ldvr, doublereal *work, integer *lwork, integer *info) {
 AOCL_DTL_TRACE_ENTRY(AOCL_DTL_LEVEL_TRACE_5);
#if AOCL_DTL_LOG_ENABLE 
 char buffer[256]; 
 snprintf(buffer, 256,"dggev3 inputs: jobvl %c, jobvr %c, n %" FLA_IS ", lda %" FLA_IS ", ldb %" FLA_IS ", ldvl %" FLA_IS ", ldvr %" FLA_IS ", lwork %" FLA_IS "",*jobvl, *jobvr, *n, *lda, *ldb, *ldvl, *ldvr, *lwork);
 ...
 *info = -16;
 ...
 /* Computing MAX */
 i__1 = 1, i__2 = *n << 3; i__1 = max(i__1,i__2); i__2 = *n * 3 + ( integer) work[1]; // ; expr subst  
 ...
 if (ilv) {
 dgghd3_(jobvl, jobvr, n, &c__1, n, &a[a_offset], lda, &b[b_offset] , ldb, &vl[vl_offset], ldvl, &vr[vr_offset], ldvr, &work[ 1], &c_n1, &ierr);
 /* Computing MAX */
 i__1 = lwkopt; i__2 = *n * 3 + (integer) work[1]; // , expr subst  
 lwkopt = max(i__1,i__2);
 ...
 dgghd3_("N", "N", n, &c__1, n, &a[a_offset], lda, &b[b_offset], ldb, &vl[vl_offset], ldvl, &vr[vr_offset], ldvr, &work[1], &c_n1, &ierr);
 /* Computing MAX */
 i__1 = lwkopt; i__2 = *n * 3 + (integer) work[1]; // , expr subst  
 lwkopt = max(i__1,i__2);
 ...
 i__1 = *lwork + 1 - iwrk;
 dgghd3_(jobvl, jobvr, n, &ilo, &ihi, &a[a_offset], lda, &b[b_offset], ldb, &vl[vl_offset], ldvl, &vr[vr_offset], ldvr, &work[iwrk], &i__1, &ierr);
 }
 else {
 i__1 = *lwork + 1 - iwrk;
 dgghd3_("N", "N", &irows, &c__1, &irows, &a[ilo + ilo * a_dim1], lda, &b[ilo + ilo * b_dim1], ldb, &vl[vl_offset], ldvl, &vr[ vr_offset], ldvr, &work[iwrk], &i__1, &ierr);
 }
 ...
 *info = *n + 2;
 ...
 ++jr) {
 /* Computing MAX */
 d__2 = temp; d__3 = (d__1 = vl[jr + jc * vl_dim1], f2c_dabs(d__1)); // , expr subst  
 temp = max(d__2,d__3);
 /* L10: */
 }
 }
 ...
 ++jr) {
 /* Computing MAX */
 d__3 = temp; d__4 = (d__1 = vl[jr + jc * vl_dim1], f2c_dabs(d__1)) + (d__2 = vl[jr + (jc + 1) * vl_dim1], f2c_dabs(d__2)); // , expr subst  
 temp = max(d__3,d__4);
 /* L20: */
 }
 }
 ...
 ++jr) {
 /* Computing MAX */
 d__2 = temp; d__3 = (d__1 = vr[jr + jc * vr_dim1], f2c_dabs(d__1)); // , expr subst  
 temp = max(d__2,d__3);
 /* L60: */
 }
 }
 ...
 ++jr) {
 /* Computing MAX */
 d__3 = temp; d__4 = (d__1 = vr[jr + jc * vr_dim1], f2c_dabs(d__1)) + (d__2 = vr[jr + (jc + 1) * vr_dim1], f2c_dabs(d__2)); // , expr subst  
 temp = max(d__3,d__4);
 /* L70: */
 }
 }
