/* ../netlib/zhptrs.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int zhptrs_(char *uplo, integer *n, integer *nrhs, doublecomplex *ap, integer *ipiv, doublecomplex *b, integer *ldb, integer *info)
{
    ...
    char buffer[256];
    snprintf(buffer, 256,"zhptrs inputs: uplo %c, n %" FLA_IS ", nrhs %" FLA_IS ", ldb %" FLA_IS ", ipiv %" FLA_IS "",*uplo, *n, *nrhs, *ldb, *ipiv);
...
L10: /* If K < 1, exit from loop. */
        ...
        else
        {
            ...
            z__1.r = -1.;
            z__1.i = -0.; // , expr subst
            zgeru_(&i__1, nrhs, &z__1, &ap[kc - (k - 1)], &c__1, &b[k - 1 + b_dim1], ldb, &b[b_dim1 + 1], ldb);
            /* Multiply by the inverse of the diagonal block. */
            i__1 = kc + k - 2;
            akm1k.r = ap[i__1].r;
...
L40: /* If K > N, exit from loop. */
        ...
        if (ipiv[k] > 0)
        {
            ...
            if (k > 1)
            {
                ...
                z__1.r = -1.;
                z__1.i = -0.; // , expr subst
                zgemv_("Conjugate transpose", &i__1, nrhs, &z__1, &b[b_offset], ldb, &ap[kc], &c__1, &c_b1, &b[k + b_dim1], ldb);
                zlacgv_(nrhs, &b[k + b_dim1], ldb);
            }
            /* Interchange rows K and IPIV(K). */
            kp = ipiv[k];
        ...
        else
        {
            ...
            if (k > 1)
            {
                ...
                z__1.r = -1.;
                z__1.i = -0.; // , expr subst
                zgemv_("Conjugate transpose", &i__1, nrhs, &z__1, &b[b_offset], ldb, &ap[kc + k], &c__1, &c_b1, &b[k + 1 + b_dim1], ldb);
                zlacgv_(nrhs, &b[k + 1 + b_dim1], ldb);
            }
            /* Interchange rows K and -IPIV(K). */
            kp = -ipiv[k];
...
L60: /* If K > N, exit from loop. */
        ...
        if (ipiv[k] > 0)
        {
            ...
            if (k < *n)
            {
                ...
                z__1.r = -1.;
                z__1.i = -0.; // , expr subst
                zgeru_(&i__1, nrhs, &z__1, &ap[kc + 1], &c__1, &b[k + b_dim1], ldb, &b[k + 1 + b_dim1], ldb);
            }
            /* Multiply by the inverse of the diagonal block. */
            i__1 = kc;
            s = 1. / ap[i__1].r;
        ...
        else
        {
            ...
            if (k < *n - 1)
            {
                ...
                z__1.r = -1.;
                z__1.i = -0.; // , expr subst
                zgeru_(&i__1, nrhs, &z__1, &ap[kc + 2], &c__1, &b[k + b_dim1], ldb, &b[k + 2 + b_dim1], ldb);
                i__1 = *n - k - 1;
                z__1.r = -1.;
                z__1.i = -0.; // , expr subst
                zgeru_(&i__1, nrhs, &z__1, &ap[kc + *n - k + 2], &c__1, &b[k + 1 + b_dim1], ldb, &b[k + 2 + b_dim1], ldb);
            }
            /* Multiply by the inverse of the diagonal block. */
            i__1 = kc + 1;
            akm1k.r = ap[i__1].r;
...
L90: /* If K < 1, exit from loop. */
        ...
        if (ipiv[k] > 0)
        {
            ...
            if (k < *n)
            {
                ...
                z__1.r = -1.;
                z__1.i = -0.; // , expr subst
                zgemv_("Conjugate transpose", &i__1, nrhs, &z__1, &b[k + 1 + b_dim1], ldb, &ap[kc + 1], &c__1, &c_b1, &b[k + b_dim1], ldb);
                zlacgv_(nrhs, &b[k + b_dim1], ldb);
            }
            /* Interchange rows K and IPIV(K). */
            kp = ipiv[k];
        ...
        else
        {
            ...
            if (k < *n)
            {
                ...
                z__1.r = -1.;
                z__1.i = -0.; // , expr subst
                zgemv_("Conjugate transpose", &i__1, nrhs, &z__1, &b[k + 1 + b_dim1], ldb, &ap[kc - (*n - k)], &c__1, &c_b1, &b[k - 1 + b_dim1], ldb);
                zlacgv_(nrhs, &b[k - 1 + b_dim1], ldb);
            }
            /* Interchange rows K and -IPIV(K). */
            kp = -ipiv[k];
