/* ../netlib/v3.9.0/cstemr.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 extern /* Subroutine */
 int slarrc_(char *, integer *, real *, real *, real *, real *, real *, integer *, integer *, integer *, integer * ), clarrv_(integer *, real *, real *, real *, real *, real *, integer *, integer *, integer *, integer *, real *, real * , real *, real *, real *, real *, integer *, integer *, real *, complex *, integer *, integer *, real *, integer *, integer *), slarre_(char *, integer *, real *, real *, integer *, integer *, real *, real *, real *, real *, real *, real *, integer *, integer *, integer *, real *, real *, real *, integer *, integer * , real *, real *, real *, integer *, integer *);
 integer iinspl, indwrk, ifirst, liwmin, nzcmin;
 real pivmin, thresh;
 extern real slanst_(char *, integer *, real *, real *);
 extern /* Subroutine */
 int slarrj_(integer *, real *, real *, integer *, integer *, real *, integer *, real *, real *, real *, integer *, real *, real *, integer *);
 integer nsplit;
 extern /* Subroutine */
 int slarrr_(integer *, real *, real *, integer *);
 real smlnum;
 extern /* Subroutine */
 ...
 *info = -19;
 ...
 if (zquery && *info == 0) {
 i__1 = z_dim1 + 1;
 z__[i__1].r = (real) nzcmin; z__[i__1].i = 0.f; // , expr subst  
 }
 ...
 *m = 1;
 ...
 i__1 = *m * z_dim1 + 1;
 r__1 = -sn;
 z__[i__1].r = r__1; z__[i__1].i = 0.f; // , expr subst  
 i__1 = *m * z_dim1 + 2;
 z__[i__1].r = cs; z__[i__1].i = 0.f; // , expr subst  
 ...
 z__[i__1].r = cs; z__[i__1].i = 0.f; // , expr subst  
 i__1 = *m * z_dim1 + 2;
 z__[i__1].r = sn; z__[i__1].i = 0.f; // , expr subst  
 /* Note: At most one of SN and CS can be zero. */
 if (sn != 0.f) {
 if (cs != 0.f) {
 isuppz[(*m << 1) - 1] = 1;
 ...
 *tryrac = FALSE_;
 ...
 /* Computing MAX */
 r__1 = sqrt(eps) * .05f; r__2 = eps * 4.f; // , expr subst  
 rtol1 = max(r__1,r__2);
 /* Computing MAX */
 r__1 = sqrt(eps) * .005f; r__2 = eps * 4.f; // , expr subst  
 rtol2 = max(r__1,r__2);
 }
 ...
 *info = abs(iinfo) + 20;
 ...
 rtol2 = eps * 4.f;
 slarrj_(&in, &work[indd + ibegin - 1], &work[inde2 + ibegin - 1], &ifirst, &ilast, &rtol2, &offset, &w[wbegin], & work[inderr + wbegin - 1], &work[indwrk], &iwork[ iindwk], &pivmin, &tnrm, &iinfo);
 ibegin = iend + 1;
 wbegin = wend + 1;
 L39: ;
 }
