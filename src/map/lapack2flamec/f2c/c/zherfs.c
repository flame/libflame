/* ../netlib/zherfs.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
L20: /* Loop until stopping criterion is satisfied. */
        ...
        z__1.r = -1.;
        z__1.i = -0.; // , expr subst
        zhemv_(uplo, n, &z__1, &a[a_offset], lda, &x[j * x_dim1 + 1], &c__1, & c_b1, &work[1], &c__1);
        ...
        if (berr[j] > eps && berr[j] * 2. <= lstres && count <= 5)
        {
            ...
            zhetrs_(uplo, n, &c__1, &af[af_offset], ldaf, &ipiv[1], &work[1], n, info);
