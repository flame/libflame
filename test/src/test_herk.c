...
// Static variables.
static char* op_str                   = "Hermitian rank-k update";
static char* flash_front_str          = "FLASH_Herk";
static char* fla_front_str            = "FLA_Herk";
...
static fla_herk_t*      herk_cntl_unb;
static fla_herk_t*      herk_cntl_blk;
static fla_blocksize_t* herk_cntl_bsize;

// Local prototypes.
void libfla_test_herk_experiment( test_params_t params,
                                  unsigned int  var,
                                  char*         sc_str,
                                  ...
                                  double*       perf,
                                  double*       residual );
void libfla_test_herk_impl( int         impl,
                            FLA_Uplo    uplo,
                            FLA_Trans   trans,
                            FLA_Obj     alpha,
                            FLA_Obj     A,
                            FLA_Obj     beta,
                            FLA_Obj     C );
void libfla_test_herk_cntl_create( unsigned int var,
                                   dim_t        b_alg_flat );
void libfla_test_herk_cntl_free( void );


void libfla_test_herk( FILE* output_stream, test_params_t params, test_op_t op )
{
	...
	if ( op.flash_front == ENABLE )
	{
		libfla_test_op_driver( flash_front_str, NULL,
		                       ...
		                       params, thresh, libfla_test_herk_experiment );
	}
                                  ...
                                  double*       residual )
{
	...
	if ( impl == FLA_TEST_FLAT_UNB_VAR ||
		...
		libfla_test_herk_cntl_create( var, b_alg_flat );
	...
	*perf = ( 1 * m * m * k ) / time_min / FLOPS_PER_UNIT_PERF;
	...
	else
	{
		...
		FLA_Gemv_external( FLA_CONJ_TRANSPOSE, alpha,   A, w, FLA_ZERO, z );
	}
                                   ...
                                   dim_t        b_alg_flat )
{
	...
	herk_cntl_bsize = FLA_Blocksize_create( b_alg_flat,
	                                        b_alg_flat,
	                                        b_alg_flat,
	                                        b_alg_flat );

	herk_cntl_unb   = FLA_Cntl_herk_obj_create( FLA_FLAT,
	                                            var_unb,
	                                            NULL,
	                                            NULL,
	                                            NULL,
	...
	herk_cntl_blk   = FLA_Cntl_herk_obj_create( FLA_FLAT,
	                                            var_blk,
	                                            herk_cntl_bsize,
	                                            fla_scalr_cntl_blas,
	                                            fla_herk_cntl_blas,
...
void libfla_test_herk_cntl_free( void )
{
	FLA_Blocksize_free( herk_cntl_bsize );

	FLA_Cntl_obj_free( herk_cntl_unb );
	FLA_Cntl_obj_free( herk_cntl_blk );
}
                            ...
                            FLA_Obj   C )
{
	switch ( impl )
	{
		...
		FLASH_Herk( uplo, trans, alpha, A, beta, C );
		break;
		...
		FLA_Herk_internal( uplo, trans, alpha, A, beta, C, herk_cntl_unb );
		break;

		case FLA_TEST_FLAT_BLK_VAR:
		FLA_Herk_internal( uplo, trans, alpha, A, beta, C, herk_cntl_blk );
		break;
