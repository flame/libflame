/* ../netlib/dlasd4.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 on Linux or Unix systems, link with .../path/to/libf2c.a -lm or, if you install libf2c.a in a standard place, with -lf2c -lm -- in that order, at the end of the command line, as in cc *.o -lf2c -lm Source for libf2c is in /netlib/f2c/libf2c.zip, e.g., http://www.netlib.org/f2c/libf2c.zip */
#include "FLA_f2c.h" /* > \brief \b DLASD4 computes the square root of the i-th updated eigenvalue of a positive symmetric rank-one modification to a positive diagonal matrix. Used by dbdsdc. */
/* =========== DOCUMENTATION =========== */
    ...
    *info = 0;
    ...
    else
    {
        ...
        if (w > 0.)
        {
            ...
            if (a > 0.)
            {
                tau2 = b * 2. / (a + sqrt((d__1 = a * a - b * 4. * c__, f2c_dabs( d__1))));
            }
            else
            {
                tau2 = (a - sqrt((d__1 = a * a - b * 4. * c__, f2c_dabs(d__1)))) / (c__ * 2.);
            }
            ...
            temp = sqrt(eps);
            if (d__[*i__] <= temp * d__[ip1] && (d__1 = z__[*i__], f2c_dabs(d__1)) <= temp && d__[*i__] > 0.)
            {
                /* Computing MIN */
                d__1 = d__[*i__] * 10.;
                tau = min(d__1,sgub);
        ...
        else
        {
            ...
            /* following, however, is the corresponding estimation of */
            /* SIGMA - D( IP1 ). */
            tau = tau2 / (d__[ip1] + sqrt((d__1 = d__[ip1] * d__[ip1] + tau2, f2c_dabs(d__1))));
        }
        *sigma = d__[ii] + tau;
        i__1 = *n;
        for (j = 1;
