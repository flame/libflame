/* ../netlib/zlals0.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 /* > \brief \b ZLALS0 applies back multiplying factors in solving the least squares problem using divide and c onquer SVD approach. Used by sgelsd. */
 /* =========== DOCUMENTATION =========== */
 ...
 */
 ...
 char buffer[256];
 snprintf(buffer, 256,"zlals0 inputs: icompq %" FLA_IS ", nl %" FLA_IS ", nr %" FLA_IS ", sqre %" FLA_IS ", nrhs %" FLA_IS ", ldb %" FLA_IS ", ldbx %" FLA_IS ", perm %" FLA_IS ", givptr %" FLA_IS ", givcol %" FLA_IS ", ldgcol %" FLA_IS ", ldgnum %" FLA_IS ", k %" FLA_IS "",*icompq, *nl, *nr, *sqre, *nrhs, *ldb, *ldbx, *perm, *givptr, *givcol, *ldgcol, *ldgnum, *k);
 ...
 *info = -20;
 ...
 i__ <= i__1;
 ++i__) {
 zdrot_(nrhs, &b[givcol[i__ + (givcol_dim1 << 1)] + b_dim1], ldb, & b[givcol[i__ + givcol_dim1] + b_dim1], ldb, &givnum[i__ + (givnum_dim1 << 1)], &givnum[i__ + givnum_dim1]);
 /* L10: */
 }
 ...
 rwork[j] = -poles[j + (poles_dim1 << 1)] * z__[j] / diflj / (poles[j + (poles_dim1 << 1)] + dj);
 }
 i__2 = j - 1;
 for (i__ = 1;
 i__ <= i__2;
 ...
 rwork[i__] = poles[i__ + (poles_dim1 << 1)] * z__[i__] / (dlamc3_(&poles[i__ + (poles_dim1 << 1)], & dsigj) - diflj) / (poles[i__ + (poles_dim1 << 1)] + dj);
 }
 /* L30: */
 }
 i__2 = *k;
 ...
 rwork[i__] = poles[i__ + (poles_dim1 << 1)] * z__[i__] / (dlamc3_(&poles[i__ + (poles_dim1 << 1)], & dsigjp) + difrj) / (poles[i__ + (poles_dim1 << 1)] + dj);
 }
 /* L40: */
 }
 rwork[1] = -1.;
 ...
 dgemv_("T", k, nrhs, &c_b13, &rwork[*k + 1 + (*nrhs << 1)], k, &rwork[1], &c__1, &c_b15, &rwork[*k + 1], &c__1);
 i__ = *k + (*nrhs << 1);
 i__2 = *nrhs;
 for (jcol = 1;
 jcol <= i__2;
 ...
 dgemv_("T", k, nrhs, &c_b13, &rwork[*k + 1 + (*nrhs << 1)], k, &rwork[1], &c__1, &c_b15, &rwork[*k + 1 + *nrhs], & c__1);
 i__2 = *nrhs;
 for (jcol = 1;
 jcol <= i__2;
 ++jcol) {
 ...
 i__4 = jcol + *k;
 i__5 = jcol + *k + *nrhs;
 z__1.r = rwork[i__4]; z__1.i = rwork[i__5]; // , expr subst  
 b[i__3].r = z__1.r; b[i__3].i = z__1.i; // , expr subst  
 /* L90: */
 }
 ...
 rwork[j] = -z__[j] / difl[j] / (dsigj + poles[j + poles_dim1]) / difr[j + (difr_dim1 << 1)];
 }
 i__2 = j - 1;
 for (i__ = 1;
 i__ <= i__2;
 ...
 rwork[i__] = z__[j] / (dlamc3_(&dsigj, &d__1) - difr[ i__ + difr_dim1]) / (dsigj + poles[i__ + poles_dim1]) / difr[i__ + (difr_dim1 << 1)];
 }
 /* L110: */
 }
 i__2 = *k;
 ...
 rwork[i__] = z__[j] / (dlamc3_(&dsigj, &d__1) - difl[ i__]) / (dsigj + poles[i__ + poles_dim1]) / difr[i__ + (difr_dim1 << 1)];
 }
 /* L120: */
 }
 /* Since B and BX are complex, the following call to DGEMV */
 ...
 i__5 = jcol + *k + *nrhs;
 z__1.r = rwork[i__4]; z__1.i = rwork[i__5]; // , expr subst  
 bx[i__3].r = z__1.r; bx[i__3].i = z__1.i; // , expr subst  
 /* L170: */
 }
 /* L180: */
 }
 ...
 zdrot_(nrhs, &b[givcol[i__ + (givcol_dim1 << 1)] + b_dim1], ldb, & b[givcol[i__ + givcol_dim1] + b_dim1], ldb, &givnum[i__ + (givnum_dim1 << 1)], &d__1);
 /* L200: */
 }
 }
 AOCL_DTL_TRACE_EXIT(AOCL_DTL_LEVEL_TRACE_5);
