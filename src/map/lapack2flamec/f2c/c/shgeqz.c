/* ../netlib/v3.9.0/shgeqz.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = *n + 1;
 ...
 /* and BETA */
 L80: if (t[ilast + ilast * t_dim1] < 0.f) {
 if (ilschr) {
 i__2 = ilast;
 for (j = ifrstm;
 j <= i__2;
 ...
 /* Computing MAX */
 /* Computing MAX */
 r__3 = 1.f, r__4 = abs(wr); r__3 = max(r__3,r__4); r__4 = abs(wi); // ; expr subst  
 r__1 = s1; r__2 = safmin * max(r__3,r__4); // , expr subst  
 temp = max(r__1,r__2);
 if (wi != 0.f) {
 goto L200;
 }
 ...
 if (abs(wr) > temp) {
 /* Computing MIN */
 r__1 = scale; r__2 = temp / abs(wr); // , expr subst  
 scale = min(r__1,r__2);
 }
 s1 = scale * s1;
 wr = scale * wr;
 ...
 ilpivt = FALSE_;
 /* Computing MAX */
 r__3 = (r__1 = t[j + 1 + (j + 1) * t_dim1], abs(r__1)); r__4 = (r__2 = t[j + 1 + (j + 2) * t_dim1], abs(r__2)); // , expr subst  
 temp = max(r__3,r__4);
 /* Computing MAX */
 r__3 = (r__1 = t[j + 2 + (j + 1) * t_dim1], abs(r__1)); r__4 = (r__2 = t[j + 2 + (j + 2) * t_dim1], abs(r__2)); // , expr subst  
 temp2 = max(r__3,r__4);
 ...
 if (abs(w11) < abs(u1)) {
 /* Computing MIN */
 r__2 = scale; r__3 = (r__1 = w11 / u1, abs(r__1)); // , expr subst  
 scale = min(r__2,r__3);
 }
 /* Solve */
 u2 = scale * u2 / w22;
