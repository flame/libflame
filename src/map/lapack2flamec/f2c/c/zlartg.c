...
int zlartg_(doublecomplex *f, doublecomplex *g, doublereal * cs, doublecomplex *sn, doublecomplex *r__)
{
    ...
    /* Computing MAX */
    /* Computing MAX */
    d__7 = (d__1 = f->r, abs(d__1));
    d__8 = (d__2 = d_imag(f), abs(d__2)); // , expr subst
    /* Computing MAX */
    d__9 = (d__3 = g->r, abs(d__3));
    d__10 = (d__4 = d_imag(g), abs(d__4)); // , expr subst
    d__5 = max(d__7,d__8);
...
L20:
    ...
    if (f2 <= max(g2,1.) * safmin)
    {
        ...
        *cs = f2s / g2s;
        ...
        /* Computing MAX */
        d__3 = (d__1 = f->r, abs(d__1));
        d__4 = (d__2 = d_imag(f), abs(d__2)); // , expr subst
        if (max(d__3,d__4) > 1.)
        {
            d__1 = f->r;
            d__2 = d_imag(f);
