/* ../netlib/v3.9.0/sbdsqr.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 extern /* Subroutine */
 int srot_(integer *, real *, integer *, real *, integer *, real *, real *), slas2_(real *, real *, real *, real *, real *);
 ...
 logical lower;
 extern /* Subroutine */
 int sswap_(integer *, real *, integer *, real *, integer *), slasq1_(integer *, real *, real *, real *, integer *), slasv2_(real *, real *, real *, real *, real *, real *, real *, real *, real *);
 ...
 *info = 0;
 ...
 /* Computing MIN */
 d__1 = (doublereal) eps;
 r__3 = 100.f; r__4 = pow_dd(&d__1, &c_b15); // , expr subst  
 ...
 ++i__) {
 /* Computing MAX */
 r__2 = smax; r__3 = (r__1 = d__[i__], abs(r__1)); // , expr subst  
 smax = max(r__2,r__3);
 /* L20: */
 }
 i__1 = *n - 1;
 ...
 L50: sminoa /= sqrt((real) (*n));
 /* Computing MAX */
 r__1 = tol * sminoa; r__2 = *n * (*n * unfl) * 6; // , expr subst  
 thresh = max(r__1,r__2);
 }
 else {
 /* Absolute accuracy desired */
 /* Computing MAX */
 r__1 = abs(tol) * smax; r__2 = *n * (*n * unfl) * 6; // , expr subst  
 thresh = max(r__1,r__2);
 }
 ...
 if ((r__2 = e[m - 1], abs(r__2)) <= abs(tol) * (r__1 = d__[m], abs( r__1)) || tol < 0.f && (r__3 = e[m - 1], abs(r__3)) <= thresh) {
 e[m - 1] = 0.f;
 goto L60;
 }
 if (tol >= 0.f) {
 ...
 mu = (r__2 = d__[lll + 1], abs(r__2)) * (mu / (mu + (r__1 = e[ lll], abs(r__1))));
 sminl = min(sminl,mu);
 /* L100: */
 }
 }
 ...
 if ((r__2 = e[ll], abs(r__2)) <= abs(tol) * (r__1 = d__[ll], abs(r__1) ) || tol < 0.f && (r__3 = e[ll], abs(r__3)) <= thresh) {
 e[ll] = 0.f;
 goto L60;
 }
 if (tol >= 0.f) {
 ...
 mu = (r__2 = d__[lll], abs(r__2)) * (mu / (mu + (r__1 = e[lll] , abs(r__1))));
 sminl = min(sminl,mu);
 /* L110: */
 }
 }
 ...
 /* Computing MAX */
 r__1 = eps; r__2 = tol * .01f; // , expr subst  
 ...
 /* Chase bulge from top to bottom */
 /* Save cosines and sines for later singular vector updates */
 f = ((r__1 = d__[ll], abs(r__1)) - shift) * (r_sign(&c_b49, &d__[ ll]) + shift / d__[ll]);
 g = e[ll];
 i__1 = m - 1;
 for (i__ = ll;
 i__ <= i__1;
 ...
 /* Chase bulge from bottom to top */
 /* Save cosines and sines for later singular vector updates */
 f = ((r__1 = d__[m], abs(r__1)) - shift) * (r_sign(&c_b49, &d__[m] ) + shift / d__[m]);
 g = e[m - 1];
 i__1 = ll + 1;
 for (i__ = m;
 i__ >= i__1;
