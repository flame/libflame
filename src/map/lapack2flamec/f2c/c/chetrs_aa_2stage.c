...
int chetrs_aa_2stage_(char *uplo, integer *n, integer *nrhs, complex *a, integer *lda, complex *tb, integer *ltb, integer *ipiv, integer *ipiv2, complex *b, integer *ldb, integer *info)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,"chetrs_aa_2stage inputs: uplo %c, n %lld, nrhs %lld, lda %lld, ltb %lld, ldb %lld",*uplo, *n, *nrhs, *lda, *ltb, *ldb);
#else
    snprintf(buffer, 256,"chetrs_aa_2stage inputs: uplo %c, n %d, nrhs %d, lda %d, ltb %d, ldb %d",*uplo, *n, *nrhs, *lda, *ltb, *ldb);
#endif
