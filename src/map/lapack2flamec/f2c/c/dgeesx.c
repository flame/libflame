/* ../netlib/dgeesx.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 char buffer[256]; 
 snprintf(buffer, 256,"dgeesx inputs: jobvs %c, sort %c, sense %c, n %" FLA_IS ", lda %" FLA_IS ", ldvs %" FLA_IS ", lwork %" FLA_IS "",*jobvs, *sort, *sense, *n, *lda, *ldvs, *lwork);
 ...
 *info = -12;
 ...
 if (! wantvs) {
 /* Computing MAX */
 i__1 = maxwrk; i__2 = *n + hswork; // , expr subst  
 ...
 if (! wantsn) {
 /* Computing MAX */
 i__1 = lwrk; i__2 = *n + *n * *n / 2; // , expr subst  
 lwrk = max(i__1,i__2);
 }
 if (wantsv || wantsb) {
 liwrk = *n * *n / 4;
 ...
 *info = ieval;
 ...
 /* otherwise, need 0 ) */
 i__1 = *lwork - iwrk + 1;
 dtrsen_(sense, jobvs, &bwork[1], n, &a[a_offset], lda, &vs[vs_offset], ldvs, &wr[1], &wi[1], sdim, rconde, rcondv, &work[iwrk], & i__1, &iwork[1], liwork, &icond);
 if (! wantsn) {
 /* Computing MAX */
 i__1 = maxwrk; i__2 = *n + (*sdim << 1) * (*n - *sdim); // , expr subst  
 maxwrk = max(i__1,i__2);
 }
 if (icond == -15) {
 ...
 *info = *n + 2;
 ...
 if (wantsv || wantsb) {
 /* Computing MAX */
 i__1 = 1; i__2 = *sdim * (*n - *sdim); // , expr subst  
