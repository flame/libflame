/* ../netlib/cla_herpvgrw.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 char buffer[256]; 
#if FLA_ENABLE_ILP64 
 snprintf(buffer, 256,"cla_herpvgrw inputs: uplo %c, n %lld, lda %lld, ldaf %lld, ipiv %lld",*uplo, *n, *lda, *ldaf, *ipiv);
#else 
 snprintf(buffer, 256,"cla_herpvgrw inputs: uplo %c, n %d, lda %d, ldaf %d, ipiv %d",*uplo, *n, *lda, *ldaf, *ipiv);
#endif
