/* ../netlib/v3.9.0/dlarrk.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = 0;
 ...
 eps = dlamch_("P");
 /* Computing MAX */
 d__1 = abs(*gl); d__2 = abs(*gu); // , expr subst  
 tnorm = max(d__1,d__2);
 rtoli = *reltol;
 atoli = *pivmin * 4.;
 ...
 *info = -1;
 ...
 /* Computing MAX */
 d__1 = abs(right); d__2 = abs(left); // , expr subst  
 tmp2 = max(d__1,d__2);
 /* Computing MAX */
