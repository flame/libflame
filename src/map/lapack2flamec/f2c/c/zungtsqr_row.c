/* zungtsqr_row.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
;
 ...
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zunrgts qr_row.f"> */
 /* > [TGZ]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zunrgts qr_row.f"> */
 /* > [ZIP]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zunrgts qr_row.f"> */
 /* > [TXT]</a> */
 /* > \endhtmlonly */
 /* Definition: */
 /* =========== */
 ...
 *info = -10;
 ...
 knb = min(i__4,i__5);
 i__4 = *n - kb + 1;
 zlarfb_gett_("I", &imb, &i__4, &knb, &t[(jb_t__ + kb - 1) * t_dim1 + 1], ldt, &a[kb + kb * a_dim1], lda, &a[ib + kb * a_dim1], lda, &work[1], &knb);
 }
 }
 }
 ...
 i__1 = *n - kb + 1;
 zlarfb_gett_("N", &c__0, &i__1, &knb, &t[kb * t_dim1 + 1], ldt, & a[kb + kb * a_dim1], lda, dummy, &c__1, &work[1], &knb);
 }
 else {
 i__1 = mb1 - kb - knb + 1;
 i__3 = *n - kb + 1;
 zlarfb_gett_("N", &i__1, &i__3, &knb, &t[kb * t_dim1 + 1], ldt, & a[kb + kb * a_dim1], lda, &a[kb + knb + kb * a_dim1], lda, &work[1], &knb);
 }
 }
 z__1.r = (doublereal) lworkopt; z__1.i = 0.; // , expr subst  
 work[1].r = z__1.r; work[1].i = z__1.i; // , expr subst  
