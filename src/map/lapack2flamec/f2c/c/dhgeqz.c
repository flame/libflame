 ...
 */
 ...
 char buffer[256]; 
 snprintf(buffer, 256,"dhgeqz inputs: job %c, compq %c, compz %c, n %" FLA_IS ", ilo %" FLA_IS ", ihi %" FLA_IS ", ldh %" FLA_IS ", ldt %" FLA_IS ", ldq %" FLA_IS ", ldz %" FLA_IS ", lwork %" FLA_IS "",*job, *compq, *compz, *n, *ilo, *ihi, *ldh, *ldt, *ldq, *ldz, *lwork);
 ...
 */
 ...
 /* Computing MAX */
 d__4 = safmin; d__5 = ulp * ((d__1 = h__[ilast + ilast * h_dim1], f2c_abs(d__1)) + (d__2 = h__[ilast - 1 + (ilast - 1) * h_dim1] , f2c_abs(d__2))); // , expr subst  
 ...
 /* Computing MAX */
 d__4 = safmin; d__5 = ulp * ((d__1 = t[ilast - 1 + ilast * t_dim1], f2c_abs(d__1)) + (d__2 = t[ilast - 1 + (ilast - 1) * t_dim1], f2c_abs( d__2))); // , expr subst  
 ...
 /* Computing MAX */
 d__4 = safmin; d__5 = ulp * ((d__1 = h__[j + j * h_dim1], f2c_abs( d__1)) + (d__2 = h__[j - 1 + (j - 1) * h_dim1], f2c_abs( d__2))); // , expr subst  
 ...
 /* Test 2: for T(j,j)=0 */
 temp = (d__1 = t[j + (j + 1) * t_dim1], f2c_abs(d__1));
 if (j > *ilo) {
 temp += (d__1 = t[j - 1 + j * t_dim1], f2c_abs(d__1));
 }
 /* Computing MAX */
 d__2 = safmin; d__3 = ulp * temp; // , expr subst  
 ...
 *info = *n + 1;
 ...
 /* 1x1 block. */
 L70: temp = h__[ilast + ilast * h_dim1];
 ...
 /* Computing MAX */
 /* Computing MAX */
 d__3 = 1., d__4 = f2c_abs(wr); d__3 = max(d__3,d__4); d__4 = f2c_abs(wi); // ; expr subst  
 d__1 = s1; d__2 = safmin * max(d__3,d__4); // , expr subst  
 temp = max(d__1,d__2);
 if (wi != 0.) {
 goto L200;
 }
 ...
 if (f2c_abs(wr) > temp) {
 /* Computing MIN */
 d__1 = scale; d__2 = temp / f2c_abs(wr); // , expr subst  
 scale = min(d__1,d__2);
 }
 s1 = scale * s1;
 wr = scale * wr;
 ...
 ilpivt = FALSE_;
 /* Computing MAX */
 d__3 = (d__1 = t[j + 1 + (j + 1) * t_dim1], f2c_abs(d__1)); d__4 = (d__2 = t[j + 1 + (j + 2) * t_dim1], f2c_abs(d__2)); // , expr subst  
 temp = max(d__3,d__4);
 /* Computing MAX */
 d__3 = (d__1 = t[j + 2 + (j + 1) * t_dim1], f2c_abs(d__1)); d__4 = (d__2 = t[j + 2 + (j + 2) * t_dim1], f2c_abs(d__2)); // , expr subst  
 temp2 = max(d__3,d__4);
 ...
 /* Computing MIN */
 d__2 = scale; d__3 = (d__1 = w11 / u1, f2c_abs(d__1)); // , expr subst  
 scale = min(d__2,d__3);
 }
 /* Solve */
 u2 = scale * u2 / w22;
