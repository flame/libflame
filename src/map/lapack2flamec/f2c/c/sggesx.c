/* ../netlib/v3.9.0/sggesx.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -18;
 ...
 if (*n > 0) {
 /* Computing MAX */
 i__1 = *n << 3; i__2 = *n * 6 + 16; // , expr subst  
 minwrk = max(i__1,i__2);
 maxwrk = minwrk - *n + *n * ilaenv_(&c__1, "SGEQRF", " ", n, & c__1, n, &c__0);
 /* Computing MAX */
 i__1 = maxwrk; i__2 = minwrk - *n + *n * ilaenv_(&c__1, "SORMQR", " ", n, &c__1, n, &c_n1); // , expr subst  
 maxwrk = max(i__1,i__2);
 if (ilvsl) {
 /* Computing MAX */
 i__1 = maxwrk; i__2 = minwrk - *n + *n * ilaenv_(&c__1, "SOR" "GQR", " ", n, &c__1, n, &c_n1); // , expr subst  
 maxwrk = max(i__1,i__2);
 }
 lwrk = maxwrk;
 if (ijob >= 1) {
 /* Computing MAX */
 i__1 = lwrk; i__2 = *n * *n / 2; // , expr subst  
 lwrk = max(i__1,i__2);
 }
 }
 else {
 ...
 *info = *n + 1;
 ...
 if (ijob >= 1) {
 /* Computing MAX */
 i__1 = maxwrk; i__2 = (*sdim << 1) * (*n - *sdim); // , expr subst  
 maxwrk = max(i__1,i__2);
 }
 if (ierr == -22) {
