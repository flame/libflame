      ...
      {        
         ...
         for( ni = 0; ni <= n; ni++ )
         {
            ...
            for( i = 0; i < n_repeats; i++ )
            {
               ...
               time = FLA_Clock();
               sspffrtx_(ad, &n, &ni, NULL, NULL);
               time = FLA_Clock() - time;
               time_min = min( time_min, time );

               /* Calculate error norm */
               snrm = xdiff_norm_s( ad, fod, n, ni );
            ...
            libfla_test_print_result_info("SPFFRTX",
                                          (char *) 's',
                                          "c",
                                          n,
                                          perf,
      ...
      {        
         ...
         for( ni = 0; ni <= n; ni++ )
         {
            ...
            for( i = 0; i < n_repeats; i++ )
            {
               ...
               time = FLA_Clock();
               dspffrtx_(ad, &n, &ni, NULL, NULL);
               time = FLA_Clock() - time;
               time_min = min( time_min, time );

               /* Calculate error norm */
               dnrm = xdiff_norm_d( ad, fod, n, ni );
      ...
      {        
         ...
         for( ni = 0; ni <= n; ni++ )
         {
            ...
            for( i = 0; i < n_repeats; i++ )
            {
               ...
               time = FLA_Clock();
               cspffrtx_(ad, &n, &ni, NULL, NULL);
               time = FLA_Clock() - time;
               time_min = min( time_min, time );

               /* Calculate error norm */
               snrm = xdiff_norm_c( ad, fod, n, ni );
      ...
      {        
         ...
         for( ni = 0; ni <= n; ni++ )
         {
            ...
            for( i = 0; i < n_repeats; i++ )
            {
                ...
                time = FLA_Clock();
                zspffrtx_(ad, &n, &ni, NULL, NULL);
                time = FLA_Clock() - time;
                time_min = min( time_min, time );

               /* Calculate error norm */
               dnrm = xdiff_norm_z( ad, fod, n, ni );
...
void libfla_test_ldltx_nopiv_ps( test_params_t *params, test_op_t op )
{
   int i;
   ...
   if ( op.fla_blk_ext == ENABLE )
   {
      for( i = 0; i < params->n_datatypes; ++i )
      {
         ...
         {
            case FLA_FLOAT:
               test_ldltx_nopiv_ps_s( params );
               break;
            case FLA_DOUBLE:
               test_ldltx_nopiv_ps_d( params );
               break;
            case FLA_COMPLEX:
               test_ldltx_nopiv_ps_c( params );
               break;
            case FLA_DOUBLE_COMPLEX:
               test_ldltx_nopiv_ps_z( params );
               break;
            default:
