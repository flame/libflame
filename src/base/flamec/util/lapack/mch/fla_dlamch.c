...
/*            ( A guess; no known machine ) */
		iwarn = TRUE_;
		 ...
		 {
	    if (gpmin - fla_min(ngpmin,ngnmin) == 3) {
		lemin = fla_max(ngpmin,ngnmin) - 1 + lt;
...
/* Computing MIN */
	    i__1 = fla_min(ngpmin,ngnmin), i__1 = fla_min(i__1,gpmin);
	    lemin = fla_min(i__1,gnmin);
/*         ( A guess; no known machine ) */
	    iwarn = TRUE_;
	}
	first = FALSE_;
