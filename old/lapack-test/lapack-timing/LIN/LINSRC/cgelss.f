...
*     .. External Subroutines ..
      EXTERNAL           CSRSCL, CBDSQR, CCOPY,
     $                   CGEBRD, CGELQF, CGEMM, CGEMV,
     $                   CGEQRF, CLACPY, CLASCL, CLASET,
     $                   CUNGBR, CUNMBR, CUNMLQ, CUNMQR,
     $                   SLABAD, SLASCL, SLASET, XERBLA
*     ..
*     .. External Functions ..
      INTEGER            ILAENV
      REAL               CLANGE, SECOND, SLAMCH, SOPBL2,
     $                   SOPBL3, SOPLA, SOPLA2
      EXTERNAL           CLANGE, SECOND, SLAMCH, SOPBL2,
...
*
         OPCNT( GELSS ) = OPCNT( GELSS ) + REAL( 6*M*N )
...
*        Scale matrix norm up to SMLNUM
*
         OPCNT( GELSS ) = OPCNT( GELSS ) + REAL( 6*M*NRHS )
...
*        (RWorkspace: none)
*
         NB = ILAENV( 1, 'CUNGBR', 'P', N, N, N, -1 )
         OPCNT( UNGBR ) = OPCNT( UNGBR ) +
     $                    SOPLA2( 'CUNGBR', 'P', N, N, N, 0, NB )
         T1 = SECOND( )
...
*
         ...
         DO 10 I = 1, N
            IF( S( I ).GT.THR ) THEN
               OPCNT( GELSS ) = OPCNT( GELSS ) + REAL( 6*NRHS + 3 )
...
*        (RWorkspace: none)
*
         NB = ILAENV( 1, 'CUNGBR', 'P', M, M, M, -1 )
         OPCNT( UNGBR ) = OPCNT( UNGBR ) +
     $                    SOPLA2( 'CUNGBR', 'P', M, M, M, 0, NB )
         T1 = SECOND( )
...
*        (RWorkspace: none)
*
         NB = ILAENV( 1, 'CUNGBR', 'P', M, N, M, -1 )
         OPCNT( UNGBR ) = OPCNT( UNGBR ) +
     $                    SOPLA2( 'CUNGBR', 'P', M, N, M, 0, NB )
         T1 = SECOND( )
...
*
      IF( IASCL.EQ.1 ) THEN
         OPCNT( GELSS ) = OPCNT( GELSS ) + REAL( 6*( N*NRHS + MINMN ) )
     ...
     $                INFO )
      END IF
      IF( IBSCL.EQ.1 ) THEN
         OPCNT( GELSS ) = OPCNT( GELSS ) + REAL( 6*N*NRHS )
