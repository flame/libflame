/* sgeqr2.f -- translated by f2c (version 20000121). You must link the resulting object file with the libraries: -lf2c -lm (in that order) */
 #include "FLA_f2c.h" /* Table of constant values */
 static integer c__1 = 1;
 /* > \brief \b SGEQR2 computes the QR factorization of a general rectangular matrix using an unblocked algorit hm. */
 /* =========== DOCUMENTATION =========== */
 ...
 */
 ...
 /* ===================================================================== */
 /* Subroutine */
 int sgeqr2_fla(integer *m, integer *n, real *a, integer *lda, real *tau, real *work, integer *info) {
 ...
 *info = -4;
 ...
 /* Computing MIN */
 i__3 = i__ + 1;
 slarfg_(&i__2, &a[i__ + i__ * a_dim1], &a[fla_min(i__3,*m) + i__ * a_dim1] , &c__1, &tau[i__]);
 if (i__ < *n) {
 ...
 i__2 = *m - i__ + 1;
 i__3 = *n - i__;
 slarf_("Left", &i__2, &i__3, &a[i__ + i__ * a_dim1], &c__1, &tau[ i__], &a[i__ + (i__ + 1) * a_dim1], lda, &work[1]);
