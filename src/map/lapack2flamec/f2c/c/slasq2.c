/* ../netlib/v3.9.0/slasq2.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 on Linux or Unix systems, link with .../path/to/libf2c.a -lm or, if you install libf2c.a in a standard place, with -lf2c -lm -- in that order, at the end of the command line, as in cc *.o -lf2c -lm Source for libf2c is in /netlib/f2c/libf2c.zip, e.g., http://www.netlib.org/f2c/libf2c.zip */
 #include "FLA_f2c.h" /* Table of constant values */
 static integer c__1 = 1;
 static integer c__2 = 2;
 /* > \brief \b SLASQ2 computes all the eigenvalues of the symmetric positive definite tridiagonal matrix assoc iated with the qd Array Z to high relative accuracy. Used by sbdsqr and sstegr. */
 /* =========== DOCUMENTATION =========== */
 ...
 *info = -(k + 201);
 ...
 e += z__[k + 1];
 /* Computing MAX */
 r__1 = qmax; r__2 = z__[k]; // , expr subst  
 qmax = max(r__1,r__2);
 /* Computing MIN */
 r__1 = emin; r__2 = z__[k + 1]; // , expr subst  
 emin = min(r__1,r__2);
 /* Computing MAX */
 r__1 = max(qmax,zmax); r__2 = z__[k + 1]; // , expr subst  
 zmax = max(r__1,r__2);
 /* L10: */
 }
 if (z__[(*n << 1) - 1] < 0.f) {
 *info = -((*n << 1) + 199);
 ...
 d__ += z__[(*n << 1) - 1];
 /* Computing MAX */
 r__1 = qmax; r__2 = z__[(*n << 1) - 1]; // , expr subst  
 qmax = max(r__1,r__2);
 zmax = max(qmax,zmax);
 ...
 /* Computing MIN */
 r__1 = emin; r__2 = z__[i4 - (pp << 1)]; // , expr subst  
 emin = min(r__1,r__2);
 /* L60: */
 }
 z__[(n0 << 2) - pp - 2] = d__;
 ...
 i4 += 4) {
 /* Computing MAX */
 r__1 = qmax; r__2 = z__[i4]; // , expr subst  
 qmax = max(r__1,r__2);
 /* L70: */
 }
 ...
 *info = 1;
 ...
 if (qmin >= emax * 4.f) {
 /* Computing MIN */
 r__1 = qmin; r__2 = z__[i4 - 3]; // , expr subst  
 qmin = min(r__1,r__2);
 /* Computing MAX */
 r__1 = emax; r__2 = z__[i4 - 5]; // , expr subst  
 emax = max(r__1,r__2);
 }
 /* Computing MAX */
 r__1 = qmax; r__2 = z__[i4 - 7] + z__[i4 - 5]; // , expr subst  
 qmax = max(r__1,r__2);
 /* Computing MIN */
 r__1 = emin; r__2 = z__[i4 - 5]; // , expr subst  
 emin = min(r__1,r__2);
 /* L90: */
 }
 i4 = 4;
 ...
 if (kmin - i0 << 1 < n0 - kmin && deemin <= z__[(n0 << 2) - 3] * .5f) {
 ipn4 = i0 + n0 << 2;
 pp = 2;
 i__2 = i0 + n0 - 1 << 1;
 for (i4 = i0 << 2;
 ...
 /* Computing MAX */
 r__1 = 0.f; r__2 = qmin - sqrt(qmin) * 2.f * sqrt(emax); // , expr subst  
 ...
 slasq3_(&i0, &n0, &z__[1], &pp, &dmin__, &sigma, &desig, &qmax, & nfail, &iter, &ndiv, &ieee, &ttype, &dmin1, &dmin2, &dn, & dn1, &dn2, &g, &tau);
 pp = 1 - pp;
 ...
 /* Computing MAX */
 r__1 = qmax; r__2 = z__[i4 + 1]; // , expr subst  
 qmax = max(r__1,r__2);
 /* Computing MIN */
 r__1 = emin; r__2 = z__[i4 - 1]; // , expr subst  
 emin = min(r__1,r__2);
 /* Computing MIN */
 r__1 = oldemn; r__2 = z__[i4]; // , expr subst  
