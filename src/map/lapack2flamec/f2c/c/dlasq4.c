/* ../netlib/v3.9.0/dlasq4.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *ttype = -1;
 ...
 if (gap1 > 0. && gap1 > b1) {
 /* Computing MAX */
 d__1 = *dn - b1 / gap1 * b1; d__2 = *dmin__ * .5; // , expr subst  
 s = max(d__1,d__2);
 *ttype = -2;
 }
 else {
 ...
 if (a2 > b1 + b2) {
 /* Computing MIN */
 d__1 = s; d__2 = a2 - (b1 + b2); // , expr subst  
 s = min(d__1,d__2);
 }
 /* Computing MAX */
 d__1 = s; d__2 = *dmin__ * .333; // , expr subst  
 s = max(d__1,d__2);
 *ttype = -3;
 }
 }
 ...
 *ttype = -7;
 ...
 /* Computing MAX */
 d__1 = s; d__2 = a2 * (1. - a2 * 1.01 * (b2 / gap2) * b2); // , expr subst  
 s = max(d__1,d__2);
 }
 else {
 /* Computing MAX */
 d__1 = s; d__2 = a2 * (1. - b2 * 1.01); // , expr subst  
 s = max(d__1,d__2);
 *ttype = -8;
 }
 }
