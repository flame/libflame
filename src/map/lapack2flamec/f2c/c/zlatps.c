...
, and we */
 ...
 char buffer[256]; 
 snprintf(buffer, 256,"zlatps inputs: uplo %c, trans %c, diag %c, normin %c, n %d",*uplo, *trans, *diag, *normin, *n);
...
. */
 ...
 i__3 = ip;
 tjjs.r = ap[i__3].r; tjjs.i = ap[i__3].i; // , expr subst  
 ...
 *scale = bignum * .5 / xmax;
 ...
 if (nounit) {
 i__3 = ip;
 z__1.r = tscal * ap[i__3].r; z__1.i = tscal * ap[i__3].i; // , expr subst  
 ...
 *scale *= rec;
 ...
 ++i__) {
 i__4 = ip - j + i__;
 z__3.r = ap[i__4].r * uscal.r - ap[i__4].i * uscal.i; z__3.i = ap[i__4].r * uscal.i + ap[i__4].i * uscal.r; // , expr subst  
 i__5 = i__;
