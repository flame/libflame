/* ../netlib/dlasy2.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
L10:
    ...
    *scale = 1.;
    gam = (d__1 = b[b_dim1 + 1], f2c_dabs(d__1));
    if (smlnum * gam > bet)
    {
        *scale = 1. / gam;
    }
...
L20: /* Computing MAX */
    /* Computing MAX */
    d__7 = (d__1 = tl[tl_dim1 + 1], f2c_dabs(d__1)), d__8 = (d__2 = tr[tr_dim1 + 1] , f2c_dabs(d__2)), d__7 = max(d__7,d__8), d__8 = (d__3 = tr[(tr_dim1 << 1) + 1], f2c_dabs(d__3)), d__7 = max(d__7,d__8), d__8 = (d__4 = tr[ tr_dim1 + 2], f2c_dabs(d__4));
    d__7 = max(d__7,d__8);
    d__8 = (d__5 = tr[(tr_dim1 << 1) + 2], f2c_dabs(d__5)); // ; expr subst
...
L30: /* Computing MAX */
    /* Computing MAX */
    d__7 = (d__1 = tr[tr_dim1 + 1], f2c_dabs(d__1)), d__8 = (d__2 = tl[tl_dim1 + 1] , f2c_dabs(d__2)), d__7 = max(d__7,d__8), d__8 = (d__3 = tl[(tl_dim1 << 1) + 1], f2c_dabs(d__3)), d__7 = max(d__7,d__8), d__8 = (d__4 = tl[ tl_dim1 + 2], f2c_dabs(d__4));
    d__7 = max(d__7,d__8);
    d__8 = (d__5 = tl[(tl_dim1 << 1) + 2], f2c_dabs(d__5)); // ; expr subst
    d__6 = eps * max(d__7,d__8);
    smin = max(d__6,smlnum);
    tmp[0] = tl[tl_dim1 + 1] + sgn * tr[tr_dim1 + 1];
...
L40: /* Solve 2 by 2 system using complete pivoting. */
    ...
    *scale = 1.;
    if (smlnum * 2. * f2c_dabs(btmp[1]) > f2c_dabs(u22) || smlnum * 2. * f2c_dabs(btmp[0]) > f2c_dabs(u11))
    {
        /* Computing MAX */
        d__1 = f2c_dabs(btmp[0]);
        d__2 = f2c_dabs(btmp[1]); // , expr subst
    ...
    else
    {
        x[x_dim1 + 2] = x2[1];
        /* Computing MAX */
        d__3 = (d__1 = x[x_dim1 + 1], f2c_dabs(d__1));
        d__4 = (d__2 = x[x_dim1 + 2] , f2c_dabs(d__2)); // , expr subst
        *xnorm = max(d__3,d__4);
    }
    return 0;
    /* 2 by 2: */
    ...
    /* Set pivots less than SMIN to SMIN. */
L50: /* Computing MAX */
    d__5 = (d__1 = tr[tr_dim1 + 1], f2c_dabs(d__1)), d__6 = (d__2 = tr[(tr_dim1 << 1) + 1], f2c_dabs(d__2)), d__5 = max(d__5,d__6), d__6 = (d__3 = tr[ tr_dim1 + 2], f2c_dabs(d__3));
    d__5 = max(d__5,d__6);
    d__6 = (d__4 = tr[(tr_dim1 << 1) + 2], f2c_dabs(d__4)); // ; expr subst
    smin = max(d__5,d__6);
    /* Computing MAX */
    d__5 = smin, d__6 = (d__1 = tl[tl_dim1 + 1], f2c_dabs(d__1)), d__5 = max(d__5, d__6), d__6 = (d__2 = tl[(tl_dim1 << 1) + 1], f2c_dabs(d__2)), d__5 = max(d__5,d__6), d__6 = (d__3 = tl[tl_dim1 + 2], f2c_dabs(d__3));
    d__5 = max(d__5,d__6);
    d__6 = (d__4 = tl[(tl_dim1 << 1) + 2], f2c_dabs(d__4)) ; // ; expr subst
    smin = max(d__5,d__6);
    /* Computing MAX */
    d__1 = eps * smin;
            ...
            ++i__)
    {
                ...
                ++ip)
        {
            for (jp = i__;
                    ...
                    ++jp)
            {
                if ((d__1 = t16[ip + (jp << 2) - 5], f2c_dabs(d__1)) >= xmax)
                {
                    xmax = (d__1 = t16[ip + (jp << 2) - 5], f2c_dabs(d__1));
                    ipsv = ip;
                    jpsv = jp;
                }
                /* L60: */
        ...
        jpiv[i__ - 1] = jpsv;
        if ((d__1 = t16[i__ + (i__ << 2) - 5], f2c_dabs(d__1)) < smin)
        {
            *info = 1;
    ...
    if (f2c_dabs(t16[15]) < smin)
    {
        t16[15] = smin;
    }
    *scale = 1.;
    if (smlnum * 8. * f2c_dabs(btmp[0]) > f2c_dabs(t16[0]) || smlnum * 8. * f2c_dabs(btmp[1]) > f2c_dabs(t16[5]) || smlnum * 8. * f2c_dabs(btmp[2]) > f2c_dabs(t16[10]) || smlnum * 8. * f2c_dabs(btmp[3]) > f2c_dabs(t16[15]))
    {
        /* Computing MAX */
        d__1 = f2c_dabs(btmp[0]), d__2 = f2c_dabs(btmp[1]), d__1 = max(d__1,d__2), d__2 = f2c_dabs(btmp[2]);
        d__1 = max(d__1,d__2);
        d__2 = f2c_dabs(btmp[3]); // ; expr subst
    ...
    x[(x_dim1 << 1) + 2] = tmp[3];
    /* Computing MAX */
    d__1 = f2c_dabs(tmp[0]) + f2c_dabs(tmp[2]);
    d__2 = f2c_dabs(tmp[1]) + f2c_dabs(tmp[3]); // , expr subst
    *xnorm = max(d__1,d__2);
    return 0;
    /* End of DLASY2 */
}
