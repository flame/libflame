...
int chetrf_(char *uplo, integer *n, complex *a, integer *lda, integer *ipiv, complex *work, integer *lwork, integer *info)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,"chetrf inputs: uplo %c, n %lld, lda %lld, lwork %lld",*uplo, *n, *lda, *lwork);
#else
    snprintf(buffer, 256,"chetrf inputs: uplo %c, n %d, lda %d, lwork %d",*uplo, *n, *lda, *lwork);
#endif
