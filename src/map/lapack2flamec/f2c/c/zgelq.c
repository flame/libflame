/* ../netlib/v3.9.0/zgelq.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 integer mintsz;
 extern /* Subroutine */
 int zlaswlq_(integer *, integer *, integer *, integer *, doublecomplex *, integer *, doublecomplex *, integer *, doublecomplex *, integer *, integer *);
 ...
 *info = -8;
 ...
 if (*info == 0) {
 if (mint) {
 t[1].r = (doublereal) mintsz; t[1].i = 0.; // , expr subst  
 }
 else {
 i__1 = mb * *m * nblcks + 5;
 t[1].r = (doublereal) i__1; t[1].i = 0.; // , expr subst  
 }
 t[2].r = (doublereal) mb; t[2].i = 0.; // , expr subst  
 t[3].r = (doublereal) nb; t[3].i = 0.; // , expr subst  
 if (minw) {
 i__1 = max(1,*n);
