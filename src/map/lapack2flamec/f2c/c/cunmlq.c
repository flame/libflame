/* ../netlib/cunmlq.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
/* ===================================================================== */
/* Subroutine */
int cunmlq_(char *side, char *trans, integer *m, integer *n, integer *k, complex *a, integer *lda, complex *tau, complex *c__, integer *ldc, complex *work, integer *lwork, integer *info)
{
    AOCL_DTL_TRACE_ENTRY(AOCL_DTL_LEVEL_TRACE_5);
#if LF_AOCL_DTL_LOG_ENABLE
    char buffer[256];
    snprintf(buffer, 256, "cunmlq inputs: side %c, trans %c, m %" FLA_IS ", n %" FLA_IS ", k %" FLA_IS ", lda %" FLA_IS ", ldc %" FLA_IS ", lwork %" FLA_IS "", *side, *trans, *m, *n, *k, *lda, *ldc, *lwork);
    AOCL_DTL_LOG(AOCL_DTL_LEVEL_TRACE_5, buffer);
#endif
    extern int fla_cunmlq(char *side, char *trans, integer *m, integer *n, integer *k, complex *a, integer *lda, complex *tau, complex *c__, integer *ldc, complex *work, integer *lwork, integer *info);

    int ret_val = fla_cunmlq(side, trans, m, n, k, a, lda, tau, c__, ldc, work, lwork, info);
    AOCL_DTL_TRACE_EXIT(AOCL_DTL_LEVEL_TRACE_5);
    return ret_val;
}

int fla_cunmlq(char *side, char *trans, integer *m, integer *n, integer *k, complex *a, integer *lda, complex *tau, complex *c__, integer *ldc, complex *work, integer *lwork, integer *info)
{
    ...
    *info = 0;
    ...
    {
        /* Use unblocked code */
        cunml2_(side, trans, m, n, k, &a[a_offset], lda, &tau[1], &c__[ c_offset], ldc, &work[1], &iinfo);
    }
    else
    {
        /* Use blocked code */
...
#else
        {
                    ...
                    i__ += i__2)
            {
...
#endif
                {
                    ...
                    i__4 = nq - i__ + 1;
                    clarft_("Forward", "Rowwise", &i__4, &ib, &a[i__ + i__ * a_dim1], lda, &tau[i__], &work[iwt], &c__65);
                }
...
#ifdef FLA_OPENMP_MULTITHREADING
                if(left)
                    clarfb_(side, transt, "Forward", "Rowwise", &mi_sub, &ni_sub, &ib, &a[i__ + i__ * a_dim1], lda, &work[iwt], &c__65, &c__[ic + (index + jc) * c_dim1], ldc, &work[1 + index], &ldwork);
                else
                    clarfb_(side, transt, "Forward", "Rowwise", &mi_sub, &ni_sub, &ib, &a[i__ + i__ * a_dim1], lda, &work[iwt], &c__65, &c__[index + ic + jc * c_dim1], ldc, &work[1 + index], &ldwork);
                #pragma omp barrier
#else
                clarfb_(side, transt, "Forward", "Rowwise", &mi, &ni, &ib, &a[i__ + i__ * a_dim1], lda, &work[iwt], &c__65, &c__[ic + jc * c_dim1], ldc, &work[1], &ldwork);
#endif
                /* L10: */
            }
        }
