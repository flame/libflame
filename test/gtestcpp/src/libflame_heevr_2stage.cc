...
void heevr_2stage_test(int ip)
{
  ...
  Fptr_NL_LAPACK_heevr_2stage HEEVR_2STAGE;
  ...
  if ((lrwork != -1) && (lrwork < max(1, 24*n))) {
    PRINTF("lrwork should be >= max(1, 24*n). Please correct the input" \
          " data.\n");
  }
  ...
  if (typeid(T) == typeid(scomplex)) {
    HEEVR_2STAGE = (Fptr_NL_LAPACK_heevr_2stage)dlsym(lapackModule, \
                        "cheevr_2stage_");
  ...
  integer info_ref = -1;
  HEEVR_2STAGE(&jobz, &range, &uplo, &n, arefbuff, &lda, &vl, &vu, &il,
