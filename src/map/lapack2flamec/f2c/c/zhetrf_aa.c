/* ../netlib/v3.9.0/zhetrf_aa.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
;
 ...
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zhetrf_ aa.f"> */
 /* > [TGZ]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zhetrf_ aa.f"> */
 /* > [ZIP]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zhetrf_ aa.f"> */
 /* > [TXT]</a> */
 /* > \endhtmlonly */
 /* Definition: */
 /* =========== */
 ...
 */
 ...
 char buffer[256];
 snprintf(buffer, 256,"zhetrf_aa inputs: uplo %c, n %" FLA_IS ", lda %" FLA_IS ", lwork %" FLA_IS "",*uplo, *n, *lda, *lwork);
 ...
 */
 ...
 i__1 = 2 - k1;
 i__2 = *n - j;
 zlahef_aa_(uplo, &i__1, &i__2, &jb, &a[max(1,j) + (j + 1) * a_dim1], lda, &ipiv[j + 1], &work[1], n, &work[*n * nb + 1]) ;
 ...
 i__1 = j + (j + 1) * a_dim1;
 a[i__1].r = 1.; a[i__1].i = 0.; // , expr subst  
 i__1 = *n - j;
 ...
 zgemm_("Conjugate transpose", "Transpose", &c__1, &mj, &i__3, &z__1, &a[j1 - k2 + j3 * a_dim1], lda, &work[j3 - j1 + 1 + k1 * *n], n, &c_b2, &a[ j3 + j3 * a_dim1], lda) ;
 ++j3;
 }
 ...
 i__4 = jb + 1;
 z__1.r = -1.; z__1.i = -0.; // , expr subst  
 zgemm_("Conjugate transpose", "Transpose", &nj, &i__3, & i__4, &z__1, &a[j1 - k2 + j2 * a_dim1], lda, & work[j3 - j1 + 1 + k1 * *n], n, &c_b2, &a[j2 + j3 * a_dim1], lda);
 }
 /* Recover T( J, J+1 ) */
 i__2 = j + (j + 1) * a_dim1;
 d_cnjg(&z__1, &alpha);
 ...
 */
 ...
 i__2 = 2 - k1;
 i__1 = *n - j;
 zlahef_aa_(uplo, &i__2, &i__1, &jb, &a[j + 1 + max(1,j) * a_dim1], lda, &ipiv[j + 1], &work[1], n, &work[*n * nb + 1]) ;
 /* Adjust IPIV and apply it back (J-th step picks (J+1)-th pivot) */
 /* Computing MIN */
 ...
 i__3 = jb + 1;
 z__1.r = -1.; z__1.i = -0.; // , expr subst  
 zgemm_("No transpose", "Conjugate transpose", &mj, & c__1, &i__3, &z__1, &work[j3 - j1 + 1 + k1 * * n], n, &a[j3 + (j1 - k2) * a_dim1], lda, & c_b2, &a[j3 + j3 * a_dim1], lda);
 ++j3;
 }
 ...
 i__4 = jb + 1;
 z__1.r = -1.; z__1.i = -0.; // , expr subst  
 zgemm_("No transpose", "Conjugate transpose", &i__3, &nj, &i__4, &z__1, &work[j3 - j1 + 1 + k1 * *n], n, &a[ j2 + (j1 - k2) * a_dim1], lda, &c_b2, &a[j3 + j2 * a_dim1], lda);
 }
 /* Recover T( J+1, J ) */
 i__1 = j + 1 + j * a_dim1;
 d_cnjg(&z__1, &alpha);
