/* ../netlib/v3.9.0/dgejsv.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -9;
 ...
 /* Computing MAX */
 d__1 = aatmax; d__2 = work[*n + p]; // , expr subst  
 aatmax = max(d__1,d__2);
 if (work[*n + p] != 0.) {
 /* Computing MIN */
 d__1 = aatmin; d__2 = work[*n + p]; // , expr subst  
 ...
 p <= i__1;
 ++p) {
 work[*m + *n + p] = scalem * (d__1 = a[p + idamax_(n, &a[p + a_dim1], lda) * a_dim1], abs(d__1));
 /* Computing MAX */
 d__1 = aatmax; d__2 = work[*m + *n + p]; // , expr subst  
 aatmax = max(d__1,d__2);
 /* Computing MIN */
 d__1 = aatmin; d__2 = work[*m + *n + p]; // , expr subst  
 aatmin = min(d__1,d__2);
 /* L1904: */
 }
 }
 ...
 p <= i__1;
 ++p) {
 if ((d__2 = a[p + p * a_dim1], abs(d__2)) >= temp1 * (d__1 = a[ a_dim1 + 1], abs(d__1))) {
 ++nr;
 }
 else {
 goto L3002;
 ...
 p <= i__1;
 ++p) {
 if ((d__2 = a[p + p * a_dim1], abs(d__2)) < epsln * (d__1 = a[p - 1 + (p - 1) * a_dim1], abs(d__1)) || (d__3 = a[p + p * a_dim1], abs(d__3)) < small || l2kill && (d__4 = a[p + p * a_dim1], abs(d__4)) < temp1) {
 goto L3402;
 }
 ++nr;
 /* L3401: */
 ...
 p <= i__1;
 ++p) {
 if ((d__1 = a[p + p * a_dim1], abs(d__1)) < small || l2kill && ( d__2 = a[p + p * a_dim1], abs(d__2)) < temp1) {
 goto L3302;
 }
 ++nr;
 /* L3301: */
 ...
 l2pert = l2pert && (d__1 = a[a_dim1 + 1] / a[nr + nr * a_dim1], abs(d__1)) > sqrt(big1);
 ...
 p <= i__2;
 ++p) {
 if (p > q && (d__1 = a[p + q * a_dim1], abs(d__1)) <= temp1 || p < q) {
 ...
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("U", &i__1, &i__2, &c_b34, &c_b34, &a[(a_dim1 << 1) + 1], lda);
 }
 ...
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("Lower", &i__1, &i__2, &c_b34, &c_b34, &a[a_dim1 + 2], lda);
 i__1 = *lwork - *n;
 dgelqf_(&nr, n, &a[a_offset], lda, &work[1], &work[*n + 1], &i__1, &ierr);
 ...
 p <= i__2;
 ++p) {
 if (p > q && (d__1 = v[p + q * v_dim1], abs(d__1)) <= temp1 || p < q) {
 ...
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("U", &i__1, &i__2, &c_b34, &c_b34, &v[(v_dim1 << 1) + 1], ldv);
 }
 ...
 ++q) {
 /* Computing MIN */
 d__3 = (d__1 = v[p + p * v_dim1], abs(d__1)); d__4 = (d__2 = v[q + q * v_dim1], abs( d__2)); // , expr subst  
 temp1 = xsc * min(d__3,d__4);
 if ((d__1 = v[q + p * v_dim1], abs(d__1)) <= temp1) {
 ...
 i__1 = nr - 1;
 i__2 = nr - 1;
 dlaset_("L", &i__1, &i__2, &c_b34, &c_b34, &v[v_dim1 + 2], ldv);
 }
 ...
 dlacpy_("U", &nr, &nr, &v[v_offset], ldv, &work[(*n << 1) + 1], n);
 ...
 ++p) {
 /* Computing MIN */
 d__3 = (d__1 = u[p + p * u_dim1], abs(d__1)); d__4 = ( d__2 = u[q + q * u_dim1], abs(d__2)); // , expr subst  
 temp1 = xsc * min(d__3,d__4);
