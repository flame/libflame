/* ../netlib/chetrf_rook.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 char buffer[256]; 
#if FLA_ENABLE_ILP64 
 snprintf(buffer, 256,"chetrf_rook inputs: uplo %c, n %lld, lda %lld, lwork %lld",*uplo, *n, *lda, *lwork);
#else 
 snprintf(buffer, 256,"chetrf_rook inputs: uplo %c, n %d, lda %d, lwork %d",*uplo, *n, *lda, *lwork);
#endif
 ...
 *info = -7;
 ...
 nb = max(i__1,1);
 /* Computing MAX */
 i__1 = 2; i__2 = ilaenv_(&c__2, "CHETRF_ROOK", uplo, n, &c_n1, & c_n1, &c_n1); // , expr subst  
 nbmin = max(i__1,i__2);
 }
 }
 else {
