...
int csycon_rook_(char *uplo, integer *n, complex *a, integer *lda, integer *ipiv, real *anorm, real *rcond, complex *work, integer *info)
{
    ...
    char buffer[256]; 
#if FLA_ENABLE_ILP64 
    snprintf(buffer, 256,"csycon_rook inputs: uplo %c, n %lld, lda %lld, ipiv %lld",*uplo, *n, *lda, *ipiv);
#else 
    snprintf(buffer, 256,"csycon_rook inputs: uplo %c, n %d, lda %d, ipiv %d",*uplo, *n, *lda, *ipiv);
#endif
