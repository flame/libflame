/* ../netlib/zlanhe.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
/* > \verbatim */
/* > */
/* > ZLANHE = ( max(f2c_dabs(A(i,j))), NORM = 'M' or 'm' */
/* > ( */
...
doublereal zlanhe_(char *norm, char *uplo, integer *n, doublecomplex *a, integer *lda, doublereal *work)
{
    ...
    else if (lsame_(norm, "M"))
    {
        ...
        if (lsame_(uplo, "U"))
        {
                    ...
                    ++j)
            {
                ...
                i__2 = j + j * a_dim1;
                sum = (d__1 = a[i__2].r, f2c_dabs(d__1));
                if (value < sum || disnan_(&sum))
                {
                    value = sum;
                }
    ...
    else if (lsame_(norm, "I") || lsame_(norm, "O") || *(unsigned char *)norm == '1')
    {
        ...
        if (lsame_(uplo, "U"))
        {
                    ...
                    ++j)
            {
                ...
                i__2 = j + j * a_dim1;
                work[j] = sum + (d__1 = a[i__2].r, f2c_dabs(d__1));
                /* L60: */
            }
            i__1 = *n;
            for (i__ = 1;
        ...
        else
        {
            ...
            {
                i__2 = j + j * a_dim1;
                sum = work[j] + (d__1 = a[i__2].r, f2c_dabs(d__1));
                i__2 = *n;
                for (i__ = j + 1;
                        i__ <= i__2;
                        ++i__)
    ...
    else if (lsame_(norm, "F") || lsame_(norm, "E"))
    {
                ...
                ++i__)
        {
            ...
            {
                i__2 = i__ + i__ * a_dim1;
                absa = (d__1 = a[i__2].r, f2c_dabs(d__1));
                if (scale < absa)
                {
                    /* Computing 2nd power */
                    d__1 = scale / absa;
