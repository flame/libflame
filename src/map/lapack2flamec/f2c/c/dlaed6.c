/* ../netlib/v3.9.0/dlaed6.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *tau = 0.;
 ...
 /* Computing MAX */
 d__1 = abs(a), d__2 = abs(b); d__1 = max(d__1,d__2); d__2 = abs(c__); // ; expr subst  
 temp = max(d__1,d__2);
 a /= temp;
 b /= temp;
 c__ /= temp;
 ...
 *tau = 0.;
 ...
 if (*orgati) {
 /* Computing MIN */
 d__3 = (d__1 = d__[2] - *tau, abs(d__1)); d__4 = (d__2 = d__[3] - * tau, abs(d__2)); // , expr subst  
 temp = min(d__3,d__4);
 }
 else {
 /* Computing MIN */
 d__3 = (d__1 = d__[1] - *tau, abs(d__1)); d__4 = (d__2 = d__[2] - * tau, abs(d__2)); // , expr subst  
 temp = min(d__3,d__4);
 }
 scale = FALSE_;
 if (temp <= small1) {
