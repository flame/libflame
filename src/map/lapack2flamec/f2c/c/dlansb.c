...
/* > \verbatim */
/* > */
/* > DLANSB = ( max(f2c_dabs(A(i,j))), NORM = 'M' or 'm' */
/* > ( */
    ...
    else if (lsame_(norm, "I") || lsame_(norm, "O") || *(unsigned char *)norm == '1')
    {
        ...
        if (lsame_(uplo, "U"))
        {
                    ...
                    ++j)
            {
                ...
                for (i__ = max(i__3,i__2);
                        ...
                        ++i__)
                {
                    absa = (d__1 = ab[l + i__ + j * ab_dim1], f2c_dabs(d__1));
                    sum += absa;
                    work[i__] += absa;
                    /* L50: */
                }
                work[j] = sum + (d__1 = ab[*k + 1 + j * ab_dim1], f2c_dabs(d__1));
                /* L60: */
            }
            i__1 = *n;
            for (i__ = 1;
        ...
        else
        {
            ...
            for (j = 1;
                    ...
                    ++j)
            {
                sum = work[j] + (d__1 = ab[j * ab_dim1 + 1], f2c_dabs(d__1));
                l = 1 - j;
                /* Computing MIN */
                i__3 = *n;
                i__2 = j + *k; // , expr subst
