...
FLA_Error FLA_Apply_pivots_macro_external_ts( FLA_cntl_init_s *FLA_cntl_init_i, FLA_Side side, FLA_Trans trans, FLA_Obj p, FLA_Obj A )
{
   int          i, j;
   int          ipiv;
   int*         buf_p    = ( int* ) FLA_Obj_buffer_at_view_ts( FLA_cntl_init_i, p );
   FLA_Obj*     blocks   = FLASH_OBJ_PTR_AT_TS( FLA_cntl_init_i, A );
   int          m_blocks = FLA_Obj_length( A );
   int          m_A      = FLA_Obj_length( *blocks );
   int          n_A      = FLA_Obj_width( *blocks );
   FLA_Datatype datatype = FLA_Obj_datatype_ts( FLA_cntl_init_i, A );
   ...
   switch ( datatype )
   {
      case FLA_FLOAT:
      {
         ...
         {
            buffer[i] = ( float* ) FLA_Obj_buffer_at_view_ts( FLA_cntl_init_i, blocks[i] );
      ...
      case FLA_DOUBLE:
      {
         ...
         for ( i = 0; i < m_blocks; i++ )
         {
            buffer[i] = ( double* ) FLA_Obj_buffer_at_view_ts( FLA_cntl_init_i, blocks[i] );
      ...
      case FLA_COMPLEX:
      {
         ...
         for ( i = 0; i < m_blocks; i++ )
         {
            buffer[i] = ( scomplex* ) FLA_Obj_buffer_at_view_ts( FLA_cntl_init_i, blocks[i] );
      ...
      case FLA_DOUBLE_COMPLEX:
      {
         ...
         for ( i = 0; i < m_blocks; i++ )
         {
            buffer[i] = ( dcomplex* ) FLA_Obj_buffer_at_view_ts( FLA_cntl_init_i, blocks[i] );
