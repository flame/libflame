...
/* ../netlib/v3.9.0/csysvxx.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 on Linux or Unix systems, link with .../path/to/libf2c.a -lm or, if you install libf2c.a in a standard place, with -lf2c -lm -- in that order, at the end of the command line, as in cc *.o -lf2c -lm Source for libf2c is in /netlib/f2c/libf2c.zip, e.g., http://www.netlib.org/f2c/libf2c.zip */
 #include "FLA_f2c.h" /* > \brief <b> CSYSVXX computes the solution to system of linear equations A * X = B for SY matrices</b> */
 /* =========== DOCUMENTATION =========== */
...
,inf) * norm(Z,inf)) for some */
 ...
 integer j;
 real amax, smin, smax;
 extern real cla_syrpvgrw_(char *, integer *, integer *, complex *, integer *, complex *, integer *, integer *, real *);
 ...
 integer infequ;
 extern /* Subroutine */
 int claqsy_(char *, integer *, complex *, integer *, real *, real *, real *, char *), csytrf_(char * , integer *, complex *, integer *, integer *, complex *, integer * , integer *);
 real smlnum;
 extern /* Subroutine */
 int clascl2_(integer *, integer *, real *, complex *, integer *), csytrs_(char *, integer *, integer *, complex *, integer *, integer *, complex *, integer *, integer *), csyequb_(char *, integer *, complex *, integer *, real *, real *, real *, complex *, integer *), csyrfsx_(char *, char *, integer *, integer *, complex *, integer *, complex *, integer *, integer *, real *, complex *, integer *, complex *, integer *, real *, real *, integer *, real *, real *, integer *, real *, complex *, real *, integer *);
 ...
 *info = -10;
 ...
 ++j) {
 /* Computing MIN */
 r__1 = smin; r__2 = s[j]; // , expr subst  
 smin = min(r__1,r__2);
 /* Computing MAX */
 r__1 = smax; r__2 = s[j]; // , expr subst  
 smax = max(r__1,r__2);
 /* L10: */
 }
 if (smin <= 0.f) {
 ...
 *info = -15;
 ...
 csyequb_(uplo, n, &a[a_offset], lda, &s[1], &scond, &amax, &work[1], & infequ);
 if (infequ == 0) {
 ...
 i__1 = max(1,*n) * 5;
 csytrf_(uplo, n, &af[af_offset], ldaf, &ipiv[1], &work[1], &i__1, info);
 ...
 if (*n > 0) {
 *rpvgrw = cla_syrpvgrw_(uplo, n, info, &a[a_offset], lda, & af[af_offset], ldaf, &ipiv[1], &rwork[1]);
 }
 return 0;
 }
 }
 ...
 *rpvgrw = cla_syrpvgrw_(uplo, n, info, &a[a_offset], lda, &af[ af_offset], ldaf, &ipiv[1], &rwork[1]);
 ...
 csyrfsx_(uplo, equed, n, nrhs, &a[a_offset], lda, &af[af_offset], ldaf, & ipiv[1], &s[1], &b[b_offset], ldb, &x[x_offset], ldx, rcond, & berr[1], n_err_bnds__, &err_bnds_norm__[err_bnds_norm_offset], & err_bnds_comp__[err_bnds_comp_offset], nparams, &params[1], &work[ 1], &rwork[1], info);
 /* Scale solutions. */
 if (rcequ) {
