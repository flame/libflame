/* ../netlib/v3.9.0/cgebrd.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -10;
 ...
 /* Computing MAX */
 i__1 = nb; i__2 = ilaenv_(&c__3, "CGEBRD", " ", m, n, &c_n1, &c_n1); // , expr subst  
 nx = max(i__1,i__2);
 ...
 i__3 = *m - i__ + 1;
 i__4 = *n - i__ + 1;
 clabrd_(&i__3, &i__4, &nb, &a[i__ + i__ * a_dim1], lda, &d__[i__], &e[ i__], &tauq[i__], &taup[i__], &work[1], &ldwrkx, &work[ldwrkx * nb + 1], &ldwrky);
 /* Update the trailing submatrix A(i+ib:m,i+ib:n), using */
 /* an update of the form A := A - V*Y**H - X*U**H */
 i__3 = *m - i__ - nb + 1;
 i__4 = *n - i__ - nb + 1;
 q__1.r = -1.f; q__1.i = -0.f; // , expr subst  
 cgemm_("No transpose", "Conjugate transpose", &i__3, &i__4, &nb, & q__1, &a[i__ + nb + i__ * a_dim1], lda, &work[ldwrkx * nb + nb + 1], &ldwrky, &c_b1, &a[i__ + nb + (i__ + nb) * a_dim1], lda);
 i__3 = *m - i__ - nb + 1;
 i__4 = *n - i__ - nb + 1;
 q__1.r = -1.f; q__1.i = -0.f; // , expr subst  
 cgemm_("No transpose", "No transpose", &i__3, &i__4, &nb, &q__1, & work[nb + 1], &ldwrkx, &a[i__ + (i__ + nb) * a_dim1], lda, & c_b1, &a[i__ + nb + (i__ + nb) * a_dim1], lda);
 ...
 i__4 = j + j * a_dim1;
 i__5 = j;
 a[i__4].r = d__[i__5]; a[i__4].i = 0.f; // , expr subst  
 i__4 = j + (j + 1) * a_dim1;
 i__5 = j;
 a[i__4].r = e[i__5]; a[i__4].i = 0.f; // , expr subst  
 /* L10: */
 }
 }
 else {
