/* ../netlib/zgeqlf.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int zgeqlf_(integer *m, integer *n, doublecomplex *a, integer *lda, doublecomplex *tau, doublecomplex *work, integer *lwork, integer *info)
{
    ...
    char buffer[256];
    snprintf(buffer, 256,"zgeqlf inputs: m %" FLA_IS ", n %" FLA_IS ", lda %" FLA_IS ", lwork %" FLA_IS "",*m, *n, *lda, *lwork);
    ...
    if (nb >= nbmin && nb < k && nx < k)
    {
                ...
                i__ += i__2)
        {
            ...
            if (*n - k + i__ > 1)
            {
                ...
                i__3 = *m - k + i__ + ib - 1;
                i__4 = *n - k + i__ - 1;
                zlarfb_("Left", "Conjugate transpose", "Backward", "Columnwi" "se", &i__3, &i__4, &ib, &a[(*n - k + i__) * a_dim1 + 1], lda, &work[1], &ldwork, &a[a_offset], lda, &work[ ib + 1], &ldwork);
            }
            /* L10: */
        }
        mu = *m - k + i__ + nb - 1;
