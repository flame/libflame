 ...
 */
 ...
 char buffer[256];
 snprintf(buffer, 256,"zhetri_3 inputs: uplo %c, n %" FLA_IS ", lda %" FLA_IS ", ipiv %" FLA_IS ", lwork %" FLA_IS "",*uplo, *n, *lda, *ipiv, *lwork);
 ...
 *info = 0;
 ...
 /* Computing MAX */
 i__1 = 1; i__2 = ilaenv_(&c__1, "ZHETRI_3", uplo, n, &c_n1, &c_n1, &c_n1); // , expr subst  
 nb = max(i__1,i__2);
 lwkopt = (*n + nb + 1) * (nb + 3);
