    ...
    *info = 0;
    ...
    switch (datatype)
    {
        case FLOAT:
        {
            ...
            /* compute I - A' * A */
            fla_lapack_slaset("full", &m_A, &m_A, &s_zero, &s_one, I, &m_A);
            /* compute I - A' * A */
            sgemm_("N", "N", &m_A, &n_A, &m_A, &s_n_one, A_inv, &lda, A, &lda, &s_one, I, &m_A);

            norm = fla_lapack_slange("1", &m_A, &m_A, I, &m_A, work);
        ...
        case DOUBLE:
        {
            ...
            res = (1 / norm_A) / norm_A_I;
            /* compute I - A' * A */
            fla_lapack_dlaset("full", &m_A, &m_A, &d_zero, &d_one, I, &m_A);
            /* compute I - A' * A */
            dgemm_("N", "N", &m_A, &n_A, &m_A, &d_n_one, A_inv, &lda, A, &lda, &d_one, I, &m_A);

            norm = fla_lapack_dlange("1", &m_A, &m_A, I, &m_A, work);
            /* Compute norm(I - A'*A) / (N * norm(A) * norm(AINV) * EPS)*/
            *residual = (norm * res) / eps / (double)n_A;
            break;
        }
        case COMPLEX:
        {
            ...
            res = (1 / norm_A) / norm_A_I;
            /* compute I - A' * A */
            fla_lapack_claset("full", &m_A, &m_A, &c_zero, &c_one, I, &m_A);
            /* compute I - A' * A */
            cgemm_("N", "N", &m_A, &n_A, &m_A, &c_n_one, A_inv, &lda, A, &lda, &c_one, I, &m_A);

            norm = fla_lapack_clange("1", &m_A, &m_A, I, &m_A, work);
            /* Compute norm(I - A'*A) / (N * norm(A) * norm(AINV) * EPS)*/
            *residual = (norm * res) / eps / (double)n_A;
            break;
        }
        case DOUBLE_COMPLEX:
        {
            ...
            res = (1 / norm_A) / norm_A_I;
            /* compute I - A' * A */
            fla_lapack_zlaset("full", &m_A, &m_A, &z_zero, &z_one, I, &m_A);
            /* compute I - A' * A */
            zgemm_("N", "N", &m_A, &n_A, &m_A, &z_n_one, A_inv, &lda, A, &lda, &z_one, I, &m_A);

            norm = fla_lapack_zlange("1", &m_A, &m_A, I, &m_A, work);
            /* Compute norm(I - A'*A) / (N * norm(A) * norm(AINV) * EPS)*/
            *residual = (norm * res) / eps / (double)n_A;
            break;
        }
