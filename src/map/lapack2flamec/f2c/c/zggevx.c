/* ../netlib/v3.9.0/zggevx.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = *n + 2;
 ...
 /* Computing MAX */
 i__3 = jr + jc * vl_dim1;
 d__3 = temp; d__4 = (d__1 = vl[i__3].r, abs(d__1)) + (d__2 = d_imag(&vl[jr + jc * vl_dim1]), abs(d__2)); // , expr subst  
 temp = max(d__3,d__4);
 /* L30: */
 }
 if (temp < smlnum) {
 ...
 /* Computing MAX */
 i__3 = jr + jc * vr_dim1;
 d__3 = temp; d__4 = (d__1 = vr[i__3].r, abs(d__1)) + (d__2 = d_imag(&vr[jr + jc * vr_dim1]), abs(d__2)); // , expr subst  
 temp = max(d__3,d__4);
 /* L60: */
 }
 if (temp < smlnum) {
