...
/*! @file libflame_hesv_aa.cc
 *  @brief Test application to validate hesv_aa() using CPP template interface.
 *  */
...
/*! @brief  hesv_aa_test is function template for hesv_aa() functions.
			T can be scomplex, dcomplex
 * @details
 * \b Purpose:
    \verbatim
	  hesv_aa_test is function template for hesv_aa() functions.
	  T can be scomplex, dcomplex
	  
	  hesv_aa_test() function template calls C and CPP based lbrary APIs with
    ...
    Fails the test case if difference > threshold or INFO < 0.
 	  ...
 	  Complex reference:
    http://www.netlib.org/lapack/explore-html/d8/d8e/group__complex_h_esolve_ga0a239b86f174f69fc5f261fb59fd14ef.html#ga0a239b86f174f69fc5f261fb59fd14ef
    Complex double reference:
    http://www.netlib.org/lapack/explore-html/d3/d9d/group__complex16_h_esolve_ga4291a01be2cd481e26db1ac30e87c0af.html#ga4291a01be2cd481e26db1ac30e87c0af
    \endverbatim
 ...
 * */
template<typename T>
void hesv_aa_test(int ip)
{
  typedef int (*fptr_NL_LAPACK_hesv_aa)(char* uplo, integer* n, integer* nrhs,
  ...
  integer lwork = lin_driver_paramslist[ip].lwork_hesv_aa;
  ...
  /* If lwork/lrwork/liwork is -1, then call this API with default work
     ...
     needed.*/
  if (lwork == -1) {
    PRINTF("lwork is -1, so call hesv_aa() to get the array sizes.\n");
    T worksize = {0};
    
    // Call CPP function
    libflame::hesv_aa<T>(&uplo, &n, &nrhs, abuff, &lda, ipivbuff, bbuff, &ldb,
  ...
  if (typeid(T) == typeid(scomplex)) {
    hesv_aa_ref = (fptr_NL_LAPACK_hesv_aa)dlsym(lapackModule, "chesv_aa_");
  } else if (typeid(T) == typeid(dcomplex)) {
    hesv_aa_ref = (fptr_NL_LAPACK_hesv_aa)dlsym(lapackModule, "zhesv_aa_");
  } else {
...
TEST(LAPACKCPP_hesv_aa, CHESV_AA) {
  for (short int index = 0; index < NUM_SUB_TESTS; index++) {
	  PRINTF("index: %d\n", index);
    hesv_aa_test<scomplex> (index);
  }
}

/* Use TEST macro and call C++ test function template with
   dcomplex as typenames.*/
TEST(LAPACKCPP_hesv_aa, ZHESV_AA) {
  for (short int index = 0; index < NUM_SUB_TESTS; index++) {
	  PRINTF("index: %d\n", index);
    hesv_aa_test<dcomplex> (index);
  }
}