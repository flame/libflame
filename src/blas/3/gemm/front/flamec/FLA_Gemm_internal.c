...
FLA_Error FLA_Gemm_internal_ts( FLA_cntl_init_s *FLA_cntl_init_i, FLA_Trans transa, FLA_Trans transb, FLA_Obj alpha, FLA_Obj A, FLA_Obj B, FLA_Obj beta, FLA_Obj C, fla_gemm_t* cntl )
{
	FLA_Error r_val = FLA_SUCCESS;

	if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
		FLA_Gemm_internal_check_ts( FLA_cntl_init_i, transa, transb, alpha, A, B, beta, C, cntl );
	...
	{
		// Recurse
		r_val = FLA_Gemm_internal_ts( FLA_cntl_init_i, transa, 
		                           transb, 
		                           alpha, 
		                           *FLASH_OBJ_PTR_AT_TS( FLA_cntl_init_i, A ), 
		                           *FLASH_OBJ_PTR_AT_TS( FLA_cntl_init_i, B ), 
		                           beta, 
		                           *FLASH_OBJ_PTR_AT_TS( FLA_cntl_init_i, C ), 
		                           FLA_cntl_init_i->FLA_Cntl_init_flash_i->flash_gemm_cntl_mm_op );
	}
	...
	else
	{
		...
		{
			// Execute leaf
			cntl = FLA_cntl_init_i->FLA_Cntl_init_flash_i->flash_gemm_cntl_blas;
		}
		...
		if      ( transa == FLA_NO_TRANSPOSE )
		{
			if      ( transb == FLA_NO_TRANSPOSE )
				r_val = FLA_Gemm_nn_ts( FLA_cntl_init_i, alpha, A, B, beta, C, cntl );
