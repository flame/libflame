/* ../netlib/cuncsd.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 char buffer[256]; 
#if FLA_ENABLE_ILP64 
 snprintf(buffer, 256,"cuncsd inputs: jobu1 %c, jobu2 %c, jobv1t %c, jobv2t %c, trans %c, signs %c, m %lld, p %lld, q %lld, ldx11 %lld, ldx12 %lld, ldx21 %lld, ldx22 %lld, ldu1 %lld, ldu2 %lld, ldv1t %lld, ldv2t %lld, lwork %lld, lrwork %lld",*jobu1, *jobu2, *jobv1t, *jobv2t, *trans, *signs, *m, *p, *q, *ldx11, *ldx12, *ldx21, *ldx22, *ldu1, *ldu2, *ldv1t, *ldv2t, *lwork, *lrwork);
#else 
 snprintf(buffer, 256,"cuncsd inputs: jobu1 %c, jobu2 %c, jobv1t %c, jobv2t %c, trans %c, signs %c, m %d, p %d, q %d, ldx11 %d, ldx12 %d, ldx21 %d, ldx22 %d, ldu1 %d, ldu2 %d, ldv1t %d, ldv2t %d, lwork %d, lrwork %d",*jobu1, *jobu2, *jobv1t, *jobv2t, *trans, *signs, *m, *p, *q, *ldx11, *ldx12, *ldx21, *ldx22, *ldu1, *ldu2, *ldv1t, *ldv2t, *lwork, *lrwork);
#endif
 ...
 *info = -26;
 ...
 /* Computing MIN */
 i__1 = *p; i__2 = *m - *p; // , expr subst  
 /* Computing MIN */
 i__3 = *q; i__4 = *m - *q; // , expr subst  
 ...
 *(unsigned char *)signst = 'D';
 ...
 cbbcsd_(jobu1, jobu2, jobv1t, jobv2t, trans, m, p, q, &theta[1], & theta[1], &u1[u1_offset], ldu1, &u2[u2_offset], ldu2, &v1t[ v1t_offset], ldv1t, &v2t[v2t_offset], ldv2t, &theta[1], & theta[1], &theta[1], &theta[1], &theta[1], &theta[1], &theta[ 1], &theta[1], &rwork[1], &c_n1, &childinfo);
 ...
 i__3 = *m - *q;
 /* Computing MAX */
 i__5 = 1; i__6 = *m - *q; // , expr subst  
 i__4 = max(i__5,i__6);
 cungqr_(&i__1, &i__2, &i__3, &u1[u1_offset], &i__4, &u1[u1_offset], & work[1], &c_n1, &childinfo);
 ...
 i__5 = 1; i__6 = *m - *q; // , expr subst  
 i__4 = max(i__5,i__6);
 cunglq_(&i__1, &i__2, &i__3, &u1[u1_offset], &i__4, &u1[u1_offset], & work[1], &c_n1, &childinfo);
 ...
 cunbdb_(trans, signs, m, p, q, &x11[x11_offset], ldx11, &x12[ x12_offset], ldx12, &x21[x21_offset], ldx21, &x22[x22_offset], ldx22, &theta[1], &theta[1], &u1[u1_offset], &u2[u2_offset], &v1t[v1t_offset], &v2t[v2t_offset], &work[1], &c_n1, & childinfo);
 lorbdbworkopt = (integer) work[1].r;
 lorbdbworkmin = lorbdbworkopt;
 /* Computing MAX */
 i__1 = iorgqr + lorgqrworkopt, i__2 = iorglq + lorglqworkopt; i__1 = max(i__1,i__2); i__2 = iorbdb + lorbdbworkopt; // ; expr subst  
 lworkopt = max(i__1,i__2) - 1;
 /* Computing MAX */
 i__1 = iorgqr + lorgqrworkmin, i__2 = iorglq + lorglqworkmin; i__1 = max(i__1,i__2); i__2 = iorbdb + lorbdbworkmin; // ; expr subst  
 ...
 *info = -24;
 ...
 i__1 = *m - *p;
 i__2 = *m - *p;
 cungqr_(&i__1, &i__2, q, &u2[u2_offset], ldu2, &work[itaup2], & work[iorgqr], &lorgqrwork, info);
 }
 if (wantv1t && *q > 0) {
 i__1 = *q - 1;
 i__2 = *q - 1;
 clacpy_("U", &i__1, &i__2, &x11[(x11_dim1 << 1) + 1], ldx11, &v1t[ (v1t_dim1 << 1) + 2], ldv1t);
 i__1 = v1t_dim1 + 1;
 ...
 i__2 = *q - 1;
 i__3 = *q - 1;
 cunglq_(&i__1, &i__2, &i__3, &v1t[(v1t_dim1 << 1) + 2], ldv1t, & work[itauq1], &work[iorglq], &lorglqwork, info);
 }
 if (wantv2t && *m - *q > 0) {
 i__1 = *m - *q;
 ...
 i__1 = *m - *p - *q;
 i__2 = *m - *p - *q;
 clacpy_("U", &i__1, &i__2, &x22[*q + 1 + (*p + 1) * x22_dim1], ldx22, &v2t[*p + 1 + (*p + 1) * v2t_dim1], ldv2t);
 }
 if (*m > *q) {
 i__1 = *m - *q;
 i__2 = *m - *q;
 i__3 = *m - *q;
 cunglq_(&i__1, &i__2, &i__3, &v2t[v2t_offset], ldv2t, &work[ itauq2], &work[iorglq], &lorglqwork, info);
 }
 }
 }
 else {
 ...
 i__1 = *m - *p;
 i__2 = *m - *p;
 cunglq_(&i__1, &i__2, q, &u2[u2_offset], ldu2, &work[itaup2], & work[iorglq], &lorglqwork, info);
 }
 if (wantv1t && *q > 0) {
 i__1 = *q - 1;
 i__2 = *q - 1;
 ...
 i__2 = *q - 1;
 i__3 = *q - 1;
 cungqr_(&i__1, &i__2, &i__3, &v1t[(v1t_dim1 << 1) + 2], ldv1t, & work[itauq1], &work[iorgqr], &lorgqrwork, info);
 }
 if (wantv2t && *m - *q > 0) {
 /* Computing MIN */
 i__1 = *p + 1;
 ...
 i__1 = *m - *p - *q;
 i__2 = *m - *p - *q;
 clacpy_("L", &i__1, &i__2, &x22[p1 + q1 * x22_dim1], ldx22, & v2t[*p + 1 + (*p + 1) * v2t_dim1], ldv2t);
 }
 i__1 = *m - *q;
 i__2 = *m - *q;
 i__3 = *m - *q;
 cungqr_(&i__1, &i__2, &i__3, &v2t[v2t_offset], ldv2t, &work[ itauq2], &work[iorgqr], &lorgqrwork, info);
 }
 }
 /* Compute the CSD of the matrix in bidiagonal-block form */
 cbbcsd_(jobu1, jobu2, jobv1t, jobv2t, trans, m, p, q, &theta[1], &rwork[ iphi], &u1[u1_offset], ldu1, &u2[u2_offset], ldu2, &v1t[ v1t_offset], ldv1t, &v2t[v2t_offset], ldv2t, &rwork[ib11d], & rwork[ib11e], &rwork[ib12d], &rwork[ib12e], &rwork[ib21d], &rwork[ ib21e], &rwork[ib22d], &rwork[ib22e], &rwork[ibbcsd], &lbbcsdwork, info);
