/* ../netlib/v3.9.0/slangt.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 i__ <= i__1;
 ++i__) {
 temp = (r__1 = d__[i__], abs(r__1)) + (r__2 = dl[i__], abs( r__2)) + (r__3 = du[i__ - 1], abs(r__3));
 if (anorm < temp || sisnan_(&temp)) {
 anorm = temp;
 }
 /* L20: */
 ...
 i__ <= i__1;
 ++i__) {
 temp = (r__1 = d__[i__], abs(r__1)) + (r__2 = du[i__], abs( r__2)) + (r__3 = dl[i__ - 1], abs(r__3));
 if (anorm < temp || sisnan_(&temp)) {
 anorm = temp;
 }
 /* L30: */
