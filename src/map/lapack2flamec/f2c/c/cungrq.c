/* ../netlib/cungrq.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int cungrq_(integer *m, integer *n, integer *k, complex *a, integer *lda, complex *tau, complex *work, integer *lwork, integer * info)
{
    ...
    char buffer[256];
    snprintf(buffer, 256,"cungrq inputs: m %" FLA_IS ", n %" FLA_IS ", k %" FLA_IS ", lda %" FLA_IS ", lwork %" FLA_IS "",*m, *n, *k, *lda, *lwork);
    ...
    if (kk > 0)
    {
                ...
                i__ += i__2)
        {
            ...
            if (ii > 1)
            {
                ...
                i__3 = *n - *k + i__ + ib - 1;
                clarft_("Backward", "Rowwise", &i__3, &ib, &a[ii + a_dim1], lda, &tau[i__], &work[1], &ldwork);
                /* Apply H**H to A(1:m-k+i-1,1:n-k+i+ib-1) from the right */
                i__3 = ii - 1;
                i__4 = *n - *k + i__ + ib - 1;
                clarfb_("Right", "Conjugate transpose", "Backward", "Rowwise", &i__3, &i__4, &ib, &a[ii + a_dim1], lda, &work[1], & ldwork, &a[a_offset], lda, &work[ib + 1], &ldwork);
            }
