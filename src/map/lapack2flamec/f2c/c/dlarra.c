/* ../netlib/dlarra.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int dlarra_(integer *n, doublereal *d__, doublereal *e, doublereal *e2, doublereal *spltol, doublereal *tnrm, integer *nsplit, integer *isplit, integer *info)
{
    ...
    *nsplit = 1;
    ...
    else
    {
        ...
        {
            eabs = (d__1 = e[i__], f2c_dabs(d__1));
            if (eabs <= *spltol * sqrt((d__1 = d__[i__], f2c_dabs(d__1))) * sqrt(( d__2 = d__[i__ + 1], f2c_dabs(d__2))))
            {
                e[i__] = 0.;
                e2[i__] = 0.;
                isplit[*nsplit] = i__;
