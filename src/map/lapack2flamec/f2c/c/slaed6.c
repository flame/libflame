/* ../netlib/v3.9.0/slaed6.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *tau = 0.f;
 ...
 /* Computing MAX */
 r__1 = abs(a), r__2 = abs(b); r__1 = max(r__1,r__2); r__2 = abs(c__); // ; expr subst  
 temp = max(r__1,r__2);
 a /= temp;
 b /= temp;
 c__ /= temp;
 ...
 *tau = 0.f;
 ...
 if (*orgati) {
 /* Computing MIN */
 r__3 = (r__1 = d__[2] - *tau, abs(r__1)); r__4 = (r__2 = d__[3] - * tau, abs(r__2)); // , expr subst  
 temp = min(r__3,r__4);
 }
 else {
 /* Computing MIN */
 r__3 = (r__1 = d__[1] - *tau, abs(r__1)); r__4 = (r__2 = d__[2] - * tau, abs(r__2)); // , expr subst  
 temp = min(r__3,r__4);
 }
 scale = FALSE_;
 if (temp <= small1) {
