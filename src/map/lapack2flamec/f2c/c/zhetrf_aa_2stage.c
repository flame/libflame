 ...
 */
 ...
 char buffer[256];
 snprintf(buffer, 256,"zhetrf_aa_2stage inputs: uplo %c, n %" FLA_IS ", lda %" FLA_IS ", ltb %" FLA_IS ", lwork %" FLA_IS "",*uplo, *n, *lda, *ltb, *lwork);
 ...
 *info = -10;
 ...
 d__1 = tb[i__4].r;
 tb[i__3].r = d__1; tb[i__3].i = 0.; // , expr subst  
 i__3 = kb;
 for (k = i__ + 1;
 k <= i__3;
 ++k) {
 ...
 tb[i__4].r = z__1.r; tb[i__4].i = z__1.i; // , expr subst  
 }
 }
 if (j < nt - 1) {
 if (j > 0) {
