...
// Static variables.
static char* op_str                   = "Symmetric rank-2k update";
static char* flash_front_str          = "FLASH_Syr2k";
static char* fla_front_str            = "FLA_Syr2k";
...
static fla_syr2k_t*     syr2k_cntl_unb;
static fla_syr2k_t*     syr2k_cntl_blk;
static fla_blocksize_t* syr2k_cntl_bsize;

// Local prototypes.
void libfla_test_syr2k_experiment( test_params_t params,
                                   unsigned int  var,
                                   char*         sc_str,
                                   ...
                                   double*       perf,
                                   double*       residual );
void libfla_test_syr2k_impl( int         impl,
                             FLA_Uplo    uplo,
                             FLA_Trans   trans,
                             FLA_Obj     alpha,
                             FLA_Obj     A,
...
void libfla_test_syr2k_cntl_create( unsigned int var,
                                    dim_t        b_alg_flat );
void libfla_test_syr2k_cntl_free( void );
...
void libfla_test_syr2k( FILE* output_stream, test_params_t params, test_op_t op )
{
	...
	if ( op.flash_front == ENABLE )
	{
		libfla_test_op_driver( flash_front_str, NULL,
		                       ...
		                       params, thresh, libfla_test_syr2k_experiment );
	}
                                   ...
                                   double*       residual )
{
	...
	if ( impl == FLA_TEST_FLAT_UNB_VAR ||
		...
		libfla_test_syr2k_cntl_create( var, b_alg_flat );
	...
	for ( i = 0; i < n_repeats; ++i )
	{
		...
		libfla_test_syr2k_impl( impl, uplo, trans, alpha, A_test, B_test, beta, C_test );
	...
	*perf = ( 2 * m * m * k ) / time_min / FLOPS_PER_UNIT_PERF;
	...
	//   y = C * x
	// and compare to
	//   z = ( beta * C_orig + alpha * A * B' + alpha * B * A' ) x      (trans = notrans)
	//   z = ( beta * C_orig + alpha * A' * B + alpha * B' * A ) x      (trans = trans)
	...
	if ( trans == FLA_NO_TRANSPOSE )
	{
		...
		FLA_Gemv_external( FLA_TRANSPOSE,    FLA_ONE, B, x, FLA_ZERO, w );
	...
	else // if ( trans == FLA_TRANSPOSE )
	{
		...
		FLA_Gemv_external( FLA_TRANSPOSE,    alpha,   B, w, FLA_ZERO, z );
		FLA_Gemv_external( FLA_NO_TRANSPOSE, FLA_ONE, B, x, FLA_ZERO, w );
		FLA_Gemv_external( FLA_TRANSPOSE,    alpha,   A, w, FLA_ONE,  z );
	}
                                    ...
                                    dim_t        b_alg_flat )
{
	...
	syr2k_cntl_bsize = FLA_Blocksize_create( b_alg_flat,
	                                         b_alg_flat,
	                                         b_alg_flat,
	                                         b_alg_flat );

	syr2k_cntl_unb   = FLA_Cntl_syr2k_obj_create( FLA_FLAT,
	                                              var_unb,
	                                              NULL,
	                                              NULL,
	                                              NULL,
	...
	syr2k_cntl_blk   = FLA_Cntl_syr2k_obj_create( FLA_FLAT,
	                                              var_blk,
	                                              syr2k_cntl_bsize,
	                                              fla_scalr_cntl_blas,
	                                              fla_syr2k_cntl_blas,
...
void libfla_test_syr2k_cntl_free( void )
{
	FLA_Blocksize_free( syr2k_cntl_bsize );

	FLA_Cntl_obj_free( syr2k_cntl_unb );
	FLA_Cntl_obj_free( syr2k_cntl_blk );
}
                             ...
                             FLA_Obj   C )
{
	switch ( impl )
	{
		...
		FLASH_Syr2k( uplo, trans, alpha, A, B, beta, C );
		break;

		case FLA_TEST_FLAT_FRONT_END:
		FLA_Syr2k( uplo, trans, alpha, A, B, beta, C );
		break;

		case FLA_TEST_FLAT_UNB_VAR:
		FLA_Syr2k_internal( uplo, trans, alpha, A, B, beta, C, syr2k_cntl_unb );
		break;

		case FLA_TEST_FLAT_BLK_VAR:
		FLA_Syr2k_internal( uplo, trans, alpha, A, B, beta, C, syr2k_cntl_blk );
		break;
