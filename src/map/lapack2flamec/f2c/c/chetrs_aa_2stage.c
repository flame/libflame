/* ../netlib/v3.9.0/chetrs_aa_2stage.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/chetrs_ aa_2stage.f"> */
/* > [TGZ]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/chetrs_ aa_2stage.f"> */
/* > [ZIP]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/chetrs_ aa_2stage.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
/* Definition: */
/* =========== */
...
int chetrs_aa_2stage_(char *uplo, integer *n, integer *nrhs, complex *a, integer *lda, complex *tb, integer *ltb, integer *ipiv, integer *ipiv2, complex *b, integer *ldb, integer *info)
{
    ...
    char buffer[256];
#if FLA_ENABLE_ILP64
    snprintf(buffer, 256,"chetrs_aa_2stage inputs: uplo %c, n %lld, nrhs %lld, lda %lld, ltb %lld, ipiv %lld, ipiv2 %lld, ldb %lld",*uplo, *n, *nrhs, *lda, *ltb, *ipiv, *ipiv2, *ldb);
#else
    snprintf(buffer, 256,"chetrs_aa_2stage inputs: uplo %c, n %d, nrhs %d, lda %d, ltb %d, ipiv %d, ipiv2 %d, ldb %d",*uplo, *n, *nrhs, *lda, *ltb, *ipiv, *ipiv2, *ldb);
#endif
    ...
    *info = 0;
    ...
    if (upper)
    {
        ...
        if (*n > nb)
        {
            ...
            i__1 = *n - nb;
            ctrsm_("L", "U", "C", "U", &i__1, nrhs, &c_b1, &a[(nb + 1) * a_dim1 + 1], lda, &b[nb + 1 + b_dim1], ldb);
        }
    ...
    else
    {
        ...
        if (*n > nb)
        {
            ...
            i__1 = *n - nb;
            ctrsm_("L", "L", "C", "U", &i__1, nrhs, &c_b1, &a[nb + 1 + a_dim1], lda, &b[nb + 1 + b_dim1], ldb);
