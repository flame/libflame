...
\renewcommand{\invariant}{
  \FlaOneByTwo{C_L}{C_R}  = 
  \FlaOneByTwo{\hat{C}_L}{ A B_T^T + \hat{C}_R } 
}
...
=
  \FlaOneByThreeL{\hat{C}_0}{\hat{C}_1}{ A B_2^T + \hat{C}_2 }
}
...
=
  \FlaOneByThreeR{\hat{C}_0}{A B_1^T + \hat{C}_1}{ A B_2^T + \hat{C}_2}
}
...
%  \ref{FLA_Gemm_nt:ws_blk_var2} in the \LaTeX source 
 
\label{FLA_Gemm_nt:ws_blk_var2} 
...
% The following command creates a label so that the figure can 
% be referenced by its number by inserting  
%  \ref{FLA_Gemm_nt:alg_blk_var2} in the \LaTeX source 
 
\label{FLA_Gemm_nt:alg_blk_var2} 
