/* ../netlib/v3.9.0/dlarre.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -1;
 ...
 /* Computing MAX */
 d__2 = gl; d__3 = tmp - tmp1 - eps * 100. * (d__1 = tmp - tmp1, abs(d__1)); // , expr subst  
 ...
 *info = -1;
 ...
 /* Computing MIN */
 d__2 = gu; d__3 = tmp + tmp1 + eps * 100. * (d__1 = tmp + tmp1, abs(d__1)); // , expr subst  
 ...
 /* Computing MAX */
 d__2 = gl; d__3 = w[wbegin] - werr[wbegin] - eps * 100. * (d__1 = w[wbegin] - werr[wbegin], abs(d__1)); // , expr subst  
 isleft = max(d__2,d__3);
 /* Computing MIN */
 d__2 = gu; d__3 = w[wend] + werr[wend] + eps * 100. * (d__1 = w[ wend] + werr[wend], abs(d__1)); // , expr subst  
 isrght = min(d__2,d__3);
 }
 ...
 /* Computing MAX */
 d__1 = tau; d__2 = eps * 2. * abs(sigma); // , expr subst  
 tau = max(d__1,d__2);
 }
 else {
 if (mb > 1) {
 ...
 work[i__ + 1] = dpivot;
 /* Computing MAX */
 d__1 = dmax__; d__2 = abs(dpivot); // , expr subst  
