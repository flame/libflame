/* ../netlib/v3.9.0/dladiv.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 dd = *d__;
 /* Computing MAX */
 d__1 = abs(*a); d__2 = abs(*b); // , expr subst  
 ab = max(d__1,d__2);
 /* Computing MAX */
 d__1 = abs(*c__); d__2 = abs(*d__); // , expr subst  
 cd = max(d__1,d__2);
 s = 1.;
