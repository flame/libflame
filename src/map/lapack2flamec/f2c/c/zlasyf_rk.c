/* ../netlib/v3.9.0/zlasyf_rk.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
;
 ...
 /* > \brief \b ZLASYF_RK computes a partial factorization of a complex symmetric indefinite matrix using bound ed Bunch-Kaufman (rook) diagonal pivoting method. */
 /* =========== DOCUMENTATION =========== */
 /* Online html documentation available at */
 /* http://www.netlib.org/lapack/explore-html/ */
 /* > \htmlonly */
 /* > Download ZLASYF_RK + dependencies */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zlasyf_ rk.f"> */
 /* > [TGZ]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zlasyf_ rk.f"> */
 /* > [ZIP]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zlasyf_ rk.f"> */
 /* > [TXT]</a> */
 /* > \endhtmlonly */
 /* Definition: */
 /* =========== */
 ...
 *info = 0;
 ...
 i__1 = k + kw * w_dim1;
 absakk = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w[k + kw * w_dim1]), abs(d__2));
 ...
 i__1 = imax + kw * w_dim1;
 colmax = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w[imax + kw * w_dim1]), abs(d__2));
 }
 else {
 colmax = 0.;
 }
 ...
 *info = k;
 ...
 rowmax = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(& w[jmax + (kw - 1) * w_dim1]), abs(d__2));
 }
 else {
 rowmax = 0.;
 }
 ...
 dtemp = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w[ itemp + (kw - 1) * w_dim1]), abs(d__2));
 if (dtemp > rowmax) {
 rowmax = dtemp;
 jmax = itemp;
 }
 ...
 if (! ((d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w[imax + (kw - 1) * w_dim1]), abs(d__2)) < alpha * rowmax)) {
 ...
 zcopy_(&p, &a[k * a_dim1 + 1], &c__1, &a[p * a_dim1 + 1], & c__1);
 /* Interchange rows K and P in last N-K+1 columns of A */
 /* and last N-K+2 columns of W */
 i__1 = *n - k + 1;
 zswap_(&i__1, &a[k + k * a_dim1], lda, &a[p + k * a_dim1], lda);
 i__1 = *n - kk + 1;
 ...
 zcopy_(&kp, &a[kk * a_dim1 + 1], &c__1, &a[kp * a_dim1 + 1], & c__1);
 /* Interchange rows KK and KP in last N-KK+1 columns */
 /* of A and W */
 i__1 = *n - kk + 1;
 zswap_(&i__1, &a[kk + kk * a_dim1], lda, &a[kp + kk * a_dim1], lda);
 i__1 = *n - kk + 1;
 ...
 r1.r = z__1.r; r1.i = z__1.i; // , expr subst  
 i__1 = k - 1;
 ...
 ++ii) {
 i__2 = ii + k * a_dim1;
 z_div(&z__1, &a[ii + k * a_dim1], &a[k + k * a_dim1]);
 ...
 i__1 = k - 1 + kw * w_dim1;
 d12.r = w[i__1].r; d12.i = w[i__1].i; // , expr subst  
 ...
 z__3.r = d11.r * d22.r - d11.i * d22.i; z__3.i = d11.r * d22.i + d11.i * d22.r; // , expr subst  
 z__2.r = z__3.r - 1.; z__2.i = z__3.i - 0.; // , expr subst  
 ...
 z_div(&z__2, &z__3, &d12);
 z__1.r = t.r * z__2.r - t.i * z__2.i; z__1.i = t.r * z__2.i + t.i * z__2.r; // , expr subst  
 a[i__2].r = z__1.r; a[i__2].i = z__1.i; // , expr subst  
 i__2 = j + k * a_dim1;
 i__3 = j + kw * w_dim1;
 ...
 *kb = *n - k;
 ...
 /* a 1-by-1 or 2-by-2 pivot block will be used */
 i__1 = k + k * w_dim1;
 absakk = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w[k + k * w_dim1]), abs(d__2));
 /* IMAX is the row-index of the largest off-diagonal element in */
 /* column K, and COLMAX is its absolute value. */
 /* Determine both COLMAX and IMAX. */
 if (k < *n) {
 ...
 i__1 = imax + k * w_dim1;
 colmax = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w[imax + k * w_dim1]), abs(d__2));
 }
 else {
 colmax = 0.;
 }
 ...
 *info = k;
 ...
 rowmax = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(& w[jmax + (k + 1) * w_dim1]), abs(d__2));
 }
 else {
 rowmax = 0.;
 }
 ...
 dtemp = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w[ itemp + (k + 1) * w_dim1]), abs(d__2));
 if (dtemp > rowmax) {
 rowmax = dtemp;
 jmax = itemp;
 }
 ...
 if (! ((d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w[imax + (k + 1) * w_dim1]), abs(d__2)) < alpha * rowmax)) {
 /* interchange rows and columns K and IMAX, */
 /* use 1-by-1 pivot block */
 kp = imax;
 ...
 /* Copy non-updated column K to column P */
 i__1 = p - k;
 zcopy_(&i__1, &a[k + k * a_dim1], &c__1, &a[p + k * a_dim1], lda);
 i__1 = *n - p + 1;
 zcopy_(&i__1, &a[p + k * a_dim1], &c__1, &a[p + p * a_dim1], & c__1);
 ...
 a[i__1].r = a[i__2].r; a[i__1].i = a[i__2].i; // , expr subst  
 i__1 = kp - k - 1;
 zcopy_(&i__1, &a[k + 1 + kk * a_dim1], &c__1, &a[kp + (k + 1) * a_dim1], lda);
 i__1 = *n - kp + 1;
 zcopy_(&i__1, &a[kp + kk * a_dim1], &c__1, &a[kp + kp * a_dim1], &c__1);
