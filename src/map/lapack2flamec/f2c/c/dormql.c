/* ../netlib/dormql.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
    ...
    *info = 0;
    ...
    if (*info == 0)
    {
        ...
        else
        {
            ...
            /* Computing MIN */
            i__1 = 64;
            i__2 = ilaenv_(&c__1, "DORMQL", ch__1, m, n, k, &c_n1); // , expr subst
            nb = min(i__1,i__2);
            lwkopt = nw * nb;
        }
    ...
    if (nb > 1 && nb < *k)
    {
        ...
        if (*lwork < iws)
        {
            ...
            /* Computing MAX */
            i__1 = 2;
            i__2 = ilaenv_(&c__2, "DORMQL", ch__1, m, n, k, &c_n1); // , expr subst
            nbmin = max(i__1,i__2);
        }
    }
    else
    ...
    else
    {
                ...
                i__ += i__2)
        {
            ...
            i__4 = nq - *k + i__ + ib - 1;
            dlarft_("Backward", "Columnwise", &i__4, &ib, &a[i__ * a_dim1 + 1] , lda, &tau[i__], t, &c__65);
            if (left)
            {
            ...
            /* Apply H or H**T */
            dlarfb_(side, trans, "Backward", "Columnwise", &mi, &ni, &ib, &a[ i__ * a_dim1 + 1], lda, t, &c__65, &c__[c_offset], ldc, & work[1], &ldwork);
            /* L10: */
        }
    }
    work[1] = (doublereal) lwkopt;
