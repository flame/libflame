/* ../netlib/v3.9.0/dhgeqz.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 */
 ...
 if (temp * (ascale * (d__1 = h__[j + 1 + j * h_dim1], abs( d__1))) <= temp2 * (ascale * atol)) {
 ilazr2 = TRUE_;
 }
 }
 ...
 *info = *n + 1;
 ...
 /* (Single shift only.) */
 if ((doublereal) maxit * safmin * (d__1 = h__[ilast + (ilast - 1) * h_dim1], abs(d__1)) < (d__2 = t[ilast - 1 + (ilast - 1) * t_dim1], abs(d__2))) {
 ...
 if ((d__1 = wr / s1 * t[ilast + ilast * t_dim1] - h__[ilast + ilast * h_dim1], abs(d__1)) > (d__2 = wr2 / s2 * t[ilast + ilast * t_dim1] - h__[ilast + ilast * h_dim1], abs(d__2) )) {
 temp = wr;
 wr = wr2;
 wr2 = temp;
 temp = s1;
 ...
 /* Computing MAX */
 /* Computing MAX */
 d__3 = 1., d__4 = abs(wr); d__3 = max(d__3,d__4); d__4 = abs(wi); // ; expr subst  
 ...
 if (abs(wr) > temp) {
 /* Computing MIN */
 d__1 = scale; d__2 = temp / abs(wr); // , expr subst  
 scale = min(d__1,d__2);
 }
 s1 = scale * s1;
 wr = scale * wr;
 ...
 temp2 = (d__1 = s1 * h__[j + j * h_dim1] - wr * t[j + j * t_dim1], abs(d__1));
 tempr = max(temp,temp2);
 if (tempr < 1. && tempr != 0.) {
 temp /= tempr;
 temp2 /= tempr;
 }
 if ((d__1 = ascale * h__[j + 1 + j * h_dim1] * temp, abs(d__1)) <= ascale * atol * temp2) {
 goto L130;
 }
 /* L120: */
 }
 ...
 ilpivt = FALSE_;
 /* Computing MAX */
 d__3 = (d__1 = t[j + 1 + (j + 1) * t_dim1], abs(d__1)); d__4 = (d__2 = t[j + 1 + (j + 2) * t_dim1], abs(d__2)); // , expr subst  
 temp = max(d__3,d__4);
 /* Computing MAX */
 d__3 = (d__1 = t[j + 2 + (j + 1) * t_dim1], abs(d__1)); d__4 = (d__2 = t[j + 2 + (j + 2) * t_dim1], abs(d__2)); // , expr subst  
 temp2 = max(d__3,d__4);
 ...
 if (abs(w11) < abs(u1)) {
 /* Computing MIN */
 d__2 = scale; d__3 = (d__1 = w11 / u1, abs(d__1)); // , expr subst  
 scale = min(d__2,d__3);
 }
 /* Solve */
 u2 = scale * u2 / w22;
