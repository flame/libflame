/* ../netlib/stbrfs.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int stbrfs_(char *uplo, char *trans, char *diag, integer *n, integer *kd, integer *nrhs, real *ab, integer *ldab, real *b, integer *ldb, real *x, integer *ldx, real *ferr, real *berr, real *work, integer *iwork, integer *info)
{
    ...
    char buffer[256];
    snprintf(buffer, 256,"stbrfs inputs: uplo %c, trans %c, diag %c, n %" FLA_IS ", kd %" FLA_IS ", nrhs %" FLA_IS ", ldab %" FLA_IS ", ldb %" FLA_IS ", ldx %" FLA_IS "",*uplo, *trans, *diag, *n, *kd, *nrhs, *ldab, *ldb, *ldx);
    ...
    *info = 0;
            ...
            ++j)
    {
        ...
        stbmv_(uplo, trans, diag, n, kd, &ab[ab_offset], ldab, &work[*n + 1], &c__1);
                ...
                if (nounit)
                {
                            ...
                            ++k)
                    {
                        ...
                        for (i__ = fla_max(i__3,i__4);
                                ...
                                ++i__)
                        {
                            work[i__] += (r__1 = ab[*kd + 1 + i__ - k + k * ab_dim1], f2c_abs(r__1)) * xk;
                            /* L30: */
                        }
                        /* L40: */
                    }
                ...
                if (nounit)
                {
                            ...
                            ++k)
                    {
                        ...
                        for (i__ = fla_max(i__4,i__5);
                                ...
                                ++i__)
                        {
                            s += (r__1 = ab[*kd + 1 + i__ - k + k * ab_dim1], f2c_abs(r__1)) * (r__2 = x[i__ + j * x_dim1], f2c_abs(r__2));
                            /* L110: */
                        }
                        work[k] += s;
                        /* L120: */
                ...
                if (nounit)
                {
                            ...
                            ++k)
                    {
                        ...
                        for (i__ = k;
                                ...
                                ++i__)
                        {
                            s += (r__1 = ab[i__ + 1 - k + k * ab_dim1], f2c_abs( r__1)) * (r__2 = x[i__ + j * x_dim1], f2c_abs( r__2));
                            /* L150: */
                        }
                        work[k] += s;
                        /* L160: */
