...
/*! @file libflame_heevr.cc
 *  @brief Test application to validate heevr() using CPP template interface.
 *  */
...
/*! @brief  heevr_test is function template for heevr() functions.
			T can be scomplex, dcomplex
			Ta can be float, double.
 * @details
 * \b Purpose:
    \verbatim
	  heevr_test is function template for heevr() functions.
	  T can be scomplex, dcomplex
	  Ta can be float, double.
	  
	  heevr_test() function template calls C and CPP based lbrary APIs with
    ...
    Fails the test case if difference > threshold or INFO < 0.
	  ...
	  Complex reference:
	  http://www.netlib.org/lapack/explore-html/d9/de3/group__complex_h_eeigen_gac02f8a55f35ec7767a36b5a9f3ebb0d8.html#gac02f8a55f35ec7767a36b5a9f3ebb0d8
	  Complex double reference:
	  http://www.netlib.org/lapack/explore-html/df/d9a/group__complex16_h_eeigen_ga60dd605c63d7183a4c289a4ab3df6df6.html#ga60dd605c63d7183a4c289a4ab3df6df6
    \endverbatim
...
void heevr_test(int ip)
{
  ...
  allocate_init_buffer(isuppz, isuppzref, 2 * max(1, m), 0);
  ...
  if ((lwork == -1) || (lrwork == -1) || (liwork == -1)) {
    PRINTF("lwork/lrwork/liwork is -1, so call heevr() to get the array" \
           " sizes.\n");
    T worksize = {0};
    Ta rworksize = 0.0;
    integer iworksize = 0;
    ...
    // Call CPP function
    libflame::heevr<T, Ta>(&jobz, &range, &uplo, &n, abuff, 
  ...
  if ((lrwork != -1) && (lrwork < max(1, 24*n))) {
    PRINTF("lrwork should be >= max(1, 24*n). Please correct the input data.\n");
  }
  if ((liwork != -1) && (liwork < max(1, 10*n))) {
    PRINTF("lwork should be >= max(1, 10*n). Please correct the input data.\n");
  }
  ...
  integer arraysize = sizeof(arrayname);
    ...
    print_array<integer>(arrayname, isuppz, (2 * max(1, m)));
    strncpy(arrayname, "ISUPPZ ref input", arraysize);
    print_array<integer>(arrayname, isuppzref, (2 * max(1, m)));
  ...
  if ((info_cpp >= 0) && (info_ref >= 0)) {
    ...
    diff += computeError<integer>(2, max(1, mtemp), isuppzref, isuppz);
...
TEST(LAPACKCPP_heevr, CHEEVR) {
  for (short int index = 0; index < NUM_SUB_TESTS; index++) {
	  PRINTF("Index: %d\n", index);
    heevr_test<scomplex, float> (index);
  }
}

/* Use TEST macro and call C++ test function template with
   dcomplex and double as typenames.*/
TEST(LAPACKCPP_heevr, ZHEEVR) {
  for (short int index = 0; index < NUM_SUB_TESTS; index++) {
	  PRINTF("Index: %d\n", index);
    heevr_test<dcomplex, double> (index);
  }
}