/* ../netlib/ssteqr.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
int ssteqr_(char *compz, integer *n, real *d__, real *e, real *z__, integer *ldz, real *work, integer *info)
{
    ...
    integer lendsv;
    extern /* Subroutine */
    int slartg_(real *, real *, real *, real *, real * ), slaset_(char *, integer *, integer *, real *, real *, real *, integer *);
    real ssfmin;
...
L60:
        ...
        if (m == l + 1)
        {
            if (icompz > 0)
            {
                ...
                work[l] = c__;
                work[*n - 1 + l] = s;
                slasr_("R", "V", "B", n, &c__2, &work[l], &work[*n - 1 + l], & z__[l * z_dim1 + 1], ldz);
            }
            else
            {
...
L110:
        ...
        if (m == l - 1)
        {
            if (icompz > 0)
            {
                ...
                work[m] = c__;
                work[*n - 1 + m] = s;
                slasr_("R", "V", "F", n, &c__2, &work[m], &work[*n - 1 + m], & z__[(l - 1) * z_dim1 + 1], ldz);
            }
            else
            {
