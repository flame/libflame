...
/* ../netlib/dsyrfsx.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
,inf) * norm(Z,inf)) for some */
 ...
 /* ===================================================================== */
 /* Subroutine */
 int dsyrfsx_(char *uplo, char *equed, integer *n, integer * nrhs, doublereal *a, integer *lda, doublereal *af, integer *ldaf, integer *ipiv, doublereal *s, doublereal *b, integer *ldb, doublereal *x, integer *ldx, doublereal *rcond, doublereal *berr, integer * n_err_bnds__, doublereal *err_bnds_norm__, doublereal * err_bnds_comp__, integer *nparams, doublereal *params, doublereal * work, integer *iwork, integer *info) {
 AOCL_DTL_TRACE_ENTRY(AOCL_DTL_LEVEL_TRACE_5);
#if AOCL_DTL_LOG_ENABLE 
 char buffer[256]; 
 snprintf(buffer, 256,"dsyrfsx inputs: uplo %c, equed %c, n %" FLA_IS ", nrhs %" FLA_IS ", lda %" FLA_IS ", ldaf %" FLA_IS ", ipiv %" FLA_IS ", ldb %" FLA_IS ", ldx %" FLA_IS ", n_err_bnds__ %" FLA_IS ", nparams %" FLA_IS "",*uplo, *equed, *n, *nrhs, *lda, *ldaf, *ipiv, *ldb, *ldx, *n_err_bnds__, *nparams);
 ...
 integer prec_type__;
 extern doublereal dla_syrcond_(char *, integer *, doublereal *, integer * , doublereal *, integer *, integer *, integer *, doublereal *, integer *, doublereal *, integer *);
 doublereal cwise_wrong__;
 extern /* Subroutine */
 int dla_syrfsx_extended_(integer *, char *, integer *, integer *, doublereal *, integer *, doublereal *, integer *, integer *, logical *, doublereal *, doublereal *, integer *, doublereal *, integer *, doublereal *, integer *, doublereal *, doublereal *, doublereal *, doublereal *, doublereal *, doublereal *, doublereal *, integer *, doublereal *, doublereal *, logical *, integer *);
 char norm[1];
 ...
 extern /* Subroutine */
 int dsycon_(char *, integer *, doublereal *, integer *, integer *, doublereal *, doublereal *, doublereal *, integer *, integer *);
 ...
 *(unsigned char *)norm = 'I';
 ...
 dla_syrfsx_extended_(&prec_type__, uplo, n, nrhs, &a[a_offset], lda, &af[af_offset], ldaf, &ipiv[1], &rcequ, &s[1], &b[b_offset], ldb, &x[x_offset], ldx, &berr[1], &n_norms__, & err_bnds_norm__[err_bnds_norm_offset], &err_bnds_comp__[ err_bnds_comp_offset], &work[*n + 1], &work[1], &work[(*n << 1) + 1], &work[1], rcond, &ithresh, &rthresh, & unstable_thresh__, &ignore_cwise__, info);
 }
 /* Computing MAX */
 d__1 = 10.; d__2 = sqrt((doublereal) (*n)); // , expr subst  
 err_lbnd__ = max(d__1,d__2) * dlamch_("Epsilon");
 if (*n_err_bnds__ >= 1 && n_norms__ >= 1) {
 /* Compute scaled normwise condition number cond(A*C). */
 if (rcequ) {
 rcond_tmp__ = dla_syrcond_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &ipiv[1], &c_n1, &s[1], info, &work[1], &iwork[1]);
 }
 else {
 rcond_tmp__ = dla_syrcond_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &ipiv[1], &c__0, &s[1], info, &work[1], &iwork[1]);
 }
 i__1 = *nrhs;
 for (j = 1;
 j <= i__1;
 ...
 *info = *n + j;
 ...
 rcond_tmp__ = dla_syrcond_(uplo, n, &a[a_offset], lda, &af[ af_offset], ldaf, &ipiv[1], &c__1, &x[j * x_dim1 + 1], info, &work[1], &iwork[1]);
 }
 else {
 rcond_tmp__ = 0.;
 }
