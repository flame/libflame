/* ../netlib/v3.9.0/dlanv2.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *sn = 0.;
 ...
 p = temp * .5;
 /* Computing MAX */
 d__1 = abs(*b); d__2 = abs(*c__); // , expr subst  
 bcmax = max(d__1,d__2);
 /* Computing MIN */
 d__1 = abs(*b); d__2 = abs(*c__); // , expr subst  
