/* ../netlib/zlansp.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 on Linux or Unix systems, link with .../path/to/libf2c.a -lm or, if you install libf2c.a in a standard place, with -lf2c -lm -- in that order, at the end of the command line, as in cc *.o -lf2c -lm Source for libf2c is in /netlib/f2c/libf2c.zip, e.g., http://www.netlib.org/f2c/libf2c.zip */
#include "FLA_f2c.h" /* Table of constant values */
static integer c__1 = 1;
/* > \brief \b ZLANSP returns the value of the 1-norm, or the Frobenius norm, or the infinity norm, or the ele ment of largest absolute value of a symmetric matrix supplied in packed form. */
/* =========== DOCUMENTATION =========== */
...
/* > \verbatim */
/* > */
/* > ZLANSP = ( max(f2c_dabs(A(i,j))), NORM = 'M' or 'm' */
/* > ( */
...
doublereal zlansp_(char *norm, char *uplo, integer *n, doublecomplex *ap, doublereal *work)
{
    ...
    else if (lsame_(norm, "F") || lsame_(norm, "E"))
    {
                ...
                ++i__)
        {
            ...
            {
                i__2 = k;
                absa = (d__1 = ap[i__2].r, f2c_dabs(d__1));
                if (scale < absa)
                {
                    /* Computing 2nd power */
                    d__1 = scale / absa;
            ...
            if (d_imag(&ap[k]) != 0.)
            {
                absa = (d__1 = d_imag(&ap[k]), f2c_dabs(d__1));
                if (scale < absa)
                {
                    /* Computing 2nd power */
                    d__1 = scale / absa;
