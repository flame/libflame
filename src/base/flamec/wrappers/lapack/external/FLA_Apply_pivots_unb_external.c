...
FLA_Error FLA_Apply_pivots_unb_external_ts( FLA_cntl_init_s *FLA_cntl_init_i, FLA_Side side, FLA_Trans trans, FLA_Obj p, FLA_Obj A )
{
  ...
  if ( FLA_Check_error_level_ts(FLA_cntl_init_i) == FLA_FULL_ERROR_CHECKING )
    FLA_Apply_pivots_check_ts( FLA_cntl_init_i, side, trans, p, A );
...
{
  return FLA_Apply_pivots_unb_external_ts( FLA_cntl_init_i, FLA_LEFT, FLA_NO_TRANSPOSE, p, A );
}
#endif
