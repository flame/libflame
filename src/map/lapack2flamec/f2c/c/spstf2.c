/* ../netlib/spstf2.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 on Linux or Unix systems, link with .../path/to/libf2c.a -lm or, if you install libf2c.a in a standard place, with -lf2c -lm -- in that order, at the end of the command line, as in cc *.o -lf2c -lm Source for libf2c is in /netlib/f2c/libf2c.zip, e.g., http://www.netlib.org/f2c/libf2c.zip */
#include "FLA_f2c.h" /* Table of constant values */
static integer c__1 = 1;
static real c_b16 = -1.f;
static real c_b18 = 1.f;
/* > \brief \b SPSTF2 computes the Cholesky factorization with complete pivoting of a real symmetric or comple x Hermitian positive semi-definite matrix. */
/* =========== DOCUMENTATION =========== */
    ...
    if (upper)
    {
                ...
                ++j)
        {
            ...
            if (j != pvt)
            {
                ...
                {
                    i__2 = *n - pvt;
                    sswap_(&i__2, &a[j + (pvt + 1) * a_dim1], lda, &a[pvt + ( pvt + 1) * a_dim1], lda);
                }
                i__2 = pvt - j - 1;
            ...
            if (j < *n)
            {
                i__2 = j - 1;
                i__3 = *n - j;
                sgemv_("Trans", &i__2, &i__3, &c_b16, &a[(j + 1) * a_dim1 + 1] , lda, &a[j * a_dim1 + 1], &c__1, &c_b18, &a[j + (j + 1) * a_dim1], lda);
                i__2 = *n - j;
                r__1 = 1.f / ajj;
    ...
    else
    {
                ...
                ++j)
        {
            ...
            if (j != pvt)
            {
                ...
                {
                    i__2 = *n - pvt;
                    sswap_(&i__2, &a[pvt + 1 + j * a_dim1], &c__1, &a[pvt + 1 + pvt * a_dim1], &c__1);
                }
                i__2 = pvt - j - 1;
            ...
            if (j < *n)
            {
                i__2 = *n - j;
                i__3 = j - 1;
                sgemv_("No Trans", &i__2, &i__3, &c_b16, &a[j + 1 + a_dim1], lda, &a[j + a_dim1], lda, &c_b18, &a[j + 1 + j * a_dim1], &c__1);
                i__2 = *n - j;
                r__1 = 1.f / ajj;
