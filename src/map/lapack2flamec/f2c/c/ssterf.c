/* ../netlib/v3.9.0/ssterf.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *info = -1;
 ...
 m <= i__1;
 ++m) {
 if ((r__3 = e[m], abs(r__3)) <= sqrt((r__1 = d__[m], abs(r__1))) * sqrt((r__2 = d__[m + 1], abs(r__2))) * eps) {
 e[m] = 0.f;
 goto L30;
 }
 /* L20: */
 ...
 m <= i__1;
 ++m) {
 if ((r__2 = e[m], abs(r__2)) <= eps2 * (r__1 = d__[m] * d__[m + 1], abs(r__1))) {
 goto L70;
 }
 /* L60: */
 }
 ...
 m >= i__1;
 --m) {
 if ((r__2 = e[m - 1], abs(r__2)) <= eps2 * (r__1 = d__[m] * d__[m - 1], abs(r__1))) {
 goto L120;
 }
 /* L110: */
 }
