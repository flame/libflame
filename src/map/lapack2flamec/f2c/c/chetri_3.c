 ...
 */
 ...
 char buffer[256]; 
#if FLA_ENABLE_ILP64 
 snprintf(buffer, 256,"chetri_3 inputs: uplo %c, n %lld, lda %lld, ipiv %lld, lwork %lld",*uplo, *n, *lda, *ipiv, *lwork);
#else 
 snprintf(buffer, 256,"chetri_3 inputs: uplo %c, n %d, lda %d, ipiv %d, lwork %d",*uplo, *n, *lda, *ipiv, *lwork);
#endif
 ...
 *info = 0;
 ...
 /* Computing MAX */
 i__1 = 1; i__2 = ilaenv_(&c__1, "CHETRI_3", uplo, n, &c_n1, &c_n1, &c_n1); // , expr subst  
 nb = max(i__1,i__2);
 lwkopt = (*n + nb + 1) * (nb + 3);
