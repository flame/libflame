/* ../netlib/zlanhb.f -- translated by f2c (version 20100827). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 on Linux or Unix systems, link with .../path/to/libf2c.a -lm or, if you install libf2c.a in a standard place, with -lf2c -lm -- in that order, at the end of the command line, as in cc *.o -lf2c -lm Source for libf2c is in /netlib/f2c/libf2c.zip, e.g., http://www.netlib.org/f2c/libf2c.zip */
#include "FLA_f2c.h" /* Table of constant values */
static integer c__1 = 1;
/* > \brief \b ZLANHB returns the value of the 1-norm, or the Frobenius norm, or the infinity norm, or the ele ment of largest absolute value of a Hermitian band matrix. */
/* =========== DOCUMENTATION =========== */
...
/* > \verbatim */
/* > */
/* > ZLANHB = ( max(f2c_dabs(A(i,j))), NORM = 'M' or 'm' */
/* > ( */
...
doublereal zlanhb_(char *norm, char *uplo, integer *n, integer *k, doublecomplex *ab, integer *ldab, doublereal *work)
{
    ...
    else if (lsame_(norm, "M"))
    {
        ...
        if (lsame_(uplo, "U"))
        {
                    ...
                    ++j)
            {
                ...
                i__3 = *k + 1 + j * ab_dim1;
                sum = (d__1 = ab[i__3].r, f2c_dabs(d__1));
                if (value < sum || disnan_(&sum))
                {
                    value = sum;
                }
    ...
    else if (lsame_(norm, "I") || lsame_(norm, "O") || *(unsigned char *)norm == '1')
    {
        ...
        if (lsame_(uplo, "U"))
        {
                    ...
                    ++j)
            {
                ...
                i__4 = *k + 1 + j * ab_dim1;
                work[j] = sum + (d__1 = ab[i__4].r, f2c_dabs(d__1));
                /* L60: */
            }
            i__1 = *n;
            for (i__ = 1;
        ...
        else
        {
            ...
            {
                i__4 = j * ab_dim1 + 1;
                sum = work[j] + (d__1 = ab[i__4].r, f2c_dabs(d__1));
                l = 1 - j;
                /* Computing MIN */
                i__3 = *n;
                i__2 = j + *k; // , expr subst
    ...
    else if (lsame_(norm, "F") || lsame_(norm, "E"))
    {
                ...
                ++j)
        {
            ...
            {
                i__4 = l + j * ab_dim1;
                absa = (d__1 = ab[i__4].r, f2c_dabs(d__1));
                if (scale < absa)
                {
                    /* Computing 2nd power */
                    d__1 = scale / absa;
