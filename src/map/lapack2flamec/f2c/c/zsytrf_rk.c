/* ../netlib/v3.9.0/zsytrf_rk.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zsytrf_ rk.f"> */
 /* > [TGZ]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zsytrf_ rk.f"> */
 /* > [ZIP]</a> */
 /* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zsytrf_ rk.f"> */
 /* > [TXT]</a> */
 /* > \endhtmlonly */
 /* Definition: */
 /* =========== */
 ...
 *info = -8;
 ...
 nb = max(i__1,1);
 /* Computing MAX */
 i__1 = 2; i__2 = ilaenv_(&c__2, "ZSYTRF_RK", uplo, n, &c_n1, & c_n1, &c_n1); // , expr subst  
 nbmin = max(i__1,i__2);
 }
 }
 else {
 ...
 */
 ...
 zlasyf_rk_(uplo, &k, &nb, &kb, &a[a_offset], lda, &e[1], &ipiv[1] , &work[1], &ldwork, &iinfo);
 }
 else {
 ...
 */
 ...
 i__1 = *n - k + 1;
 zlasyf_rk_(uplo, &i__1, &nb, &kb, &a[k + k * a_dim1], lda, &e[k], &ipiv[k], &work[1], &ldwork, &iinfo);
 }
 else {
 /* Use unblocked code to factorize columns k:n of A */
 i__1 = *n - k + 1;
 zsytf2_rk_(uplo, &i__1, &a[k + k * a_dim1], lda, &e[k], &ipiv[k], &iinfo);
 kb = *n - k + 1;
 }
 /* Set INFO on the first occurrence of a zero pivot */
 if (*info == 0 && iinfo > 0) {
