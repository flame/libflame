/* ../netlib/v3.9.0/zlarrv.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
;
 ...
 /* > \brief \b ZLARRV computes the eigenvectors of the tridiagonal matrix T = L D LT given L, D and the eigenv alues of L D LT. */
 /* =========== DOCUMENTATION =========== */
 ...
 */
 ...
 logical stp2ii;
 extern /* Subroutine */
 int zlar1v_(integer *, integer *, integer *, doublereal *, doublereal *, doublereal *, doublereal *, doublereal *, doublereal *, doublereal *, doublecomplex *, logical *, integer *, doublereal *, doublereal *, integer *, integer *, doublereal *, doublereal *, doublereal *, doublereal *) ;
 doublereal lambda;
 ...
 *rtol2 = eps * 4.;
 ...
 z__[i__2].r = 1.; z__[i__2].i = 0.; // , expr subst  
 ...
 *info = -1;
 ...
 if (oldfst > 1) {
 /* Computing MAX */
 d__1 = wgap[wbegin + oldfst - 2]; d__2 = w[wbegin + oldfst - 1] - werr[wbegin + oldfst - 1] - w[ wbegin + oldfst - 2] - werr[wbegin + oldfst - 2]; // , expr subst  
 wgap[wbegin + oldfst - 2] = max(d__1,d__2);
 }
 if (wbegin + oldlst - 1 < wend) {
 /* Computing MAX */
 d__1 = wgap[wbegin + oldlst - 1]; d__2 = w[wbegin + oldlst] - werr[wbegin + oldlst] - w[wbegin + oldlst - 1] - werr[wbegin + oldlst - 1]; // , expr subst  
 ...
 else if (wgap[wbegin + j - 1] >= *minrgp * (d__1 = work[ wbegin + j - 1], abs(d__1))) {
 ...
 if (newfst == 1) {
 /* Computing MAX */
 d__1 = 0.; d__2 = w[wbegin] - werr[wbegin] - *vl; // , expr subst  
 lgap = max(d__1,d__2);
 }
 else {
 ...
 offset = indexw[wbegin] - 1;
 dlarrb_(&in, &d__[ibegin], &work[indlld + ibegin - 1], &p, &p, &rqtol, &rqtol, &offset, & work[wbegin], &wgap[wbegin], &werr[wbegin] , &work[indwrk], &iwork[iindwk], pivmin, & spdiam, &in, &iinfo);
 /* L55: */
 }
 ...
 dlarrf_(&in, &d__[ibegin], &l[ibegin], &work[indld + ibegin - 1], &newfst, &newlst, &work[wbegin], &wgap[wbegin], &werr[wbegin], &spdiam, &lgap, &rgap, pivmin, &tau, &work[indin1], &work[ indin2], &work[indwrk], &iinfo);
 ...
 i__6 = indin1 + k - 1;
 z__1.r = work[i__6]; z__1.i = 0.; // , expr subst  
 z__[i__5].r = z__1.r; z__[i__5].i = z__1.i; // , expr subst  
 ...
 i__5 = indin1 + in - 1;
 z__1.r = work[i__5]; z__1.i = 0.; // , expr subst  
 ...
 z__1.r = ssigma; z__1.i = 0.; // , expr subst  
 z__[i__4].r = z__1.r; z__[i__4].i = z__1.i; // , expr subst  
 ...
 *info = -2;
 ...
 /* Computing MAX */
 d__1 = abs(left); d__2 = abs(right); // , expr subst  
 ...
 offset = indexw[wbegin] - 1;
 d__1 = eps * 2.;
 dlarrb_(&in, &d__[ibegin], &work[indlld + ibegin - 1], &indeig, &indeig, &c_b28, &d__1, & offset, &work[wbegin], &wgap[wbegin], & werr[wbegin], &work[indwrk], &iwork[ iindwk], pivmin, &spdiam, &itmp1, &iinfo);
 if (iinfo != 0) {
 *info = -3;
 return 0;
 }
 ...
 L__1 = ! usedbs;
 zlar1v_(&in, &c__1, &in, &lambda, &d__[ibegin], &l[ ibegin], &work[indld + ibegin - 1], &work[ indlld + ibegin - 1], pivmin, &gaptol, &z__[ ibegin + windex * z_dim1], &L__1, &negcnt, & ztz, &mingma, &iwork[iindr + windex], &isuppz[ (windex << 1) - 1], &nrminv, &resid, &rqcorr, &work[indwrk]);
 if (iter == 0) {
 bstres = resid;
 bstw = lambda;
 }
 ...
 /* Computing MIN */
 i__4 = isupmn; i__5 = isuppz[(windex << 1) - 1]; // , expr subst  
 isupmn = min(i__4,i__5);
 /* Computing MAX */
 i__4 = isupmx; i__5 = isuppz[windex * 2]; // , expr subst  
 ...
 *info = 5;
 ...
 z__[i__5].r = 0.; z__[i__5].i = 0.; // , expr subst  
 /* L122: */
 }
 }
 if (isupmx > zto) {
 ...
 if (k > 1) {
 /* Computing MAX */
 d__1 = wgap[windmn]; d__2 = w[windex] - werr[ windex] - w[windmn] - werr[windmn]; // , expr subst  
 wgap[windmn] = max(d__1,d__2);
 }
 if (windex < wend) {
 /* Computing MAX */
 d__1 = savgap; d__2 = w[windpl] - werr[windpl] - w[windex] - werr[windex]; // , expr subst  
