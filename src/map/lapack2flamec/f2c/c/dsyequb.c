/* ../netlib/v3.9.0/dsyequb.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 *amax = 0.;
 ...
 ++i__) {
 /* Computing MAX */
 d__2 = s[i__]; d__3 = (d__1 = a[i__ + j * a_dim1], abs(d__1)); // , expr subst  
 s[i__] = max(d__2,d__3);
 /* Computing MAX */
 d__2 = s[j]; d__3 = (d__1 = a[i__ + j * a_dim1], abs(d__1)); // , expr subst  
 s[j] = max(d__2,d__3);
 /* Computing MAX */
 d__2 = *amax; d__3 = (d__1 = a[i__ + j * a_dim1], abs(d__1)); // , expr subst  
 *amax = max(d__2,d__3);
 }
 /* Computing MAX */
 d__2 = s[j]; d__3 = (d__1 = a[j + j * a_dim1], abs(d__1)); // , expr subst  
 s[j] = max(d__2,d__3);
 /* Computing MAX */
 d__2 = *amax; d__3 = (d__1 = a[j + j * a_dim1], abs(d__1)); // , expr subst  
 *amax = max(d__2,d__3);
 }
 }
 else {
 ...
 *amax = max(d__2,d__3);
 ...
 i__ <= i__2;
 ++i__) {
 work[i__] += (d__1 = a[i__ + j * a_dim1], abs(d__1)) * s[ j];
 work[j] += (d__1 = a[i__ + j * a_dim1], abs(d__1)) * s[ i__];
 }
