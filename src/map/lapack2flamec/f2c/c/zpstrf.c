/* ../netlib/v3.9.0/zpstrf.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
...
;
 ...
 /* > \brief \b ZPSTRF computes the Cholesky factorization with complete pivoting of a complex Hermitian positi ve semidefinite matrix. */
 /* =========== DOCUMENTATION =========== */
 ...
 *info = -4;
 ...
 if (nb <= 1 || nb >= *n) {
 /* Use unblocked code */
 zpstf2_(uplo, n, &a[a_dim1 + 1], lda, &piv[1], rank, tol, &work[1], info);
 goto L230;
 }
 else {
 /* Initialize PIV */
 ...
 *info = 1;
 ...
 i__5 = j - 1 + i__ * a_dim1;
 z__1.r = z__2.r * a[i__5].r - z__2.i * a[i__5].i; z__1.i = z__2.r * a[i__5].i + z__2.i * a[ i__5].r; // , expr subst  
 work[i__] += z__1.r;
 }
 i__5 = i__ + i__ * a_dim1;
 ...
 i__4 = j + j * a_dim1;
 a[i__4].r = ajj; a[i__4].i = 0.; // , expr subst  
 goto L220;
 }
 }
 if (j != pvt) {
 ...
 i__4 = pvt + pvt * a_dim1;
 i__5 = j + j * a_dim1;
 a[i__4].r = a[i__5].r; a[i__4].i = a[i__5].i; // , expr subst  
 i__4 = j - 1;
 zswap_(&i__4, &a[j * a_dim1 + 1], &c__1, &a[pvt * a_dim1 + 1], &c__1);
 if (pvt < *n) {
 i__4 = *n - pvt;
 zswap_(&i__4, &a[j + (pvt + 1) * a_dim1], lda, &a[ pvt + (pvt + 1) * a_dim1], lda);
 }
 i__4 = pvt - 1;
 for (i__ = j + 1;
 i__ <= i__4;
 ...
 ztemp.r = z__1.r; ztemp.i = z__1.i; // , expr subst  
 i__5 = j + i__ * a_dim1;
 d_cnjg(&z__1, &a[i__ + pvt * a_dim1]);
 a[i__5].r = z__1.r; a[i__5].i = z__1.i; // , expr subst  
 i__5 = i__ + pvt * a_dim1;
 a[i__5].r = ztemp.r; a[i__5].i = ztemp.i; // , expr subst  
 /* L140: */
 }
 i__4 = j + pvt * a_dim1;
 ...
 zgemv_("Trans", &i__4, &i__5, &z__1, &a[k + (j + 1) * a_dim1], lda, &a[k + j * a_dim1], &c__1, & c_b1, &a[j + (j + 1) * a_dim1], lda);
 i__4 = j - 1;
 zlacgv_(&i__4, &a[j * a_dim1 + 1], &c__1);
 i__4 = *n - j;
 d__1 = 1. / ajj;
 ...
 if (k + jb <= *n) {
 i__3 = *n - j + 1;
 zherk_("Upper", "Conj Trans", &i__3, &jb, &c_b29, &a[k + j * a_dim1], lda, &c_b30, &a[j + j * a_dim1], lda);
 }
 /* L160: */
 }
 }
 ...
 a[i__4].r = a[i__5].r; a[i__4].i = a[i__5].i; // , expr subst  
 i__4 = j - 1;
 zswap_(&i__4, &a[j + a_dim1], lda, &a[pvt + a_dim1], lda);
 if (pvt < *n) {
 i__4 = *n - pvt;
 zswap_(&i__4, &a[pvt + 1 + j * a_dim1], &c__1, &a[ pvt + 1 + pvt * a_dim1], &c__1);
 }
 i__4 = pvt - 1;
 for (i__ = j + 1;
 i__ <= i__4;
 ...
 i__5 = j - k;
 z__1.r = -1.; z__1.i = -0.; // , expr subst  
 zgemv_("No Trans", &i__4, &i__5, &z__1, &a[j + 1 + k * a_dim1], lda, &a[j + k * a_dim1], lda, &c_b1, &a[j + 1 + j * a_dim1], &c__1);
 i__4 = j - 1;
 zlacgv_(&i__4, &a[j + a_dim1], lda);
 i__4 = *n - j;
 d__1 = 1. / ajj;
 ...
 if (k + jb <= *n) {
 i__3 = *n - j + 1;
 zherk_("Lower", "No Trans", &i__3, &jb, &c_b29, &a[j + k * a_dim1], lda, &c_b30, &a[j + j * a_dim1], lda);
 }
 /* L210: */
 }
 }
 ...
 *rank = *n;
 goto L230;
 L220: /* Rank is the number of steps completed. Set INFO = 1 to signal */
