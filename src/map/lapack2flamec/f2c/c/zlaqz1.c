/* zlaqz1.f -- translated by f2c (version 20160102). You must link the resulting object file with libf2c: on Microsoft Windows system, link with libf2c.lib;
 ...
 b[i__1].r = 0.; b[i__1].i = 0.; // , expr subst  
 i__1 = *ihi - *istartm;
 zrot_(&i__1, &b[*istartm + *ihi * b_dim1], &c__1, &b[*istartm + (*ihi - 1) * b_dim1], &c__1, &c__, &s);
 i__1 = *ihi - *istartm + 1;
 zrot_(&i__1, &a[*istartm + *ihi * a_dim1], &c__1, &a[*istartm + (*ihi - 1) * a_dim1], &c__1, &c__, &s);
 if (*ilz) {
 zrot_(nz, &z__[(*ihi - *zstart + 1) * z_dim1 + 1], &c__1, &z__[(* ihi - 1 - *zstart + 1) * z_dim1 + 1], &c__1, &c__, &s);
 }
 }
 else {
 ...
 b[i__1].r = 0.; b[i__1].i = 0.; // , expr subst  
 i__1 = *k + 2 - *istartm + 1;
 zrot_(&i__1, &a[*istartm + (*k + 1) * a_dim1], &c__1, &a[*istartm + * k * a_dim1], &c__1, &c__, &s);
 i__1 = *k - *istartm + 1;
 zrot_(&i__1, &b[*istartm + (*k + 1) * b_dim1], &c__1, &b[*istartm + * k * b_dim1], &c__1, &c__, &s);
 if (*ilz) {
 zrot_(nz, &z__[(*k + 1 - *zstart + 1) * z_dim1 + 1], &c__1, &z__[( *k - *zstart + 1) * z_dim1 + 1], &c__1, &c__, &s);
 }
 ...
 i__1 = *istopm - *k;
 zrot_(&i__1, &a[*k + 1 + (*k + 1) * a_dim1], lda, &a[*k + 2 + (*k + 1) * a_dim1], lda, &c__, &s);
 i__1 = *istopm - *k;
 zrot_(&i__1, &b[*k + 1 + (*k + 1) * b_dim1], ldb, &b[*k + 2 + (*k + 1) * b_dim1], ldb, &c__, &s);
 if (*ilq) {
 d_cnjg(&z__1, &s);
 zrot_(nq, &q[(*k + 1 - *qstart + 1) * q_dim1 + 1], &c__1, &q[(*k + 2 - *qstart + 1) * q_dim1 + 1], &c__1, &c__, &z__1);
 }
 }
 /* End of ZLAQZ1 */
 return 0;
